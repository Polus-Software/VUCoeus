...
public class TableSorterCodeTable extends TableMap {
    int indexes[];
     ...
     */
    public TableSorterCodeTable(TableModel model) {
        setModel(model);
    }
    /**
     * This will construct the new Table Sorter
     * @param model TableModel
     */
    public TableSorterCodeTable(TableModel model, boolean isCaseSensitive ) {
    ...
    /**
     * This will set the Table model & set is Case Sensitive or Not
     * @param model TableModel
     * @param isCaseSensitive 
     */
    public void setModel(TableModel model, boolean isCaseSensitive) {
        this.isCaseSensitive = isCaseSensitive;
        hasSorted = false;
        super.setModel(model);
        reallocateIndexes();
...
//        }
    ...
    public void addMouseListenerToHeaderInTable(JTable table) {
        final TableSorterCodeTable sorter = this;
        ...
        MouseAdapter listMouseListener = new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getClickCount() == 1 && column != -1) {
                    ...
                    if( selRow != -1 ) {
                        tableSelected = true;
//                        tableHiddenIndex = selRow;
//                        tableHiddenIndex = Integer.parseInt((String) currentTable.getValueAt(
//                                                selRow,currentTable.getColumnCount() -1 ) );
                    ...
                    tableHiddenIndex = indexes[selRow];
                    hasSorted = true;
                    sorter.sortByColumn(column, ascending);
                    if( tableSelected ) {
                        selRow = getSelectedIndex(tableHiddenIndex);
    ...
    public void insertRow(int row, Vector data) {
        if( row < indexes.length ) {
            ...
            int indexRow = getInsertedRowIndex();
            super.insertRow( indexRow, data );
        }
    ...
    private int getViewIndex( int row ) {
        ...
        for( int i=0; i< rowCount; i++ ){
            if( row == Integer.parseInt( (String)currentTable.getValueAt(i,
                            currentTable.getColumnCount()-1) ) ) {
                return i;
            }
        }
    ...
    // should be called before adding / inserting a new row only
    // this method now only handles adding a new row (inseting a new row to the end).
    // need some modification if inserting a new row to other position instead of 
    // last one.
    private int getInsertedRowIndex(){
        int lastRow = currentTable.getRowCount();
        int[] tempIdexes = new int[indexes.length + 1];
        for( int i=0; i< indexes.length; i++ ){
            tempIdexes[i] = indexes[i];
        }
        tempIdexes[indexes.length] = lastRow;
        indexes = tempIdexes;
        return lastRow; // in code table wants adding a new row at last row
    ...
    private void removeRowIndex(int row){
        int[] tempIdexes = new int[indexes.length - 1];
        int j = 0;
        for( int i=0; i< indexes.length; i++ ){
            if (i != row)
            {   
                if ( indexes[i] > indexes[row])
                    tempIdexes[j++] = indexes[i] - 1;
                else
                    tempIdexes[j++] = indexes[i];
            }
        }
