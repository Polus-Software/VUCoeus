/*
 * @(#)RolodexTextField.java
 *
 * Copyright (c) Massachusetts Institute of Technology
 * 77 Massachusetts Avenue, Cambridge, MA 02139-4307
 * All rights reserved.
 *
 * Created on March 7, 2003, 7:44 PM
 */
...
/**
 * This class is used to have a text field specific to Rolodex details, which will show
 * the detail window for the given id on double click and also used to validate the
 * entered text with the database.
 * @author  ravikanth
 */
public class RolodexTextField extends CoeusTextField implements IdValidator {
    
    private static final String SERVLET_NAME = "rolMntServlet";
    private static final char REQUEST_TYPE = 'V';
    
    private GenerateDetailWindow detailWindow;
    private String detailId;
    
    /** Creates a new instance of RolodexTextField */
    public RolodexTextField() {
        super();
        init();
    }
    
    /**
     * This will construct the RolodexTextField with defult text
     * @param text String representing the default value
     */
    public RolodexTextField(String text){
        super(text);
        init();
    }
    
    /**
     * This will construct the RolodexTextField with defult length
     * @param columns integer which represents the length allowed to type in
     * this textfield.
     */
    public RolodexTextField(int columns){
        super(columns);
        init();
    }
    
    /**
     * This will construct the RolodexTextField with defult text & length
     * @param text String representing the default value
     * @param columns integer which represents the length allowed to type in
     * this textfield.
     */
    public RolodexTextField(String text, int columns){
        super(text,columns);
        init();
    }
    
    /**
     * This will construct the RolodexTextField with defult document, text & length
     * @param doc Document for this textfield.
     * @param text String representing the default value
     * @param columns integer which represents the length allowed to type in
     * this textfield.
     */
    public RolodexTextField(Document doc, String text, int columns){
        ...
        addMouseListener(new MouseAdapter(){
            public void mouseClicked(MouseEvent me){
                if (me.getClickCount() == 2) {
                    try{
                        detailWindow.showRolodexDetails(detailId);
                    } catch ( Exception ex){
