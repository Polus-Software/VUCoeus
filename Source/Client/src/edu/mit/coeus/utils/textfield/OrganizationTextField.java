/*
 * @(#)OrganizationTextField.java
 *
 * Copyright (c) Massachusetts Institute of Technology
 * 77 Massachusetts Avenue, Cambridge, MA 02139-4307
 * All rights reserved.
 *
 * Created on March 7, 2003, 7:34 PM
 */
...
/**
 * This class is used to have a text field specific to Organization details, which will show 
 * the detail window for the given id on double click and also used to validate the
 * entered text with the database.
 * @author  ravikanth
 */
public class OrganizationTextField extends CoeusTextField implements IdValidator {
    
    private static final String SERVLET_NAME = "orgMntServlet";
    private static final char REQUEST_TYPE = 'D';
    
    private GenerateDetailWindow detailWindow;
    private String detailId;
    
    /** Creates a new instance of OrganizationTextField */
    public OrganizationTextField() {
        super();
        init();
    }
    
    /**
     * This will construct the OrganizationTextField with defult text
     * @param text String representing the default value
     */
    public OrganizationTextField(String text){
        super(text);
        init();
    }
    
    /**
     * This will construct the OrganizationTextField with defult length
     * @param columns integer which represents the length allowed to type in
     * this textfield.
     */
    public OrganizationTextField(int columns){
        super(columns);
        init();
    }
    
    /**
     * This will construct the OrganizationTextField with defult text & length
     * @param text String representing the default value
     * @param columns integer which represents the length allowed to type in
     * this textfield.
     */
    public OrganizationTextField(String text, int columns){
        super(text,columns);
        init();
    }
    
    /**
     * This will construct the OrganizationTextField with defult document, text & length
     * @param doc Document for this textfield.
     * @param text String representing the default value
     * @param columns integer which represents the length allowed to type in
     * this textfield.
     */
    public OrganizationTextField(Document doc, String text, int columns){
        ...
        addMouseListener(new MouseAdapter(){
            public void mouseClicked(MouseEvent me){
                if (me.getClickCount() == 2) {
                    try{
                        detailWindow.showOrganizationDetails(detailId);
                    } catch ( Exception ex){
