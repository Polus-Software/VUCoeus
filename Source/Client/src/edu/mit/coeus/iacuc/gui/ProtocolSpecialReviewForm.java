   ...
   /** This method is used to perform Validations.
    ...
    public boolean validateData() throws Exception, CoeusUIException{
        ...
        for(int inInd=0; inInd < rowCount ;inInd++){
            ...
            String expDate = (String)tblSpecialReview.getValueAt(inInd, 6);
            ...
            if(specialDesc != null && specialDesc.trim().length() > 0 && 
                ...
                boolean validate = validateRowData(sCode, appCode, proNum, apDate, arDate, expDate);
                if(!validate){
                    valid = false;
                    break;
                }else{
...
//                int col = tblSpecialReview.getSelectedColumn();
        ...
        if(vecValidateRules != null){
            ...
            for(int index = 0; index < size ; index++){
                ...
                if(srBean != null){
                    ...
                    //COEUSQA-1724-Added For new expiration date column-start
                    expirationDateFlag = srBean.getExpirationDateFlag();
                    //COEUSQA-1724-Added For new expiration date column-end
    ...
    private boolean validateRowData(int code , int aCode, String protocolNum, 
                                            String applDate, String apprDate,String expDate) throws Exception{
            ...
            if(expDateFlag != null){
                if(expDateFlag.trim().equalsIgnoreCase("Y")) {
                    if(expDate == null || expDate.trim().length() <=0){
                        errorMessage(coeusMessageResources.parseMessageKey(
                            "protocol_SpecialReviewForm_exceptionCode.1307"));
                        //tblSpecialReview.requestFocus();
                        return false;
                    }
                }
   ...
   /** This method is used to show the alert messages to the user.
    ...
    private void errorMessage(String mesg) throws CoeusUIException{
        ...
        coeusUIException.setTabIndex(CoeusGuiConstants.IACUC_SPECIAL_REVIEW_EXCEP_TAB_INDEX);
        ...
        if((vecSpecialReviewData!= null) &&
            (vecSpecialReviewData.size() > 0)){
                ...
                inCtrdata++){
                    ...
                    //COEUSQA-1724-Added For new expiration date column-start
                    Date expirationDate = protocolSpecialReviewFormBean.getExpirationDate();
                    //COEUSQA-1724-Added For new expiration date column-end
...
//                    }
    ...
    private void setTableEditors(){
        ...
        //column.setMaxWidth(0);
        column.setPreferredWidth(0);
        column = tblSpecialReview.getColumnModel().getColumn(9);
        column.setMinWidth(0);
        //column.setMaxWidth(0);
        column.setPreferredWidth(0);
            ...
            if (totalRows > 0) {
                ...
                if (selectedRow != -1) {
                    ...
                    if (0 == selectedOption) {
                        ...
                        if(!TypeConstants.INSERT_RECORD.equals(protocolReviewBean.getAcType())) {
                            protocolReviewBean.setAcType( DELETE_RECORD );
                            vecDeletedSpecialreviewCodes.add( protocolReviewBean );                           
                        }                        
                        vecSpecialReviewData.remove( selectedRow );
    ...
    public class CustomTableModel extends DefaultTableModel{
        ...
        public CustomTableModel(){
            super(new Object[][]{}, new Object [] 
                {"Icon", "Special Review", "Approval", "Protocol No.", "Appl. Date", "Appr. Date", "Exprn. Date" ,"SpecialRevNum", "SeqNo", "protocolNo"});
        }
