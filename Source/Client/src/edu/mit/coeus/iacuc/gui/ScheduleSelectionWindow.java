...
public class ScheduleSelectionWindow extends CoeusDlgWindow
{
    ...
    {//GEN-HEADEREND:event_btnOkActionPerformed
        ...
        if (tblSchedules.getSelectedRowCount()>0) {
            //Added for COEUSQA-3335 : IACUC - Allow Removing Schedule in Submission Details - start
            //If determination notification already sent and still user want to select new schedule then giving an warning
            //If selects yes then it allows to add selected schedule
            ScheduleDetailsBean scduleBean =  getSelectedSchedule();
            String newScheduleId = scduleBean.getScheduleId();
            if(isMailSent() && !newScheduleId.equalsIgnoreCase(getOldScheduleId())){
