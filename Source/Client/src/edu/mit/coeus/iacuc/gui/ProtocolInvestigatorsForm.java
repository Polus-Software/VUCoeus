...
public class ProtocolInvestigatorsForm extends JComponent
    ...
    private boolean isStudyPersonnelExists = false;
    private static final String INVESTIGATOR_DATA = "investigatorData";
    private static final String PERSON_RESP_DATA = "personRespData";
    private static final String DELETE_PERSON_BEFORE_DEL_PERSON_RESPONSE = "person_trainingCode_excetionCode.1103";
    ...
    private void setFormData(){
         if(existingInvestigators != null){
            ...
            if( tblPerson.getRowCount() > 0 ) {
                ...
                String firstPerson =(tblPerson.getValueAt( 0,5 ) == null ? CoeusGuiConstants.EMPTY_STRING
                    : tblPerson.getValueAt( 0,5 ).toString());
                //Coeus enhancement 32.2 - step 3 : end
           if( ( firstPerson != null )&& (!firstPerson.equalsIgnoreCase(CoeusGuiConstants.EMPTY_STRING) )){
        ...
        saveRequired = false;
        //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -Start
        notifyObserver(existingInvestigators, INVESTIGATOR_DATA);
        //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -End
    }
...
//                    if( functionType == DISPLAY_MODE ){
    ...
    public String getDescriptions(String affTypeCode){
        ComboBoxBean bean;
        for( int indx = 0; indx < vecAffiliation.size(); indx++) {
            bean = (ComboBoxBean) vecAffiliation.get(indx);
            if( bean.getCode().equalsIgnoreCase( affTypeCode ) ) {
        ...
        while (enumKeys.hasMoreElements() ) {
            ...
            for( int indx = 0; indx < tblPerson.getRowCount(); indx++ ) {
                ...
                if( protocolInvBean.getPersonId().equalsIgnoreCase( perId ) ){
                    ...
                    protocolInvBean.setAffiliationTypeDescription(affDesc);
                    break;
                }
            }
        }
            ...
            for(int invCount = 0 ; invCount < existingInvestigators.size() ;
                ...
                if (investigatorData != null ) {
                    ...
                    if ( investigatorData.containsKey(
                        ...
                        //COEUSQA-2807 - IACUC- Affiliation for study personnel needs a separate code table 
                        ProtocolInvestigatorsBean updatedInvBean = (ProtocolInvestigatorsBean)investigatorData.get(protocolInvBean.getPersonId());
                        if(updatedInvBean.getAffiliationTypeCode() != protocolInvBean.getAffiliationTypeCode()) {
                            investigatorModified = true;
                            protocolInvBean.setAffiliationTypeCode(updatedInvBean.getAffiliationTypeCode());
                            protocolInvBean.setAffiliationTypeDescription(updatedInvBean.getAffiliationTypeDescription());
                        }
...
//                if(!found){
    ...
    public boolean validateData() throws Exception, CoeusUIException{
         ...
         if(tblPerson.isEditing()){
            if(tblPerson.getCellEditor() != null){
        ...
        if(tblPerson.getRowCount()>0){
            ...
            String selPersonId=CoeusGuiConstants.EMPTY_STRING;
            String selPersonName = CoeusGuiConstants.EMPTY_STRING;
    ...
    public boolean validateFormData() throws Exception, CoeusUIException{
        ...
        // Added for COEUSQA-2807 : IACUC_Affiliation_for_study_personnel_needs_separate_code_table - Start
         for(int inInd=0; inInd < rowCount ;inInd++){
            String affilaitionType = (String)((DefaultTableModel)tblPerson.getModel()).getValueAt(inInd,AFFILIATION_COLUMN_INDEX);
            if(affilaitionType == null || CoeusGuiConstants.EMPTY_STRING.equals(affilaitionType)){
                tblPerson.setRowSelectionInterval(inInd,inInd);
                log(coeusMessageResources.parseMessageKey(ERR_SELECT_AFFILIATION));
                break;
            }
        }
        // Added for COEUSQA-2807 : IACUC_Affiliation_for_study_personnel_needs_separate_code_table - Start
    ...
    private void log(String mesg) throws CoeusUIException{
       //Commented by sharath(20 - Aug - 2003)
        ...
        coeusUIException.setTabIndex(CoeusGuiConstants.IACUC_INVESTIGATOR_EXCEP_TAB_INDEX);
    ...
    private Vector getUnitTableData( String personId ){
        ...
        for( int indx = 0; indx <  unitDataBean.size(); indx++ ){
            try{
                ...
                rowData = new Vector();
                rowData.add( CoeusGuiConstants.EMPTY_STRING);
    ...
    private Vector getPersonTableColumnNames(){
        ...
        personHeaders.add( CoeusGuiConstants.EMPTY_STRING );
        ...
        for( int indx = 0; indx < dataBean.size() ; indx ++ ){
            try{
               ...
               if( protocolInvestigatorBean != null){
                    ...
                    protocolNumber=protocolInvestigatorBean.getProtocolNumber();
                    ...
                    personTableRow.add( CoeusGuiConstants.EMPTY_STRING);
                    personTableRow.add(
    ...
    public void actionPerformed( ActionEvent btnActionEvent ){
        ...
        if( actionSource.equals( btnFindPerson ) ) {
            ...
            try{ 
                ...
                investigator.setAffiliationTypeDescription( faculty ? getDescriptions("1") : getDescriptions("3"));
                //COEUSQA-3795
                               ...
                               //  row 
                                ...
                                if(oldUnitNumber == null 
                                   ...
                                   tblLead.setValueAt( adminPerson , 
                                    ...
                                    newData.add( CoeusGuiConstants.EMPTY_STRING);
...
//            newData.add("Affiliate");
           ...
           // newData.add(CoeusGuiConstants.EMPTY_STRING);
            ...
            //COEUSQA-2807 - IACUC- Affiliation for study personnel needs a separate code table 
            ((DefaultTableModel)tblPerson.getModel()).setValueAt(CoeusGuiConstants.EMPTY_STRING, rowCount, 3);
            saveRequired = true;
                ...
                if(name != null && name.trim().length()>0){
                    //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -Start
                     ...
                     String personId = (String)tblPerson.getValueAt(
                                                                selectedRow,5);
                     if(isPersonResponsePersentForPerson(personId) && !isStudyPersonnelExists){
                         isDeletePersonResponse = true;                       
                     }
                     if(isDeletePersonResponse){
                        message =  "Are you sure you want to remove "+name+" it's associated units and persons response?";
                     }else{
                         message = "Are you sure you want to remove "+name+" and associated units?";
                     }
                     //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -End
                    int selectedOption 
                        = showDeleteConfirmMessage(message);
...
//                                                                selectedRow,5);
                        ...
                        notifyObserver(vecObserverData, INVESTIGATOR_DATA);
                        if(isDeletePersonResponse){
    ...
    private void setInvestigatorBean(ProtocolInvestigatorsBean investigator,
        ...
        int selectedRow = tblPerson.getSelectedRow();
        String oldPersonId = CoeusGuiConstants.EMPTY_STRING;
        ...
        if(!personId.equals(oldPersonId)){
            ...
            if(! checkDuplicatePerson( personId ,selectedRow)){
                ...
                if( replaceRow ){
                   ...
                   tblPerson.setValueAt(CoeusGuiConstants.EMPTY_STRING,selectedRow,0);
                   ...
                   if(CoeusGuiConstants.EMPTY_STRING.equals(oldPersonId.trim())){
                         replaced = false;
                   }                        
                   Vector vecObserverData = new Vector();
                   if(vecFinalObserverData == null || vecFinalObserverData.isEmpty()){
                   ...
                   vecObserverData.addAll(vecFinalObserverData);
                   vecObserverData.add(investigator);
                   if(replaced){
                   vecObserverData.remove(tblPerson.getSelectedRow());
                   }
                   notifyObserver(vecObserverData, INVESTIGATOR_DATA);
                   //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -End
                   //Coeus enhancement 32.2 - step 30 : start
                        ...
                        vecObserverData.addAll(vecFinalObserverData);
                        ProtocolInvestigatorsBean investigatorsBean = new ProtocolInvestigatorsBean();
                        investigatorsBean.setPersonId((String) newData.get(5));
                        investigatorsBean.setPersonName((String) newData.get(1));
                        vecObserverData.add(investigatorsBean);
                        notifyObserver(vecObserverData,INVESTIGATOR_DATA);
                        //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -End
                        setTableEditors();
                        int  newRowCount = tblPerson.getRowCount();
    ...
    public class ImageRenderer extends DefaultTableCellRenderer {
        ...
        int column) {
            int selectedRow=tblPerson.getSelectedRow();
            setOpaque(false);
//            if(!value.equals(CoeusGuiConstants.EMPTY_STRING)) {
    ...
    private class NameEditor extends DefaultCellEditor implements TableCellEditor {
        ...
        private String oldValue = CoeusGuiConstants.EMPTY_STRING;
        ...
        private void updatePersonDetails(){
            ...
            if(selectedRow != -1){
                ...
                primaryInv = ((Boolean)tblPerson.getValueAt(selectedRow
                    ,2)).booleanValue();
                //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -Start
                if(!oldValue.equalsIgnoreCase( txtName.getText().trim()) && !CoeusGuiConstants.EMPTY_STRING.equals(oldValue)){
                    boolean isDeletePersonResponse = false;
                    String message = CoeusGuiConstants.EMPTY_STRING;
                if(isPersonResponsePersentForPerson(oldPersonId) && !isStudyPersonnelExists){
                     isDeletePersonResponse = true;                       
                   }
                if(isDeletePersonResponse){
                    message =  "Are you sure you want to remove "+oldValue+" it's associated units and persons response?";                  
                    int selectedOption 
                        = showDeleteConfirmMessage(message);
                    if( selectedOption == JOptionPane.NO_OPTION ){
                       tblPerson.setValueAt(oldValue,selectedRow,1);
                       tblPerson.setValueAt(oldPersonId,selectedRow,5);
                       tblPerson.getCellEditor().cancelCellEditing();
                       return; 
                     }else{
                        Vector vecDeletedPersonId = new Vector();
                        vecDeletedPersonId.add(oldPersonId);
                        notifyObserver(vecDeletedPersonId, PERSON_RESP_DATA);
                     }
                }  
                //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -End
...
//                        personInfo.getFacFlag().equalsIgnoreCase("y") ? "Faculty" : "Affiliate");
        ...
        if(responderBean != null){
            if(responderBean.isSuccessfulResponse()) {
                ...
                if(personInfo.getPersonID() == null){
                    ((JTextField)source).setText(CoeusGuiConstants.EMPTY_STRING); 
                    CoeusOptionPane.showErrorDialog(
    ...
    private class NumberEditor extends DefaultCellEditor implements TableCellEditor {
        ...
        private void getUnitDetails(){
            ...
            String unitNo = CoeusGuiConstants.EMPTY_STRING;
            if(txtName.getText() != null && txtName.getText().trim().length()>0){
                if(!oldValue.equalsIgnoreCase(txtName.getText().trim())){
                    UnitDetailFormBean unitDetail 
    ...
    public void valueChanged( ListSelectionEvent listSelectionEvent ) {
        ...
        if( (source.equals(perSelectionModel) )&& (selectedRow >= 0 )
            ...
            investigatorData.get( tblPerson.getValueAt( selectedRow, 
                5) == null ? CoeusGuiConstants.EMPTY_STRING : tblPerson.getValueAt( selectedRow,
                5).toString() );
...
//                    if(functionType != DISPLAY_MODE){                    
    ...
    /*private void updateFacultyFlag( int personRow ) {
        ...
        investigatorData.get( tblPerson.getValueAt( personRow, 
            4) == null ? CoeusGuiConstants.EMPTY_STRING : tblPerson.getValueAt( personRow,
            4).toString() );
        if( investigatorRow != null ) {
    ...
    private void resetLeadUnitFlag(String personId) {
        ...
        if (personId != null && !personId.equalsIgnoreCase(CoeusGuiConstants.EMPTY_STRING)){
            Vector units = (Vector)unitHashData.get( personId );
            if( units != null ){
    ...
    private String checkForNull( Object value ){
        ...
        /*return value.toString().equalsIgnoreCase( "null" ) ?  CoeusGuiConstants.EMPTY_STRING :
            value.toString();
         */
    }
    ...
    public java.lang.String getLeadUnitNumber() {
        return ( leadUnitNumber == null ? CoeusGuiConstants.EMPTY_STRING : leadUnitNumber ) ;
    }
        
    //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -Start
     /**
      * Method to Registers the Observer
     ...
     /**
      * Method to UnRegisters the Observer
      * @param Observer observer
      */
     ...
     */
     private void notifyObserver(Vector investigatorInfo, String observerType){
         HashMap hmInfo = new HashMap();
         if(INVESTIGATOR_DATA.equals(observerType)){
             vecFinalObserverData = new Vector();
             vecFinalObserverData = investigatorInfo;
             hmInfo.put("investigatorInfo",investigatorInfo);
             newObservable.notifyObservers(hmInfo);
         }else{
             String deletedPersonId = (String) investigatorInfo.get(0);
     ...
     */
     public void update(Observable o, Object arg) {                
        ...
        if(hmNofiyInfo.get("personInfo")!=null){      
     ...
     */
     private boolean isPersonResponsePersentForPerson(String personId){
        ...
        if(personresponsibleExists){
           if(vecStudyPersonData != null && !vecStudyPersonData.isEmpty()){
              ProtocolKeyPersonnelBean keyPersonnelBean;
              for(Object obj:vecStudyPersonData){
                  keyPersonnelBean = (ProtocolKeyPersonnelBean)obj;
                  if(personId.equals(keyPersonnelBean.getPersonId())){
                     isStudyPersonnelExists = true; 
                     break;
                  }
              }
           } 
