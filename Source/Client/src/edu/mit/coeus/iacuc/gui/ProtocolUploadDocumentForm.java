...
//    }
    ...
    public void setFormData() throws CoeusException {
        if(functionType != previousFunctionType){
            ...
            if(protocolId != null){                 
                if(isProtocolAmendRenewType(protocolId)){
    ...
    private void setParentProtocolData() throws CoeusException {
        ...
        if (protocolNumber != null) {             
            protocolNumber = checkProtocolAmendtype(protocolNumber); 
...
//            btnAmend.setEnabled(false);
            //Added for COEUSQA-3042 Allow users to add an attachment for a renewal - Start
            try {
                // To check whether the protocol number contains the character 'R' which means it is a renewal protocol.
                // If it is a renewal protocol then the delete, add and modify buttons should be enabled, else disabled.
                if(isRenewalContinuation(protocolInfoBean.getProtocolNumber())){
            ...
            //Added for COEUSQA-3042 Allow users to add an attachment for a renewal - End
        }else{
            btnDelete.setEnabled(true);
            btnAdd.setEnabled(true);
            btnModify.setEnabled(true);
...
//        }
    ...
    //Added for COEUSQA-3042 Allow users to add an attachment for a renewal - Start
    /**
     * method to return boolean value based on protocol number
     * @param protocolNumber string to pass the protocol number
     * @throws CoeusException If Any exception occurs
     * @return boolean to whether the protocolnumber contains 'R' or 'C'
     */
    public boolean isRenewalContinuation(String protocolNumber) throws CoeusException{
        boolean isRenewal = false;
        if(protocolNumber!= null && ((protocolNumber.indexOf('R')) > 0)){
            isRenewal = true;
        }else if(protocolNumber!= null && ((protocolNumber.indexOf('C')) > 0)){
            isRenewal = true;
        }
        return isRenewal;
    }
...
//        }
        ...
        //Save the protocol, before adding the attachment
                if(!canAddAttachment && protocolDetailForm != null && checkOriginalfunctionType()){
...
//            btnDelete.setEnabled(false);
//            btnAmend.setEnabled(false);
            //Modified for COEUSQA-3042 Allow users to add an attachment for a renewal - Start
            try {
                if(isRenewalContinuation(protocolInfoBean.getProtocolNumber())){
                    btnModify.setEnabled(true);
                    btnDelete.setEnabled(true);
            ...
            //Modified for COEUSQA-3042 Allow users to add an attachment for a renewal - End
        }
    }
    //Added for case 3552 - IRB attachments - start
    /**
    ...
    //COEUSQA-1724-Added for IACUC new Amendment Type - Start
    /*This method is used to check the original function type
     *@return boolean value
     */
    private boolean checkOriginalfunctionType(){
        boolean isOriginalFunctionType = false;
        if(protocolDetailForm.getOriginalFunctionType() == CoeusConstants.IACUC_AMENDMENT
       || protocolDetailForm.getOriginalFunctionType() == CoeusConstants.IACUC_RENEWAL
       || protocolDetailForm.getOriginalFunctionType() == CoeusConstants.IACUC_RENEWAL_WITH_AMENDMENT
       || protocolDetailForm.getOriginalFunctionType() == CoeusConstants.IACUC_CONTINUATION_CONTINUING_REVIEW
       || protocolDetailForm.getOriginalFunctionType() == CoeusConstants.IACUC_CONTI_CONTINUING_REVIEW_WITH_AMEND
       || protocolDetailForm.getOriginalFunctionType() == 'N'
       || protocolDetailForm.isSaveRequired()){
            isOriginalFunctionType = true;
        }
        return isOriginalFunctionType;
    }
    
    /*This method is used to check the  Protocol type
     *@return boolean value
     */
    private boolean isProtocolAmendRenewType(String protocolId){
        boolean isProtocolAmendRenew = false;
        if(( protocolId.indexOf(CoeusConstants.IACUC_AMENDMENT) != -1 ) ||
        ( protocolId.indexOf(CoeusConstants.IACUC_RENEWAL) != -1 ) ||
        ( protocolId.indexOf(CoeusConstants.IACUC_RENEWAL_WITH_AMENDMENT) != -1 ) ||
        ( protocolId.indexOf(CoeusConstants.IACUC_CONTINUATION_CONTINUING_REVIEW) != -1 ) ||
        ( protocolId.indexOf(CoeusConstants.IACUC_CONTI_CONTINUING_REVIEW_WITH_AMEND) != -1 )){
            isProtocolAmendRenew = true;
        }
        return isProtocolAmendRenew;
    }
    
    /*This method is used to check the Protocol Amendment type
     *@param String protocolNumber
     *@return String protocolNumber
     */
     private String checkProtocolAmendtype(String protocolNumber) {
        if (protocolNumber.indexOf(CoeusConstants.IACUC_AMENDMENT) != -1) {
            protocolNumber = protocolNumber.substring(0,
                    protocolNumber.indexOf(CoeusConstants.IACUC_AMENDMENT));
        } else if(protocolNumber.indexOf(CoeusConstants.IACUC_RENEWAL) != -1) {
            protocolNumber = protocolNumber.substring(0,
                    protocolNumber.indexOf(CoeusConstants.IACUC_RENEWAL));
        } else if(protocolNumber.indexOf(CoeusConstants.IACUC_RENEWAL_WITH_AMENDMENT) != -1) {
            protocolNumber = protocolNumber.substring(0,
                    protocolNumber.indexOf(CoeusConstants.IACUC_RENEWAL_WITH_AMENDMENT));
        } else if(protocolNumber.indexOf(CoeusConstants.IACUC_CONTINUATION_CONTINUING_REVIEW) != -1) {
            protocolNumber = protocolNumber.substring(0,
                    protocolNumber.indexOf(CoeusConstants.IACUC_CONTINUATION_CONTINUING_REVIEW));
        } else if(protocolNumber.indexOf(CoeusConstants.IACUC_CONTI_CONTINUING_REVIEW_WITH_AMEND) != -1) {
            protocolNumber = protocolNumber.substring(0,
                    protocolNumber.indexOf(CoeusConstants.IACUC_CONTI_CONTINUING_REVIEW_WITH_AMEND));
        }
        return protocolNumber;
    }
    //COEUSQA-1724-Added for IACUC new Amendment Type - end
     
    //Added for case 3552 - IRB attachments - end
    //  Added for the case#COEUSQA-2353Need to a better way to handle large numbers of protocol attachments-start
     /*
      *  Class for Protocol List HeaderColumn for sorting.
      */
    public class ColumnHeaderListener extends MouseAdapter {
     ...
     /*
      *  Class for Parent Protocol List HeaderColumn for sorting.
      */
