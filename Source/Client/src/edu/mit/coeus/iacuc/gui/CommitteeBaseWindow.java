...
implements ActionListener, Observer {
    ...
    private void modifyCommittee() throws Exception{
        ...
        if(selectedRow != -1){
            ...
            createCommittee('M',keyValue,mdiForm,selectedRow);
        }
    }
    ...
    private void displayCommittee() throws Exception {
        ...
        if(selectedRow != -1){
            String keyValue = tblCommittee.getValueAt(selectedRow,0).toString();
            createCommittee('D',keyValue,mdiForm,selectedRow);
        }
    }
    ...
    /*
     *  Method used to create the committe detail.
     *
     * @param functionType
     * @param committeeId
     */
    private void createCommittee(char functionType,String committeeId,
    CoeusAppletMDIForm mdiForm, int selectedRow ) throws Exception{
        /**
        ...
        try{
            committeeDetails = new CommitteeDetailsForm(functionType,
            committeeId, mdiForm);
        ...
        if( obj instanceof CommitteeMaintenanceFormBean ) {
            try {
                ...
                if (((BaseWindowObservable)observable).getFunctionType() == 'A'){
                    ...
                    ((DefaultTableModel)tblCommittee.getModel()).setValueAt(
                    committeeBean.getUnitNumber(),baseTableRow,2);
                    ((DefaultTableModel)tblCommittee.getModel()).setValueAt(
                    committeeBean.getUnitName(),baseTableRow,3);
                    ((DefaultTableModel)tblCommittee.getModel()).setValueAt(
                    committeeBean.getDescription(),baseTableRow,4);
