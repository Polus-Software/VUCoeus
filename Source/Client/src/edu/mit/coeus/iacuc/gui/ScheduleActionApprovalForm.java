...
public class ScheduleActionApprovalForm extends CoeusDlgWindow {
	...
	private boolean releaseLock;
       ...
       /*COEUSQA-1724 New Constants created for IACUC Lite Amendments and Renewals Implementation - start*/
        private static final char RENEWAL_WITH_AMENDMENT = 'E';
        private static final char CONTINUATION_CONTINUING_RENEWAL = 'C';
        private static final char CONTINUATION_CONTINUING_RENEWAL_AMEND = 'O';
        /*COEUSQA-1724 New Constants created for IACUC Lite Amendments and Renewals Implementation - end*/
        //COEUSDEV-330 : End
        //Added For Code Refactiring - Start
        private static final String DATE_FORMAT = "/-:,.";
        private static final String PLEASE_ENTER_VALID_DATE = "Please enter a valid date in mm/dd/yyyy format.";
        private static final String VALID_DATE_FORMAT = "dd-MMM-yyyy";
        //Added For Code Refactiring - End
        private static final int FYI = 104;
        private static final int YEARS_TO_BE_ROLLED = 3;       
			...
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				try {
					if (validateData()) {
						...
						dtUtils.restoreDate(txtApprovalDate.getText(),DATE_FORMAT)).getTime()) ;
						
						expirationDate = new java.sql.Date(dtFormat.parse(
						dtUtils.restoreDate(txtExpirationDate.getText(),DATE_FORMAT)).getTime()) ;
						
						actionDate = new java.sql.Date(dtFormat.parse(
						dtUtils.restoreDate(txtActionDate.getText(),DATE_FORMAT)).getTime()) ;
                                                //continueAction = true ;
						//dispose() ;
						...
						CoeusOptionPane.showErrorDialog(PLEASE_ENTER_VALID_DATE) ;
					}
				}
				catch(Exception ex) {
					CoeusOptionPane.showErrorDialog(PLEASE_ENTER_VALID_DATE) ;
...
//		VALID_DATE_FORMAT);
		...
		//Roll YEARS_TO_BE_ROLLED years from the current year
		cal.roll(Calendar.YEAR, YEARS_TO_BE_ROLLED) ;
		...
		String expiryDate = dtUtils.formatDate( mon + "/" + day + "/" + year, "/", VALID_DATE_FORMAT);
	...
	public void showForm() {
		...
		// submissiontype = 102
                /*COEUSQA-1724 New Condition added for IACUC Lite Amendments and Renewals Implementation*/
		if (this.actionBean != null) {
			if ((this.actionBean.getActionTypeCode() == IacucProtocolActionsConstants.RESPONSE_APPROVAL ||
                                this.actionBean.getActionTypeCode() == IacucProtocolActionsConstants.DESIGNATED_REVIEW_APPROVAL ||
                                this.actionBean.getActionTypeCode() == IacucProtocolActionsConstants.APPROVED ||
                                // Added for COEUSQA-2666: Complete Administrative Review functionality in IACUC
                                this.actionBean.getActionTypeCode() == IacucProtocolActionsConstants.ADMINISTRATIVE_APPROVAL)
                                // COEUSQA-2666: End
                                && isProtocolAmendrenew()){
				...
				txtExpirationDate.setEnabled(false) ;
                                ...
                                if(getProtocolSubmissionTypeCode() == FYI){
                                    lblApprovalDate.setVisible(false) ;
                                    txtApprovalDate.setVisible(false) ;
                                    lblApprovalDate.setEnabled(false) ;
                                    txtApprovalDate.setEnabled(false) ;
                                ...
                                try{
                                    ...
                                    CoeusOptionPane.showInfoDialog( PException.getMessage());
				}
...
//			}
	...
	private String getOriginalApprovalDate() {
		...
		if (actionBean != null) {
			RequesterBean requesterBean = new RequesterBean();
                        ...
                        moduleCode.add(ModuleConstants.IACUC_MODULE_CODE);
	...
	public void showExpiryDate() throws java.text.ParseException {
		if (this.actionBean != null) {
			if (this.actionBean.getActionTypeCode() == IacucProtocolActionsConstants.IACUC_REVIEW_NOT_REQUIRED) {
				lblExpirationDate.setVisible(false) ;
				txtExpirationDate.setVisible(false) ;
				lblExpirationDate.setEnabled(false) ;
				txtExpirationDate.setEnabled(false) ;
				...
				txtExpirationDate.setText(dtUtils.formatDate( expDate, "/", VALID_DATE_FORMAT));
			} 
		}
	...
	private int getProtocolSubmissionTypeCode() {
		...
		if (this.actionBean != null) {
			...
			// 'r' is GET_IACUC_REVIEW_COMMENTS just to reuse the existing code this is being used.
	...
	/* Case 818  end - prahalad - May 10 2004 */
        ...
        /*COEUSQA-1724 Method created for IACUC Lite Amendments and Renewals Implementation - start*/
        /**
	 * This method is used to check the protocol is amendment protocol or not.
	 * @return boolean value.
	 */
        private boolean isProtocolAmendrenew(){
            boolean isProtocolAmendRenew = false;
            if(this.actionBean.getProtocolNumber().indexOf(AMENDMENT) != -1 
                // Commented RENEWAL and RENEWAL_WITH_AMENDMENT since the expiration date 
                // and approval date are required for Renewal and Renewal with Amendment
//              || this.actionBean.getProtocolNumber().indexOf(RENEWAL) != -1
//              || this.actionBean.getProtocolNumber().indexOf(RENEWAL_WITH_AMENDMENT) != -1 
              || this.actionBean.getProtocolNumber().indexOf(CONTINUATION_CONTINUING_RENEWAL) != -1
              || this.actionBean.getProtocolNumber().indexOf(CONTINUATION_CONTINUING_RENEWAL_AMEND) != -1){
                isProtocolAmendRenew = true;
            }
            return isProtocolAmendRenew;
        }
        /*COEUSQA-1724 Method created for IACUC Lite Amendments and Renewals Implementation - End*/
