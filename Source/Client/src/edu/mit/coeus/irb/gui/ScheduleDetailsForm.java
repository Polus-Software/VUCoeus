...
public class ScheduleDetailsForm extends CoeusInternalFrame
        implements ActionListener,ProtocolActionsInterface {
...
//    private CoeusDlgWindow dlgAgendaView = null;
    ...
    private CommitteeScheduleDetailsForm commSchDtlsForm;
    ...
    // prps start
    //private CoeusMenuItem scheduleSubmit, scheduleDefer, scheduleAgenda, scheduleNextCommittee, scheduleTabled, scheduleApprove, scheduleCommitteeAction;
...
//    private ProtocolInfoBean protocolInfo;
    ...
    private int committeeTypeCode = 0;
    ...
    public ScheduleDetailsForm(char functionType,String scheduleId,
        ...
        super("IRB Schedule Details" + ( (scheduleId != null
...
//                    }
    ...
    /**
     * This method is used to set the reference to the <CODE>CommitteeScheduleDetailsForm</CODE>.
     *
     * @param commSchForm reference to <CODE>CommitteeScheduleDetailsForm</CODE>.
     */
    protected void setCommitteeScheduleDetailsForm(
            CommitteeScheduleDetailsForm commSchForm) {
 ...
 */
        committeeTypeCode = scheduleDetails.getCommitteeTypeCode();
        commSchedAttedentsForm = new CommitteeScheduleAttendanceForm( mdiForm,
                functionType,
                scheduleDetails.getCommitteeId(),
                scheduleId,
    ...
    public ScheduleDetailsBean getScheduleDetails(boolean isModifyMode) throws Exception {
        ...
        // Added forCOEUSQA-2686_IACUC-CHANGES TO SCHEDULE MAINTENANCE FOR 4.4.3 RELEASE_end
                ...
                while (it.hasNext()) {
                    ...
                    if (hashScheduleActions.get(actionCode).equals(actionType.getActionCommand())) { // first match the actioncommand to actioncode and if itz zero then display voting details
                        if (Integer.parseInt(actionCode) == 0) {
                             //coeusqa-4009 Committee Actions not allowed for SubmissionType Request for Suspension
                             ProtocolSubmissionInfoBean protocolSubmissionInfoBean = protocolAssignmentForm.getSelectedSubmissionDetails();
                             if(protocolSubmissionInfoBean.getSubmissionTypeCode()!=110){
                             executeAction(0, true) ;  // actioncode will change depending on the user selection, on voting form
                             }else{
                                CoeusOptionPane.showInfoDialog("Committee Actions cannot be performed if Submission type is 'Request for Suspension'"); 
                             }
                              //coeusqa-4009 Committee Actions not allowed for SubmissionType Request for Suspension
                        }else {
                            // added by manoj to perform protocol Actions 05/09/2003 starts
                            int actCode = Integer.parseInt(actionCode);
                            if(actCode==ProtocolActionsInterface.ACTION_PROTOCOL_WITHDRAWN
                                    || actCode == ProtocolActionsInterface.ACTION_GRANT_EXEMPTION
                                    || actCode == ProtocolActionsInterface.ACTION_IRB_REVIEW_NOT_REQUIRED//Coeus enhancement Case #1880 - step 3
                                    || actCode == ProtocolActionsInterface.ACTION_EXPEDITED_APPROVAL
                                    || actCode == ProtocolActionsInterface.ACTION_RESPONSE_APPROVAL//Added by nadh for response action enhencement
                                    || actCode == ProtocolActionsInterface.ACTION_IRB_ACKNOWLEDGEMENT//Coeus enhancement Case #1791 - step 3
                                    ){
        ...
        if (validateBeforePerformingAction(actionCode)) {
            ...
            if (showVotingForm) {   // here show the voting details screen (this is the extra step)
                if(authorizedToPerform(actionCode)){
                    ...
                    if( serverSideValidation(ProtocolActionsInterface.ACTION_COMMITTEE_ACTIONS, selRow) ){
                        showVotingDetails();
...
//            releaseUpdateLock();
    ...
    private CoeusMenu scheduleDetailsActionMenu(boolean enable) {
        ...
        //prps on on june 30th 2003 added this new menu item "Committee Actions"
        //and removed Tabled and Approve items. This items will be called from
        //voting details screen
        
        // vecScheduleDetailAction,vecVotingFormAction and hashScheduleActions  will be populated
        // from xml file or from the database. The reason for using Vector is to display menu in a
        // particular order.
        ...
        if(committeeTypeCode == IRB_COMMITTEE){
            vecScheduleDetailAction.add(0, "200") ;
            vecScheduleDetailAction.add(1, "201") ;
            vecScheduleDetailAction.add(2, "303") ;
            vecScheduleDetailAction.add(3, "0") ;
            vecScheduleDetailAction.add(4, "205") ;
            // added by manoj for Enhancement 01/09/2003
            vecScheduleDetailAction.add(5, "208") ;//Added by nadh for response action enhencement
            //Coeus enhancement Case #1791 - step 1: start
            vecScheduleDetailAction.add(6, "209") ;
            //Coeus enhancement Case #1791 - step 1: end
            
            vecScheduleDetailAction.add(7, "206") ;
            //Coeus enhancement Case #1880 - step 1: start
            vecScheduleDetailAction.add(8, "210");
        }else if(committeeTypeCode == IACUC_COMMITTEE){
            vecScheduleDetailAction.add(0, "200") ;
            vecScheduleDetailAction.add(1, "303") ;
            vecScheduleDetailAction.add(2, "0") ;
            vecScheduleDetailAction.add(3, "208") ;
        }
        ...
        //Code for new Enhancement by Vyjayanthi on 27/08/03
        //vecScheduleDetailAction.add(3, "207") ;
        //vecScheduleDetailAction.add(4, "208") ;
        //vecScheduleDetailAction.add(5, "209") ;
        //End
        
        vecVotingFormAction.add(0, "204") ;
        vecVotingFormAction.add(1, "202") ;
        vecVotingFormAction.add(2, "203") ;
        //adding new Motion Item  - 304 Disapprove in Commitee Actions - Start
        vecVotingFormAction.add(3, "304") ;
        //adding new Motion Item  - 304 Disapprove in Commitee Actions - End
        
        hashScheduleActions.put("200", "Assign To Agenda") ;
      
        hashScheduleActions.put("0", "Committee Actions") ;
        hashScheduleActions.put(""+ProtocolActionsInterface.ACTION_PROTOCOL_WITHDRAWN, "Withdraw Submission") ;
        // added by manoj for new Enhancement 01/09/2003
        //IACUC Changes - Start
        if(committeeTypeCode == IRB_COMMITTEE){
            hashScheduleActions.put("201", "Defer") ;
            
            hashScheduleActions.put(""+ProtocolActionsInterface.ACTION_EXPEDITED_APPROVAL, "Expedited Approval") ;
            
            hashScheduleActions.put(""+ProtocolActionsInterface.ACTION_GRANT_EXEMPTION, "Grant Exemption") ;
            //Coeus enhancement Case #1880 - step 2: start
            hashScheduleActions.put(""+ProtocolActionsInterface.ACTION_IRB_REVIEW_NOT_REQUIRED, "IRB Review Not Required") ;
            //Coeus enhancement Case #1880 - step 2: end
        }
        //IACUC Changes - End
        // emd added by manoj
        //Added by nadh for response action enhencement - start
        hashScheduleActions.put(""+ProtocolActionsInterface.ACTION_RESPONSE_APPROVAL, "Response Approval") ;
        //Added by nadh for response action enhencement - end
        //Coeus enhancement Case #1791 - step 2: start
        //IACUC Changes - Start
        if(committeeTypeCode == IRB_COMMITTEE){
            hashScheduleActions.put(""+ProtocolActionsInterface.ACTION_IRB_ACKNOWLEDGEMENT, "IRB Acknowledgment") ;
        }
        //IACUC Changes - End
        //Coeus enhancement Case #1791 - step 2: start
        hashScheduleActions.put("202", "Substantive Revisions Required") ; // Table = Substantive Revisions Required
        hashScheduleActions.put("204", "Approve") ;
        hashScheduleActions.put("203", "Specific Minor Revisions Required") ;
        //adding new Motion Item  - 304 Disapprove in Commitee Actions - Start
        hashScheduleActions.put("304", "Disapprove");
        //adding new Motion Item  - 304 Disapprove in Commitee Actions - End
        
        //IACUC Changes - Start
        if(committeeTypeCode == IACUC_COMMITTEE){
        ...
        //Code for new Enhancement by Vyjayanthi on 27/08/03
        //hashScheduleActions.put("207", "Revision Requested") ;
        //hashScheduleActions.put("208", "Expedited Approval") ;
        //hashScheduleActions.put("209", "Grant Exemption") ;
        //End
        ...
        for (int i=0 ; i< vecScheduleDetailAction.size() ; i++) {
            ...
            //depending on the right PERFORM_IRB_ACTIONS_ON_PROTO
        ...
        if(committeeTypeCode == IRB_COMMITTEE){
            fileChildren.insertElementAt(SEPERATOR,3);
        }else if(committeeTypeCode == IACUC_COMMITTEE){
                fileChildren.insertElementAt(SEPERATOR,2);
        }
        //IACUC Changes - Start
        if(committeeTypeCode == IACUC_COMMITTEE){
...
//        return subNum ;
    ...
    private void handleActionMenuItem(int actionCode, int rowIndex) {
        ...
        if(actionCode == 101) // "Submit_To_IRB"
        {
            if (!serverSideValidation(actionCode, rowIndex)) {
                return ;
            }
            stepsMandatoryForSubmitToIRB(actionCode, rowIndex, false);
...
//        boolean continueAction ;
        ...
        strTitle = coeusMessageResources.parseMessageKey("actionInputFrm_TitleCode." + actionCode + "1")
        + "-" + tblProtocol.getValueAt(rowIndex,0) ;
        msgPrompt = coeusMessageResources.parseMessageKey("actionInputFrm_PromptCode." + actionCode + "2") ;
        strDefault = coeusMessageResources.parseMessageKey("actionInputFrm_DefaultCode." + actionCode + "3") ;
        ...
        if (actionCode == 204) {
            if (followupAction) { // if itz a follow up action then make sure that the prompt flag is set, else u dont flash the input form
                if (promptUser) {
                    ...
                    actionDate = appForm.getActionDate();//Added by Jobin for Action Date
        ...
        if (actionCode == 202 || actionCode == 201 || actionCode == 200 || actionCode == 203) // Substantive Revisions Required(Tabled), Defer, Assign to Agenda, Specific Minor Revisions Required
        {
            if (followupAction) { // if itz a follow up action then make sure that the prompt flag is set, else u dont flash the input form
                if (promptUser) {
                    msgPrompt = promptMessage ;
                    ...
                    actionDate = inputForm.getActionDate();//Added by Jobin for Action Date field in ScheduleInput Form
        ...
        if (actionCode == 6 ) {   // Submit to next schedule will be same as submit to IRB but the only difference
            ...
            stepsMandatoryForSubmitToIRB(actionCode, rowIndex, true) ;
...
//        boolean success=false;
    ...
    private void stepsMandatoryForSubmitToIRB(int actionCode, int rowIndex, boolean skipCommiteeScreen) {
        try {
...
//                }
//                else{
//                    boolean isMinutePresent = ((Boolean)response.getDataObject()).booleanValue();
//                    if(!isMinutePresent){
                ...
                if( submissionForm.isDataSaved()) {
                    ...
                    //prps commented this line - dec 16 2003
                    // submit to irb might have follow up actions so this screen will not
                    // be closed
                    //mdiForm.removeFrame(
                    ...
                    functionType = MODIFY_MODE ;
                    releaseUpdateLock() ;
                    //this.doDefaultCloseAction(); //prps commented this line - dec 16 2003
                    submissionForm.dispose() ;
                    //prps end new code
                }
//                    }
 ...
 */
    ...
    /**
     * Method to get a confirmation to add PDF as book
     * @return
     */
