		...
		<tr class="rowLine">
			...
			<td align="left" valign="top">
				<table width="98%" border="0" align="center" cellpadding="2"
					...
					<jsp:useBean id="awardInfoHeader" scope="session"
						class="java.util.Vector" />
					<logic:iterate name="awardInfoHeader" id="awardMenuBean"
				...
				<table width="100%" border="0" align="center" cellpadding="2"
					...
					<tr>
						...
						<td class="coeusMenu"><font color="#6D0202"><%=linkName%>
						</font></td>
						<td align=right nowrap class="selectedMenuIndicator"><logic:equal
								name="awardMenuBean" property="selected" value="true">
								<bean:message key="menu.selected" />
							</logic:equal></td>
						<%} else if(menuId.trim().equals("A007")){%>
						<td class="coeusMenu"><a
							href="<%=request.getContextPath()%>/awardPrintnew.do?awdType=2"
							target="_blank"><%=linkName%></a></td>
						<%}else if(menuId.trim().equals("A008")){%>
						<td class="coeusMenu"><a
							href="<%=request.getContextPath()%>/awardPrintnew.do?awdType=1"
							target="_blank"><%=linkName%></a></td>
						<%}else if(menuId.trim().equals("A009")){%>
						<td class="coeusMenu"><a
							href="<%=request.getContextPath()%>/awardPrintnew.do?awdType=3"
							target="_blank"><%=linkName%></a></td>
						<%}%>
