<%-- 
    Document   : AwardListPrint
    Created on : Dec 29, 2010, 8:53:47 PM
    Author     : shibuk
--%>
...
<%--<% response.setHeader("Content-Disposition", "attachment; filename=\"Shb-table.xls\""); %>--%>
<%--<% response.setHeader("Content-Disposition", "attachment; filename=myExcel.xls"); %>--%>
<% response.setHeader("Content-Disposition", "inline; filename = myAward.xls"); %>

<%--<%@page contentType="application/excel" pageEncoding="UTF-8"%>--%>
<%--<%@page contentType=".xls application/vnd.ms-excel" pageEncoding="UTF-8"%>--%>
<%--<%@page contentType=".xls application/x-excel" pageEncoding="UTF-8"%>--%>
<%--<%@page contentType=".xls application/x-msexcel" pageEncoding="UTF-8"%>--%>
...
<jsp:useBean id="childHierarchy" scope="session"
...
<%
   parentList = (Vector)session.getAttribute("awardList");
   childList = (Vector)session.getAttribute("childHierarchy");
   columNames = (Vector)session.getAttribute("awardColumnNames");
   widthLst = (HashMap) session.getAttribute("widthList");
   IteratorChildList = (Vector)session.getAttribute("childHierarchy");
                                         ...
                                         for(int index=0;index<awardColumnNames.size();index++){
                                            ...
                                            if(key != null){
                                                ...
                                                if(index == 0){awardNumber=value;}
                                                    ...
                                                    if(value != null && value.length() > clWidthIntxy){
														...
														<td style="white-space: normal;" align="left"
															width="<%=clmwidth%>%">
															<%--<a href="<%=request.getContextPath()%>/getAwardInfo.do?awardNumber=<%=awardNumber%>"><u>--%>
															<%=value%> <%-- </u></a>--%>
														</td>
														<%
                                            }
                                      ...
                                      if(selectedChildList != null && selectedChildList.size() > 0) {
                                        ...
                                        while(itr.hasNext()) {
                                          ...
                                          String strBgColor = "#DCE5F1";
                                            ...
                                            out.println("<tr id="+rowCount+" style=\"height:auto; \" bgcolor=\""+strBgColor+"\"  onmouseover=\"className='TableItemOn'\" onmouseout=\"className='TableItemOff'\" class=\"TableItemOff\">");
                                            //out.println("<td>");
                                           // if(chldCount > 0) {
                                                ...
                                                clWidthInt=clWidthInt*2;
                                                       if(key != null){
                                                        ...
                                                        if(index == 0){awrdNo=value1;}
                                                            ...
                                                            if(value1 != null && value1.length() > clWidthInt){
