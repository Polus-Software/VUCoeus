...
%>
		...
		<tr>
			<td colspan="5"><b>For your project listed below, think
					about your relationship with all your Financial Entities and update
					the Financial Interest status as appropriate:</b></td>
		</tr>
		<tr>
			<td align="right" width="15%" style="float: none" nowrap><b><%=moduleNm%>
					# :</b></td>

			<td style="float: none" align="left" width="30%" colspan="4"><bean:write
					name="coiProjectDetail" property="coiProjectId" /></td>
		...
		<tr>
			...
			<td style="float: none" align="left" colspan="3"><bean:write
					name="coiProjectDetail" property="coiProjectTitle" /></td>
			<td align="right" style="padding-right: 6px;"><html:button
					styleClass="clsavebutton" value="Skip Project" property="skip"
					onclick="javaScript:skipProject();" /></td>
			<%}else{ %>
			<td align="right" style="float: none" nowrap><b>Title :</b></td>
			<td style="float: none" align="left" colspan="4"><bean:write
					name="coiProjectDetail" property="coiProjectTitle" /></td>
			<%}%>
		</tr>

		<tr>
			<td align="right" width="15%" style="float: none" nowrap><logic:equal
					value="Protocol" property="coiProjectType" name="coiProjectDetail">
					<b>Application Date:</b>
				</logic:equal> <logic:notEqual value="Protocol" property="coiProjectType"
					name="coiProjectDetail">
					<b>Start Date:</b>
				</logic:notEqual></td>
			<td style="float: none" align="left" width="15%"><bean:write
					name="coiProjectDetail" property="coiProjectStartDate"
					format="dd-MMM-yyyy" /></td>
			<td align="right" width="23%" style="float: none" nowrap><logic:equal
					value="Protocol" property="coiProjectType" name="coiProjectDetail">
					<b>Expiration Date:</b>
				</logic:equal> <logic:notEqual value="Protocol" property="coiProjectType"
					name="coiProjectDetail">
			...
			<td style="float: none" align="left" width="15%"><bean:write
					name="coiProjectDetail" property="coiProjectEndDate"
					format="dd-MMM-yyyy" /></td>
			<%    
     if((project_Type.equalsIgnoreCase("Annual"))||(project_Type.equalsIgnoreCase("Revision"))){  %>
			<td nowrap align="right" width="35%"><b>Project <%=projectNum%>
					of <%=totalNumOfPjt%> projects
			</b></td>
			<%} else{%>
		...
		<tr>
			<td class="copybold" colspan="5"><img width="100%" height="2"
				border="0"
		...
		<tr>
			...
			<td colspan="4" width="40%" nowrap><b> Set Financial
					Interest Status for all Financial Entities to:</b> <html:select
					property="coiStatusCode" onchange="changeStatusCode();"
					onclick="changeStatusCode();" style="width:190px"
					value="--------Please Select---------">
    ...
    String projectType=(String) request.getAttribute("projectType");     
     if((projectType.equalsIgnoreCase("Annual"))||(projectType.equalsIgnoreCase("Revision"))){  %>
			...
			<td width="35%" nowrap>&nbsp;&nbsp;<html:button
					styleClass="clsavebutton" value="Save & Continue" property="save"
					onclick="javaScript:saveFinEntityConflict();" /> <html:button
					property="saveAndProceed" styleClass="clsavebutton" value="Quit"
...
%>
			...
			<tr class="copybold" bgcolor="#9DBFE9" class="rowLine">
				<td colspan="5"><br />
					<div style="font-size: 13; float: left;">
						<b>Financial Entity:&emsp;&emsp;<bean:write
					...
					<div style="font-size: 13; float: right;">
						...
						<select id="<%=nameType%>" name="<%=nameType%>"
							...
							<option <%=selected%> value="<%=bean.getCode()%>"
								onclick="javascript:checkConflict('<%=beancodeConflict%>','<%= inx%>');">
								<%=bean.getDescription() %>
        ...
        if(commentObj==null){comments="";}
			...
			<tr>
				<td width="10%"></td>
				<td colspan="4"><b><bean:message bundle="coi"
							key="financialEntity.Qstn1" />.</b></td>
			</tr>
			<tr>
				<td colspan="5"><div style="margin-left: 115px;">
						<font color="black"><i>" <%=comments%> "
						</i></font>
					</div></td>
				<textarea style="display: none" id="relDesc<%=nameType%>"
					name="relDesc<%=nameType%>" cols="77" rows="3"><%=comment%></textarea>
			...
			<tr>
				<td width="13%"></td>
				<td colspan="4"><font color="red" face="bold">* </font><b>Describe
						the relationship between this project and this entity, include any
						involvement of students and staff. </b></td>
			</tr>
			<tr>
				<td width="15%"></td>
				<td colspan="4"><b>If there is no relationship, explain
						why.</b></td>
			</tr>

			<tr>
				<td colspan="5" align="right"><textarea
						id="orgRelDesc<%=nameType%>" name="orgRelDesc<%=nameType%>"
						cols="77" rows="8" style="width: 85%;"><%=orgRelationshipDescription.trim()%></textarea></td>
			</tr>
			<tr>
				<td class="copybold" colspan="5"><img width="100%" height="2"
					border="0"
		...
		<tr>
			<td colspan="5"><html:button styleClass="clsavebutton"
					value="Save & Continue" property="save"
					onclick="javaScript:saveFinEntityConflict();" /> &nbsp;<html:button
					property="saveAndProceed" styleClass="clsavebutton" value="Quit"
					onclick="javascript:exitToCoi()" /></td>
		</tr>
