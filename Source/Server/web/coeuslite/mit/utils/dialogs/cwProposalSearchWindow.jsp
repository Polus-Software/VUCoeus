...
<head>
<title><bean:message key="searchWindow.title.proposal" />/title>
	<style type="text/css">
//
div {
	font-family: verdana;
...
function populateResultData(){
	...
	<link
		...
		rel="stylesheet" type="text/css"> <%--link href="../css/ipf.css" rel="stylesheet" type="text/css"--%>
	<style type="text/css">
</style>
</head>

<body onload="populateResultData()"
	STYLE="background-image: url('../../web images/coeusirb/body_background.gif'); border: 0">

	<% String srch = "/proposalSearch.do?type=+strType&proposalSearch=true"; %>
	<%if(resData == null) {%>
...
%>
		<table>
			<table width='100%' class="tabtable"
				STYLE="background-image: url('../../images/coeusLite/nav_background.gif'); border: 0">
				<tr valign="center">
...
%>
				...
				<tr class='copy'
					bgColor="<%= ((++index)%2==0?"#CCCCCC":"#D9D9D9") %>"
		...
		for(int colIndex=0; colIndex<colNames.size(); colIndex++){
			...
			String value = objValue == null ? "&nbsp" : objValue.toString();
                        String searchLink = "javaScript:getRowData('"
                                            ...
                                            +"/displayProposal.do?&proposalNo="
                                            +resultRow.get("PROPOSAL_NUMBER")
                                            +"')";
