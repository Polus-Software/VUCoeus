...
<title><bean:message key="searchWindow.title.person"
		bundle="coi" /></title>
<style type="text/css">
//
div {
...
<body onload="populateResultData()" class='tabtable'>
	<% String srch = "/personSearchCOI.do?type=+strType&personSearch=true"; %>
	<%if(resData == null) {%>
...
%>
    ...
    <logic:iterate id="dispBean" name="displayList" type="edu.mit.coeus.search.bean.DisplayBean">
        ...
        <td width="<bean:write name='dispBean' property='name'/>"><font
...
%>
		...
		for(int colIndex=0; colIndex<colNames.size(); colIndex++){
			...
			String value = objValue == null ? "&nbsp" : objValue.toString();
                        ...
                        String searchLink2= request.getContextPath()
                                            +"/personDetails.do?&userName="
                                            +resultRow.get("USER_NAME")
                                            +"&fullName="
                                            +resultRow.get("FULL_NAME")
                                            +"&personId="
                                            +resultRow.get("PERSON_ID");
                                            
                                            //+"')";
                        searchLink2 = searchLink2.replace('\'','`');
                        searchLink = searchLink + searchLink2 + "')";
