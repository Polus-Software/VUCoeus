...
<script language="JavaScript" type="text/JavaScript">
            ...
            function getRowData(rowId,rowStatus) {
              if(rowStatus == 'Inactive') {
                  alert('<bean:message bundle="iacuc" key="protocol.exceptionCode.2702"/>');
              }
              else {
                  document.iacucSearch.action = "<%=request.getContextPath()%>/iacucSearch.do?actionPerformed="+rowId;
                  document.iacucSearch.submit();
              }            
            }
            function openIacucProtocol(url) {
            ...
            <!-- COEUSQA:3005 - Dependency between the Location Name and Location Type in IACUC - Start -->
              <%  if("Location Type".equals(session.getAttribute("type"))) {
                  if(resData != null) {
                      String locationTypeCode = resData.get("LOCATION_TYPE_CODE").toString();
              %>
                   window.opener.loadLocationForLocType(<%=session.getAttribute("studyGroupId").toString()%>, <%=session.getAttribute("studyGroupLocationId").toString()%>, <%=session.getAttribute("locationIndex").toString()%>, <%=locationTypeCode%>);
               <% } } %>
            <!-- COEUSQA:3005 - End -->

            <%  if("Location Type In Location Tab".equals(session.getAttribute("type"))) {
            if(resData != null) {
            String locationTypeCode = resData.get("LOCATION_TYPE_CODE").toString();
            %>
            window.opener.loadLocationTypeFromSearch(<%=session.getAttribute("locationIndex").toString()%>,<%=session.getAttribute("locationFormId").toString()%>, <%=locationTypeCode%>);
            <% } } %>
              }
              <%}%> 
...
<body onload="populateResultData()" class='tabtable'>
	<% String srch = "/iacucSearch.do?type=+strType"; %>
	<%if(resData == null) {%>
				...
				<tr valign="center">
					<td class='tableheader'>&nbsp;<font color="#FFFFFF" size="2"><b>
								<font face="Arial, Verdana, Helvetica, sans-serif" color='black'>
									<bean:write name='searchinfoholder' property='displayLabel' />&nbsp;<bean:message
...
%>
		...
		for(int colIndex=0; colIndex<colNames.size(); colIndex++){
			...
			String value = objValue == null ? "&nbsp" : objValue.toString();
                        String searchLink = "";
                        if(searchName != null && "All Protocols".equals(displayName)){
                             String destPage = CoeusLiteConstants.GENERAL_INFO_PAGE;
                             searchLink = "javaScript:openIacucProtocol('"
                                +request.getContextPath()
                                +"/getIacucData.do?SEARCH_ACTION=SEARCH_WINDOW&protocolNumber="
                            ...
                            // searchLink = "javascript:getRowData('" +resRowCnt +"')";
                            searchLink = "javascript:getRowData('" +resRowCnt +"', '" +resultRow.get("STATUSDESC")+ "')";
                            //COEUSQA:2859 - End
                        }
