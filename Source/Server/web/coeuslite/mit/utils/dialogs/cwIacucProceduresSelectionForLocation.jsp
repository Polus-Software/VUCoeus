...
<script>
    function formSubmit(formId){
        document.iacucProceduresLocationList.action = "<%=request.getContextPath()%>/addRemoveProceduresForLocationForm.do?selectedFormId="+formId;
...
<body bgcolor="#376DAA">
    ...
    String selectedFormId = (String)request.getAttribute("selectedFormId");
    ...
    %>
	<html:form action="/addRemoveProceduresForLocationForm.do"
		method="post">
		<script>
    <%
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
						...
						<tr valign="top">
							<td>
								<%if(!modeValue){%> <bean:message bundle="iacuc"
									key="studyGroup.msg.procedureSelectForLocation" /> <%}else{%> <bean:message
									bundle="iacuc" key="studyGroup.msg.procedureForLocation" /> <%}%>
							</td>
						</tr>
					</table>
				</td>
					...
					<table width="100%" border="0" cellpadding="2" cellspacing="0"
						...
						<logic:present name="vecAddedSpeciesData" scope="session">
                            ...
                            %>
								...
								<logic:present name="iacucProcedureLocationTab" scope="session"
									property="additionalInfoList">
									<logic:iterate id="locProcDetail"
										name="iacucProcedureLocationTab" property="additionalInfoList"
										type="org.apache.struts.action.DynaActionForm" scope="session">
										<%String formId = locProcDetail.get("locationFormId").toString();
                                    int indexColor = 1;
                                    
                                    %>
										<logic:present name="locProcDetail"
											property="proceduresForLocationForm">
											<logic:iterate id="procedureDetail" name="locProcDetail"
												property="proceduresForLocationForm"
                                            ...
                                            int procSpeciesId = ((Integer)procedureDetail.get("speciesId")).intValue();
                                            if(selectedFormId != null && selectedFormId.equals(formId)
                                            ...
                                            %>
												...
												<tr bgcolor="<%=strBgColor%>" colspan="5"
													...
													<td class="copybold" colspan="4">
														<%indexColor++;
                                                    String selectAllLink = "javaScript:selectAllProcedures(this,'"+("procedureDetail"+speciesId)+"')";
                                                    %> <html:checkbox
															name="procedureDetail" styleId="<%=strSpeciesId%>"
															styleClass="copy" indexed="true"
															property="isAllProcedureSelected"
															disabled="<%=modeValue%>" onclick="<%=selectAllLink%>" />
														<bean:message bundle="iacuc"
															key="studyGroup.label.allProcedures" />
													</td>
												</tr>
                                            ...
                                            %>
												<tr bgcolor="<%=strBgColor%>" colspan="5"
													...
													<td class="copybold" colspan="4">
														&nbsp;&nbsp;&nbsp;&nbsp; <%
                                                    String checkBoxName = "procedureDetail"+speciesId+procedureIndex;
                                                    String isProcedureSelected = (String)procedureDetail.get("isProcedureSelected");
                                                    ...
                                                    %> <input
														...
														value="<%=isProcedureSelected%>" class="copy" disabled>
														<%}%> <%}%> <%=procedureDetail.get("procedureDesc")%>
													</td>
												</tr>
												<%indexColor++;%>
												<%procedureIndex++;%>
								...
								<script>
                                 var allProcSelElement = document.getElementById('<%=strSpeciesId%>');
			...
			<tr align="right" class="theader" height='30'>
				<td align='left' colspan="5">&nbsp; <%if(!modeValue){
                    String submitLInk = "javaScript:formSubmit('"+selectedFormId+"')";%>
