...
// Flooble Dynamic  Calendar. 
	...
	//This a default method is used to pass the offset value with 105,To display the cal beside the control. 
	...
	//This method is used to pass the offset value 100(100 + 105) to display the cal bit away from the control.
	function displayCalendarOffset(fieldId,lpos ) { 
		var formElement = fc_getObj(fieldId);
		leftpos = lpos;
		topPos = -10;
		displayCalendar(formElement);
	...
	//This method is used to position the control with Top position set
	function displayCalendarTop(fieldId,lpos, tpos) { 
		var formElement = fc_getObj(fieldId);
		//leftpos = lpos;
		//topPos = tpos;
		displayCalendar(formElement);
	...
	//This method is used to display the calendar as per the top position and left position values passed.
	function displayCalendarWithTopLeft(fieldId,top,lpos ) { 
...
*/	
	function displayCalendar(formElement) {
		...
		if (typeof(cal) == 'undefined' ) {
			...
			calendars[formElement.id] = cal;
                ...
                //For Morzilla
                if (!formElement[0].name) {
			formElement.id = fc_calCount++;
		} 
                    
		var cal = calendars[formElement[0].name];
                if (typeof(cal) == 'undefined' ) {
                	cal = new floobleCalendar();
			cal.setElement(formElement);
                	calendars[0] = cal;
	...
	function fc_setDate(d, m, y) {
		...
		this.date.setDate(d);
                ...
                if (this.mode == 1 &&  browserType== "IE") 
                {
                	this.element.value = this.formatDate();
		}
		else if (this.mode == 1 &&  browserType != "IE") 
                {
                	(this.element)[0].value = this.formatDate();
		} 
                else 
                {
			this.setDateFields();
	...
	function fc_show() {
		...
		if (typeof(this.div) == 'undefined') {
			...
			this.div.style.left = fc_absoluteOffsetLeft(this.element)+ 105 +  leftpos;
                        if(browserType == 'IE'  )
                         {   
                            // alert("show IE"+browserType);    
			    this.div.style.top = fc_absoluteOffsetTop(this.element) + this.element.offsetHeight - 10 - topPos;                        
                          }
                          else
                          {
                               //  alert("show MOZ"+browserType); 
                                this.div.style.top = fc_absoluteOffsetTop(this.element) + (this.element)[0].offsetHeight - 10 - topPos;
                          }    
	...
	function fc_generateHTML() {
		...
		var html = '<TABLE border="0" cellspacing="2" cellpadding="2" width="170"><TR CLASS="fc_head"><TD CLASS="fc_head" COLSPAN="6" height="20">&nbsp; Pick a Date</TD><TD CLASS="fc_head" align="center" onClick="getCalendar(\'' + this.id + '\').hide();" style="cursor:pointer;"><B>X</B></TD></TR>';
		html += '<TR><TD CLASS="fc_date" onMouseover="this.className = \'fc_dateHover\';" onMouseout="this.className=\'fc_date\';" onClick="getCalendar(\'' + this.id + '\').moveMonth(-12);"><B>&laquo;&laquo;</B></TD><TD CLASS="fc_date" onMouseover="this.className = \'fc_dateHover\';" onMouseout="this.className=\'fc_date\';" onClick="getCalendar(\'' + this.id + '\').moveMonth(-1);"><B>&laquo;</B></TD><TD COLSPAN="3" CLASS="fc_mon">' + fc_months[this.showDate.getMonth()] + ' ' + fc_getYear(this.showDate) + '</TD><TD CLASS="fc_date" onMouseover="this.className = \'fc_dateHover\';" onMouseout="this.className=\'fc_date\';" onClick="getCalendar(\'' + this.id + '\').moveMonth(1);"><B>&raquo;</B></TD><TD CLASS="fc_date" onMouseover="this.className = \'fc_dateHover\';" onMouseout="this.className=\'fc_date\';" onClick="getCalendar(\'' + this.id + '\').moveMonth(12);"><B>&raquo;&raquo;</B></TD></TR>';
		html += '<TR><TD WIDTH="14%" CLASS="fc_wknd">Su</TD><TD WIDTH="14%" CLASS="fc_wk">Mo</TD><TD WIDTH="14%" CLASS="fc_wk">Tu</TD><TD WIDTH="14%" CLASS="fc_wk">We</TD><TD WIDTH="14%" CLASS="fc_wk">Th</TD><TD class="fc_wk" WIDTH="14%">Fr</TD><TD class="fc_wknd" WIDTH="14%">Sa</TD></TR>';
		html += '<TR>';
		var dow = 0;
		var i, style;
		var totald = fc_monthLength(this.showDate);

		for (i = 1; i <= fc_firstDOW(this.showDate); i++) {
				dow++;
				html += '<TD> </TD>';
			}
	...
	function fc_moveMonth(amount) {
		...
		var y = fc_getYear(this.showDate);
                var d = this.showDate.getDate();
		if (amount == 1)  {
			if (m == 11)  {
				this.showDate.setMonth(0);
				this.showDate.setYear(y + 1);
			} else {
                                this.showDate.setMonth(1);
			...
			if (m == 0)  {
				...
				this.showDate.setMonth(m - 1);
                    ...
                    if(m==1 && d==29){
                        this.showDate.setYear(y + 1);
                        this.showDate.setMonth(m);
                        this.showDate.setDate(d-1);
                    } else {
                        this.showDate.setYear(y + 1);
                    }
		} else if (amount == -12) {
	...
	function fc_absoluteOffsetTop(obj) {
     ...
     function fc_parseToken(date, token, value, start) {
		...
		if (command == 'm' || command == 'M') {
			if (token.length == 2) { v = value.substring(start, start + 2); date.setMonth(1);date.setMonth(v - 1); }
			if (token.length == 3) { 
				v = value.substring(start, start + 3);
				var i;
				for (i = 0; i < fc_months.length; i++) {
					if (fc_months[i].toUpperCase() == v.toUpperCase()) { date.setMonth(1);date.setMonth(i); }
				}
			} 
		}
                v = value.substring(6,10); date.setFullYear(v);
     }	
