...
var Prototype = {
  ...
  Browser: {
    ...
    Version:(function() {	//	Find version number has an offset of browser name (generally)
		var v, p, b = navigator.userAgent.toLowerCase();
		if ((p = b.indexOf('konqueror')+9)	>= 9
		||	(p = b.indexOf('firefox')+7)	>= 7
		||	(p = b.indexOf('omniweb')+7)	>= 7
		||	(p = b.indexOf('version')+7)	>= 7
		||	(p = b.indexOf('safari')+6)		>= 6
		||	(p = b.indexOf('opera')+5)		>= 5
		||	(p = b.indexOf('webtv')+5)		>= 5
		||	(p = b.indexOf('icab')+4)		>= 4
		||	(p = b.indexOf('msie')+4)		>= 4
		)	v = parseInt(b.charAt(p+1));
		else if ((p = b.indexOf('compatible')) >= 0)
			v = parseInt(b.charAt(9));
		return v;
	})()
  },
...
Element.Methods = {
  ...
  addClassName: function(element, className) {
	...
	setClassNames:function(e,o){
		if((e=$(e))) {
			var b,r,cL=e.className||"";
			for(var n in o) {
				r=new RegExp("(^|\\s)"+n+"(\\s|$)");
				if((o[n]==false)||((b=r.test(cL))&&o[n]!=true)){
					cL=cL.replace(r," ").strip()
				} else {
					if(!b){
						cL+=(cL?" ":"")+n
					}
...
Fx.Base = Class.create({
	...
	step: function(){
		...
		if (time < this.time + this.options.duration){
			...
			this.now = this.to;
			if (this._complete
			&&	this._complete.bind) setTimeout(this._complete.bind(this, this.element), 9);
			if (this.options.onComplete
			&&	this.options.onComplete.bind) setTimeout(this.options.onComplete.bind(this, this.element), 10);
		}
		this.increase();
	},
	...
	setStyle: function(e, p, v){
		if (p == 'opacity'){
			...
			if (v<1) {
				...
				else e.style.opacity = v;
			} else {
				if (window.ActiveXObject) e.style.filter = "";
...
Fx.Accordion.prototype = Object.extend(new Fx.Base(), {
	...
	extendOptions: function(options){
		Object.extend(this.options, Object.extend({
			...
			height: true,
			opacity: false,// process heavy; Opacity is buggy in IE6 (!(Prototype.Browser.IE && Prototype.Browser.Version<7)),
			width: false
		}, options || {}));
	},
	...
	initialize: function(togglers, elements, options){
		...
		this.elements.each(function(el, i){
			this.now[i+1] = {};
			this.__d[i+1] = {h:parseInt(el.style.height),w:parseInt(el.style.width)};	//	Save inline size definitions
			el.style.height = '0';
	...
	hideThis: function(i){
		...
		if (this.options.closeClass) this.togglers[i].addClassName(this.options.closeClass);
		if (this.options.openClass) this.togglers[i].removeClassName(this.options.openClass);
	},

	showThis: function(i){
		if (this.options.height) this.h = {'height': [this.elements[i].offsetHeight, this.options.fixedHeight || this.__d[i+1].h || this.elements[i].scrollHeight]};
		if (this.options.width) this.w = {'width': [this.elements[i].offsetWidth, this.options.fixedWidth || this.__d[i+1].w || this.elements[i].scrollWidth]};
		if (this.options.opacity) this.o = {'opacity': [this.now[i+1]['opacity'] || 0, 1]};
		if (this.options.openClass) this.togglers[i].addClassName(this.options.openClass);
		if (this.options.closeClass) this.togglers[i].removeClassName(this.options.closeClass);
	},
	...
	showThisHideOpen: function(iToShow){
		if (iToShow != this.previousClick || this.options.alwaysHide){
			...
			this.elements.each(function(el, i){
				...
				if (i != iToShow) this.hideThis(i);
				else if (this.options.alwaysHide){
					if (el.offsetHeight == el.scrollHeight){	//	This doesn't handle horizontal scroll well
						this.hideThis(i);
						madeInactive = true;
					} else if (el.offsetHeight == 0) this.showThis(i);
					else err = true;
	...
	_complete:function() {
		var el = this.elements[this._index];						//	Remove process styles
		if (el && el.style) {
			el = el.style;
			if (this.options.height && !this.options.fixedHeight) el.height =  "";
			if (this.options.width && !this.options.fixedWidth) el.width =  "";
			if (this.options.opacity) el.opacity = "";
		}
	}
...
PS.NameSpace = {						//	NameSpace Methods
	
	define:function(path,classDef) {
		var o = window;
		var d = classDef;
		var t, a = path.split(".");
		while (t = a.shift()) {
			if (d && a.length<1) o = o[t] = d;
			else {
				if (o[t] == null) o[t] = {};
				o = o[t];
			}
	...
	rename:function(path,toName) {
		var p = path.split(".");
		var n = toName.split(".");
		if (p.length != n.length) return false;
		var t,s,o = window;
		while ((t = p.shift()) && (s = n.shift())) {
			if (t != s) {
				o[s] = o[t];
				delete o[t];
			}
