...
<%@ page
	import="org.apache.struts.validator.DynaValidatorForm ,
        edu.mit.coeuslite.utils.CoeusLiteConstants, edu.mit.coeuslite.iacuc.bean.ProtocolHeaderDetailsBean,java.util.HashMap"%>
<jsp:useBean id="vecRolesType" scope="session" class="java.util.Vector" />
<jsp:useBean id="iacucRolesDetails" scope="session"
...
<%  String EMPTY_STRING = "";
    ...
    if(mode.equalsIgnoreCase(CoeusLiteConstants.DISPLAY_MODE)){
        mode=(String)session.getAttribute("iacucRolesMode"+session.getId());
        modeValue=true;
...
function removeLink(value,data){
    if (confirm('<bean:message bundle="iacuc" key="protocolRoles.removeConfirm"/>')==true){
        document.iacucRolesForm.action = "<%=request.getContextPath()%>/deleteIacucUser.do?removeProtoRole="+value+"&removeProtoUser="+data;
        document.iacucRolesForm.submit();
    }
}       
var roleName ="";
function searchWindow(value) {       
 ...
 function fetch_Data(result){        
          ...
          document.iacucRolesForm.action = "<%=request.getContextPath()%>/addIacucUser.do?protoRole="+roleName+"&protoUserId="+userId+"&protoUserName="+userName+"&protoUnitNum="+unitNum+"&protoUnitName="+unitName+"&protoUserHomeUnit="+homeUnit;
          document.iacucRolesForm.submit();
    }        
</script>
</head>
<body>
	...
	<html:form action="/getIacucRoles">
		<script type="text/javascript">
    document.getElementById('txt').innerHTML = '<bean:message bundle="iacuc"  key="helpPageTextProtocol.Roles"/>';
</script>
			...
			<tr class='core'>
				<td>
					<table width='100%' border='0' cellpadding='2' cellspacing='0'
						...
						<% int count = 1;%>
						<logic:iterate id="hmapId" name="iacucRolesDetails"
							indexId="index">
							<bean:define id="protoRole" name="hmapId" property="key" />
							<% HashMap hmRoleName = (HashMap)session.getAttribute("iacucRoleNameId");
							...
							<tr>
								...
								<td align=right>
									...
									<%if(((Integer)protoRole != CoeusLiteConstants.IACUC_PROTOCOL_APPROVER_ID) && !modeValue){%>
									<% 
                                    link = "javaScript:searchWindow('"+protoRole+"');";%>
									<html:link href="<%=link%>">
										<u><bean:message bundle="iacuc" key="protocolRoles.Users" /></u>
									</html:link> <%}%>
								</td>
								<td>
										...
										<table width='100%' border='0' cellpadding='3' cellspacing='1'
											...
											<tr>
												<td class='theader' width='5%' nowrap><bean:message
														bundle="iacuc" key="protocolRoles.UserID" /></td>
												<td class='theader' width='30%'><bean:message
														bundle="iacuc" key="protocolRoles.UserName" /></td>
												<td class='theader' colspan='2' width='50%'><bean:message
														bundle="iacuc" key="protocolRoles.HomeUnit" /></td>
												<%if(!modeValue){%>
											...
											<logic:iterate id="user" name="beanData"
												...
												<tr bgcolor="<%=strBgColor%>"
													...
													<%--<%if(!createUser.equals(roleUserId) || !roleNam.equals("IACUC Protocol Aggregator")){%> --%>
													<!-- Modified for COEUSQA-2824_IRB and IACUC protocol access permissions should not include approver_start-->
													<%if(((Integer)protoRole != CoeusLiteConstants.IACUC_PROTOCOL_APPROVER_ID)){%>
													<%if((beanData != null && beanData.size() > 1) 
                                                                &&!createUser.equals(roleUserId) 
                                                                ||!roleNam.equals("IACUC Protocol Aggregator")){%>

													<%String removeLink = "javaScript:removeLink('"+protoRole+"','"+roleUserId+"')";%>
													<td class='copy' width='15%' align=center><html:link
															href="<%=removeLink%>">
															<bean:message bundle="iacuc" key="protocolRoles.Remove" />
														</html:link></td>
													<%}else{%>
						...
						<tr>
							<td colspan='4' class='copybold'><font><bean:message
										bundle="iacuc" key="protocolRoles.error" /></font></td>
						</tr>
						<%}%>
					</table>
				</td>
