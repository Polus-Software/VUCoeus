...
<jsp:useBean id="iacucReferenceTypes" scope="session"
	class="java.util.Vector" />
<jsp:useBean id="iacucReferenceList" scope="session"
	class="java.util.Vector" />
...
<script>
       ...
       function validatepage() {   
            document.iacucReference.acType.value = "I";           
        }
        
        function delete_data(desc,timestamp){
       if (confirm(<bean:message key="protocolReference.deleteMessage"/>)==true){
            document.iacucReference.acType.value = "D";
            document.iacucReference.referenceNumber.value = desc;            
            document.iacucReference.awUpdateTimestamp.value = timestamp;
            document.iacucReference.action = "<%=request.getContextPath()%>/deleteIacucReferenceAction.do";
            document.iacucReference.submit();
        }
    }
       ...
       if(val.length<1000)
        ...
        if (window.screen) {
               ...
               topPos = Math.floor(((window.screen.height - 350) / 2));
                ...
                var loc = '<bean:write name='ctxtPath'/>/coeuslite/mit/utils/dialogs/cwViewSpecialReview.jsp?value='+value +'&type=R&module=IACUC_PROTOCOL';
...
<body>
	...
	<html:form action="/saveIacucReferenceAction">
		<script type="text/javascript">
            document.getElementById('txt').innerHTML = '<bean:message bundle="iacuc" key="helpPageTextProtocol.References"/>';
        </script>
		<%if(!modeValue){%>
					...
					<table width='100%' align=center border='0' cellpadding='0'
						...
						<tr class='tableheader'>
							<td width="80%" align="left" valign="top" colspan='3'><bean:message
									bundle="iacuc" key="protocolReference.ProtocolReferences" /></td>
    ...
    --%>
						...
						<tr>
							...
							<td class="copybold" align="right" width='7%' nowrap>
								<%-- Commented for case id#2627
            <font color="red">*</font>
            --%> <bean:message key="protocolReference.ReferenceType" />:&nbsp;
							...
							<td align=left width='25%' class='copy'><html:select
									...
									<html:options collection="iacucReferenceTypes" property="code"
            ...
            --%> <bean:message key="protocolReference.ReferenceNumber" />:&nbsp;
						...
						<tr>
							<td colspan="6">
								<table width='100%' border='0'>
									<tr>
										<td class='copybold' width='7%' align="right"><bean:message
												key="specialReviewLabel.ApplDate" />:</td>
										<td align=left width='9%'><html:text
												property="applicationDate" maxlength="10" size="13"
												...
												<img id="imgIRBDate" title="" height="16" alt=""
													...
													onclick='dataChanged()' width="16" border="0"
													runat="server">
											</html:link> <%}%></td>
										<td align=right width='7%' class='copybold'><bean:message
												key="specialReviewLabel.ApprDate" />:</td>
										<td align=left width='9%'><html:text
												property="approvalDate" maxlength="10" size="13"
												onchange="dataChanged()" /> <%if(!modeValue){%> <html:link
												href="javascript:void(0)"
										...
										<td align=left width='9%'><html:text
												property="expirationDate" maxlength="10" size="13"
												onchange="dataChanged()" /> <%if(!modeValue){%> <html:link
												href="javascript:void(0)"
												onclick="displayCalendarWithTopLeft('expirationDate',10,-45)"
												tabindex="32">
												<img id="imgIRBDate" title="" height="16" alt=""
													src="<bean:write name='ctxtPath'/>/coeusliteimages/cal.gif"
													onclick='dataChanged()' width="16" border="0"
						...
						<tr>
							...
							<td class='copybold' width='10%' align=right><bean:message
            ...
            if(navigator.appName == "Microsoft Internet Explorer")
            {
                document.iacucReference.comments.cols=117;
                document.iacucReference.comments.rows=3;
            }
       </script>
						</tr>
						<tr>
			...
			<tr>
				<td class='core'>
					...
					<table width='100%' align=center border='0' cellpadding='0'
                       ...
                       int count = 0;
                       %>
						<logic:iterate id="reference" name="iacucReferenceList"
							...
							<tr bgcolor='<%=strBgColor%>'
								...
								<td width="15%" class="copy"><coeusUtils:formatDate
										name="reference" property="expirationDate" /></td>
		...
		<html:hidden property="protocolNumber" value="" />
		<html:hidden property="sequenceNumber" value="" />
		<html:hidden property="referenceNumber" value="" />
		<html:hidden property="awUpdateTimestamp" value="" />
	</html:form>
	<script>
          DATA_CHANGED = 'false';
          if(errValue && !errLock) {
          ...
          document.iacucReference.acType.value = 'I';
          LINK = "<%=request.getContextPath()%>/saveIacucReferenceAction.do";
          FORM_LINK = document.iacucReference;
