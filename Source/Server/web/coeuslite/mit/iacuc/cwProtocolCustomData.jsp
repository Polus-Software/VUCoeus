...
<script>
    ...
    function tabSelected(linkSelected, status){
        if(status == 'G'){            
            CLICKED_LINK = "<%=request.getContextPath()%>/getIacucGroups.do?selectedTab="+linkSelected+"&oldSelectedTab=<%=selectedTab%>";            
            if(validate()){
                document.protocolCustomDataList.action = "<%=request.getContextPath()%>/getIacucGroups.do?selectedTab="+linkSelected+"&oldSelectedTab=<%=selectedTab%>";
...
document.getElementById('txt').innerHTML = '<bean:message bundle="iacuc" key="helpPageTextProtocol.Others"/>';
</script>
	<!-- Added for Page Help - End -->
	<html:form action="/saveIacucCustomData.do">
				...
				<td>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td height="20" align="left" valign="top" class="tableheader">
								<bean:message key="protocolCustomData.label.header" />
							</td>
	...
	<script>
      ...
      LINK = "<%=request.getContextPath()%>/saveIacucCustomData.do";
