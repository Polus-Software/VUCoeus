...
<script>
        function completeSubmission(){
                document.iacucProtocolActionsForm.action = "<%=request.getContextPath()%>/saveIacucActions.do?fromCompleteActionMenu=true";
                document.iacucProtocolActionsForm.submit();
            }
         function cancelCompletion(){
                if(!confirm('<bean:message bundle="iacuc" key="protocolAction.msg.cancelConfirmation"/> ')){
                    return;
                }
                document.iacucProtocolActionsForm.action = "<%=request.getContextPath()%>/cancelIacucSubmission.do";
                document.iacucProtocolActionsForm.submit();
         }
        </script>
</head>
...
%>
	...
	<html:form action="/completeIacucSubmission" method="post">
		<a name="top"></a>
				...
				<td align="left" valign="top" class='core'>
					<table width="100%" border="0" align="center" cellpadding="3"
                       ...
                       if(mandaroryQnrMessage != null && !"".equals(mandaroryQnrMessage)){%>
							...
							<script>document.iacucProtocolActionsForm.Save.focus();</script>
							<%}%>
