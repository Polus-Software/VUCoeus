...
<%if(CoeusLiteConstants.YES.equalsIgnoreCase(newQuestionnaireVersion)){%>   
    ...
    // COEUSDEV-230: Answered questionnaire says it is not Answered in Approval in Progress Proposal - Start
    document.dynaBeanList.action = "<%=request.getContextPath()%>/getIacucSubmissionQuestionnaire.do?questionnaireId=<%=questionnaireId%>&questionaireLabel=<%=questionnaireLabel%>&menuId=<%=menuId%>&actionFrom=<%=actionFrom%>&newVersionMsgDisplayed=true";
    document.dynaBeanList.submit();  
    // COEUSDEV-230: Answered questionnaire says it is not Answered in Approval in Progress Proposal - End
<%}%>
</script>
		...
		<script>
    <%if(questionnaireModuleObject.getModuleItemCode() == ModuleConstants.IACUC_MODULE_CODE){%>
