...
<body>
    ...
    function showHide(val,value){
           ...
           function showScheduleAttachments(schID, acType, PROTOCOL_TYPE, MenuId, SUBHEADER_ID)
          {
            var winleft = (screen.width - 800) / 2;
            var winUp = (screen.height - 450) / 2; 
            var win = "scrollbars=1,resizable=1,width=800,height=500,left="+winleft+",top="+winUp;    
            sList = window.open("<%=request.getContextPath()%>/iacucList.do?scheduleId="+schID+"&IACUC_TYPE="+PROTOCOL_TYPE, "list", win);                        
          }
										...
										<logic:iterate id="protocol" name="iacucList"
											...
											<tr bgcolor="<%=strBgColor%>" valign="top"
												...
												<td align="left" nowrap class="copy"><a
													href="<%=request.getContextPath()%>/iacucList.do?IACUC_TYPE=IACUC_REVIEWER_SCHEDULE_AGENDA_SEARCH&#38;Menu_Id=008&scheduleId=<%=scheduleId%>&fromIsReviewer=true&isAgenda=false&openAgenda=false"><u>
															<%=value%>
													</u> </a></td>
												<%
                                                    }
												...
												<td align="left" nowrap class="copy">
													...
													href="<%=request.getContextPath()%>/iacucList.do?&scheduleId=<%=scheduleId%>&acType=V&IACUC_TYPE=IACUC_REVIEWER_SCHEDULE_SEARCH&#38;Menu_Id=008&SUBHEADER_ID=SH008&isAgenda=true&openAgenda=true"
													, target="_blank"><u> View Agenda</u> </a> <%}else{%> <a
													href="<%=request.getContextPath()%>/iacucList.do?&scheduleId=<%=scheduleId%>&acType=V&IACUC_TYPE=IACUC_REVIEWER_SCHEDULE_SEARCH&#38;Menu_Id=008&SUBHEADER_ID=SH008&isAgenda=true&openAgenda=true"><u>
															View Agenda</u> </a> <%}%>
												</td>

												<!-- COEUSQA:3333 - IRB and IACUC - Ability to Add Attachments to Minutes and Agenda - Start -->
												<td align="left" nowrap class="copy"><a
													href="javaScript:showScheduleAttachments('<%=scheduleId%>', 'V', 'SCHEDULE_ATTACHMENTS', '002', 'SH008');"
																			...
																			<td>
																				<table width="100%" height="100%" border="0"
																					...
																					<tr bgcolor="<%=strInnerBgColor%>" valign="top"
																						...
																						<td align="left" nowrap class="copy"><a
																							href="<%=request.getContextPath()%>/getIacucData.do?SEARCH_ACTION=SEARCH_ACTION&protocolNumber=<%=submittedprotocol%>&PAGE=<%=innerDestPage.trim()%>&sequenceNumber=<%=protoSequenceNumber%>&submissionNumber=<%=protoSubmissionNumber%>&fromIsReviewer=true"><u>
																									<%=submittedprotocol%>
																							</u> </a></td>
																						<td align="left" nowrap class="copy"><a
																							href="<%=request.getContextPath()%>/getIacucData.do?SEARCH_ACTION=SEARCH_ACTION&protocolNumber=<%=submittedprotocol%>&PAGE=<%=innerDestPage.trim()%>&sequenceNumber=<%=protoSequenceNumber%>&submissionNumber=<%=protoSubmissionNumber%>&fromIsReviewer=true"><u>
																						...
																						<td align="center" nowrap class="copy">
																							<%
                                                                                                            int reviewerTypeCode = 0;
                                                                                                            if(hmSubmittedprotocol.get("REVIEWER_TYPE_CODE") != null && hmSubmittedprotocol.get("REVIEWER_TYPE_CODE") !=""){
                                                                                                                  reviewerTypeCode = Integer.parseInt(hmSubmittedprotocol.get("REVIEWER_TYPE_CODE").toString());
                                                                                                            }
		...
		<script>
                   <%if(!hasErrorMessage){%>
                    var msg = '<bean:message key="schedule_no_agenda_details"/>';
                        var agendaNumPresent = "<%=(String)session.getAttribute("IACUC_AGENDA_NUMBER"+session.getId())%>";
