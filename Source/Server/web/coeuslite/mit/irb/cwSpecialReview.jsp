...
<script>
        ...
        {
            document.specialReview.action = "<%=request.getContextPath()%>/specialReviewActions.do?actionPerformed="+action;
            document.specialReview.submit();
        }
      
      function delete_data(data,desc,timestamp){
       if (confirm("Are you sure you want to delete the special review?")==true){
            document.specialReview.acType.value = data;
            document.specialReview.specialReviewNumber.value = desc;            
            document.specialReview.awUpdateTimestamp.value = timestamp;
            document.specialReview.action = "<%=request.getContextPath()%>/specialReviewAction.do";
            document.specialReview.submit();
        }
      }
    
      function edit_data(desc,timestamp){
            document.specialReview.acType.value = 'E';
            document.specialReview.specialReviewNumber.value = desc;            
            document.specialReview.awUpdateTimestamp.value = timestamp;
            document.specialReview.action = "<%=request.getContextPath()%>/specialReviewAction.do";
            document.specialReview.submit();
        ...
        var val = document.specialReview.acType.value;
        if(val!='U'){
            document.specialReview.acType.value = data;
        }
        document.specialReview.action = "<%=request.getContextPath()%>/specialReviewActions.do";
        ...
        function add_specialReview(){
            ...
            if(validate() == true){
                document.specialReview.action = "<%=request.getContextPath()%>/getProtocolData.do?PAGE=S";
                document.specialReview.submit();
            }
        }
       ...
       if(val.length<1000)
        ...
        if (window.screen) {
         ...
         function disableSaveButton(){
            document.specialReview.save.disabled=true;
         }
        </script>
...
<body>
	<% 
        ...
        Boolean splRvwModeInDisplay = (Boolean) session.getAttribute("MODIFY_PROTO_SPECIAL_RVW"+session.getId());
	...
	<html:form action="/specialReviewActions" method="post"
		onsubmit="disableSaveButton()">
		<a name="top"></a>
		<%--  ************  START OF BODY TABLE  ************************--%>
		<!-- New Template for cwViewFinEntity - Start   -->
		<script type="text/javascript">
    document.getElementById('txt').innerHTML = '<bean:message key="helpPageTextProtocol.SpecialReview"/>';
</script>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							<td height="20%" align="left" valign="top" class="theader">
								<bean:message key="specialReviewLabel.SpecialReview" />
							</td>
					...
					<table width="100%" border="0" align="center" cellpadding="0"
						...
						<tr>
							<td align="left" valign="top">
								<table width="100%" height="20%" border="0" cellpadding="0"
									...
									<tr>
										<td><bean:message
												key="specialReviewLabel.AddSpecialReview" /></td>
									</tr>
								</table>
							</td>
						</tr>
								...
								<table width='100%' border='0'>
                                    ...
                                    --%> &nbsp;&nbsp;<bean:message
										...
										<td width='40%' align=left class='copy'>&nbsp; <html:select
												...
												<html:options collection="splReview" property="code"
                                    ...
                                    --%> <bean:message
										...
										<td align=left valign=bottom class='copy'>&nbsp; <html:select
												property="approvalCode" styleClass="cltextbox-medium"
												onchange="dataChanged()" disabled="<%=modeValue%>">
												<html:option value="">
													<bean:message key="specialReviewLabel.pleaseSelect" />
												</html:option>
						...
						<tr>
							<td>
								...
								<table width='100%' border='0'>
									<tr>
										<td nowrap class="copybold" width="20%" align="left">&nbsp;&nbsp;<bean:message
												key="specialReviewLabel.ProtocolNo" />:
										</td>
										...
										<td nowrap class="copybold" width="5%" align="left">&nbsp;&nbsp;<bean:message
												key="specialReviewLabel.ApplDate" />:
										</td>
										...
										<td nowrap class="copybold" width="5%" align="left">&nbsp;&nbsp;<bean:message
												key="specialReviewLabel.ApprDate" />:
										</td>
										<td valign="top" nowrap class="copy" width='10%' align=left>
											<html:text property="approvalDate" styleClass="textbox"
												...
												<img id="imgIRBDate" title="" height="16" alt=""
													...
													width="16" onclick='dataChanged()' border="0"
													runat="server">
											</html:link> <%--a id="hlIRBDate" onclick="displayCalendarWithTopLeft('approvalDate',10,-150)" tabindex="32" href="javascript:void(0);"
                                        runat="server"><img id="imgIRBDate" title="" height="16" alt="" src="<bean:write name='ctxtPath'/>/coeusliteimages/cal.gif" width="16"
                                        border="0" runat="server">
                                    </a--%> <%}%>
										</td>
									...
									<tr>
										<td nowrap class="copybold" width="7%" align="left"
											valign="top">&nbsp;&nbsp;<bean:message
												key="specialReviewLabel.Comments" />:
										</td>
										<td class="copy">
											<%--div class='copyitalics'><bean:message key="specialReview.commentsLimit"/></div--%>
											&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <html:textarea
												property="comments" styleClass="copy" cols="119" rows="3"
				...
				<td align="left" valign="top" class='core'><table width="100%"
						...
						<tr>
							<td colspan="4" align="left" valign="top"><table
									...
									cellspacing="0" class="tableheader">
									<tr>
										<td><bean:message key="specialReviewLabel.ListofSpReview" />
										</td>
									</tr>
								</table></td>
						</tr>
						<tr align="center">
							<td colspan="3"><br>
								<DIV
									STYLE="overflow: auto; width: 780px; height: 330px; padding: 0px; margin: 0px">
									<table width="100%" border="0" cellpadding="0" class="tabtable">
										<tr align="left">
											...
											<td width="10%" class="theader"><bean:message
													key="specialReviewLabel.Approval" /></td>
											<td width="13%" class="theader"><bean:message
...
//JM 5-31-2011 updated per 4.4.2                                               
                                           ...
                                           %>
                                                ...
                                                String applDateVal = dateUtils.formatDate((String)review.get("applicationDate"),edu.mitweb.coeus.irb.DateUtils.MM_DD_YYYY);
                                                review.set("applicationDate",applDateVal);
                                                System.out.println("edu.mitweb.coeus.irb.DateUtils.MM_DD_YYYY "+edu.mitweb.coeus.irb.DateUtils.MM_DD_YYYY);
                                                System.out.println("applDateVal"+dateUtils.formatDate((String)review.get("applicationDate"),edu.mitweb.coeus.irb.DateUtils.MM_DD_YYYY));
                                                //}else{
                                                //applDateVal = "";
                                                --%> <%String applDate = "";
	...
	<script>
          ...
          if(document.specialReview.acType.value != 'U'){
            document.specialReview.acType.value = 'I';
          } else {
            document.getElementById('add_label').style.display = 'block';
          }
          LINK = "<%=request.getContextPath()%>/specialReviewActions.do"; 
          FORM_LINK = document.specialReview;
