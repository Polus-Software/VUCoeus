...
<%@ page import="edu.mit.coeuslite.utils.CoeusLiteConstants,
            ...
            java.util.HashMap, java.util.Vector, edu.mit.coeus.utils.ModuleConstants"%>
...
<% 
    ...
    String mode=(String)session.getAttribute("questionnaireMode"+session.getId()); 
    ...
    String completionFlag = (String)request.getParameter("completed");
    String applicableSubmoduleCode = (String)request.getParameter("apSubModuleCode");
    String applicableModuleItemKey = (String)request.getParameter("apModuleItemKey");
    String applicableModuleSubItemKey = (String)request.getParameter("apModuleSubItemKey");
    int moduleCode = questionnaireModuleObject.getModuleItemCode();
    if((moduleCode == 7 || moduleCode == 9) && questionnaireModuleObject.getModuleSubItemCode() == 1){
        questionnaireInfoReqd = true;
        if("0".equals(applicableSubmoduleCode) 
            && "Y".equals(completionFlag)){
...
<%if(CoeusLiteConstants.YES.equalsIgnoreCase(newQuestionnaireVersion)){%>   
    ...
    alert('New version of this form is available. Answer a un-answered question in the latest version of this form.');
    // COEUSDEV-230: Answered questionnaire says it is not Answered in Approval in Progress Proposal - Start
    document.dynaBeanList.action = "<%=request.getContextPath()%>/getQuestionnaire.do?questionnaireId=<%=questionnaireId%>&menuId=<%=menuId%>&actionFrom=<%=actionFrom%>&newVersionMsgDisplayed=true&questionaireLabel=<%=questionnaireLabel%>&completed=<%=completionFlag%>&apSubModuleCode=<%=applicableSubmoduleCode%>&apModuleItemKey=<%=applicableModuleItemKey%>&apModuleSubItemKey=<%=applicableModuleSubItemKey%>";
    document.dynaBeanList.submit();  
    // COEUSDEV-230: Answered questionnaire says it is not Answered in Approval in Progress Proposal - End
<%}%>
</script>
...
<script>
    <%if(questionnaireModuleObject.getModuleItemCode() == ModuleConstants.PROTOCOL_MODULE_CODE){%>
        alert('Questionnaire Completed for protocol <%=questionnaireModuleObject.getModuleItemKey()%>');
    <%} else if(questionnaireModuleObject.getModuleItemCode() == ModuleConstants.PROPOSAL_DEV_MODULE_CODE){%>
        alert('Questionnaire Completed for proposal <%=questionnaireModuleObject.getModuleItemKey()%>');
    ...
    <tr>
                 ...
                 %>
                  ...
                  <tr class="copybold">
                      ...
                      <td class='copybold' align=left width="15%">
                          <%String queLink = "javascript:showQuestion('/showQuestionExplanation.do?questionNo="+dynaFormData.get("questionId")+"&questionNumber="+dynaFormData.get("questionNumber")+"')";%>
                                   ...
                                   if(validAnswers.equalsIgnoreCase("Text")){%>
                                        ...
                                        <%if(maxLength <= 2000){%>
                                                <%if(maxLength <= 60){ 
                                                        ...
                                                        //Added for COEUSQA-3478 : CoeusLite Protocol Review Issue - Questionnaire Text Box Scroll - start
                                                        // Need to add extra space to text field to view whole answer. 
                                                        String textFieldSize = "";    
                                                        String txtFieldLength = "";
                                                        if(maxLength <= 20){
                                                            textFieldSize = "38";    
                                                        }else{
                                                            if(!"".equals(answerMaxLength)){
                                                                int answerMaxLen = Integer.parseInt(answerMaxLength)+18;
                                                                txtFieldLength = ""+answerMaxLen;
                                                            }
                                                            textFieldSize = txtFieldLength;    
                                                             
                                                        }
                                                        //COEUSDEV - 
                                                    //COEUSDEV-136 - End
                                                    //Added for COEUSQA-3478 : CoeusLite Protocol Review Issue - Questionnaire Text Box Scroll - end
                                                    //Sets text field maxlength to 10, if the datatype is date
                                                    if(answerDataType!= null && answerDataType.equals("Date")){ 
                                                    %>
                                                    <html:text property="answer" name="dynaFormData" indexed="true" size="20" maxlength="10" disabled="<%=modeValue%>" onchange="dataChanged()"/>
                                                    <%}else{%>
                                                    <html:text property="answer" name="dynaFormData" indexed="true" size="<%=textFieldSize%>" maxlength="<%=answerMaxLength%>" disabled="<%=modeValue%>" onchange="dataChanged()"/>
                                                    <%}%>
                                                    <!--COEUSDEV-137 - End  -->
                                                <%}else{%>
                                                    <%  String styleId = "textArea"+textAreaId;                                                        
                                                        String javaScriptLink = "javaScript:countCharacters('"+styleId+"',this.form.left,'"+answerMaxLength+"')"; 
                                                       %>
                                                    <html:textarea property="answer" name="dynaFormData" styleId="<%=styleId%>" styleClass="textbox-longer" cols="150" rows="5" onkeydown="<%=javaScriptLink%>" onkeyup="<%=javaScriptLink%>" indexed="true" readonly="<%=modeValue%>"  onchange="dataChanged()"/>
                                                <%}%>
                                          <%}%>                                        
                                   ...
                                   <%}else if(validAnswers.equalsIgnoreCase("YN")){
                                    ...
                                    <%}else if(isLookupPresent){%>
                                     ...
                                     <%  String pageUrl="javaScript:openLookupWindow('"+lookupGUI+"','"+lookUpName+"','','"+count+"')";
                                        ...
                                        <%}%>
                                        <html:text property="searchName"  name="dynaFormData" maxlength="2000" styleClass="cltextbox-color" indexed="true" readonly="true"/>
                                     <%}%>     
      ...
      <tr>
       ...
       <tr align="left" class='table'> 
           <td class='savebutton' colspan="2" nowrap>
               
               &nbsp;<html:button property="saveAndProceed" styleClass="clsavebutton" value="<%=buttonName%>" onclick="javascript:processData('SAVE')"/>
               <%if(modifyCompletedQuestionnaire != null && "YES".equals(modifyCompletedQuestionnaire) && !originalMode){%>
               &nbsp;<html:button property="saveAndComplete" styleClass="clsavebutton" value="Save & Complete" onclick="javascript:processData('SAVE_COMPLETE')"/>
               <%}%>
               &nbsp;<html:button property="print" styleClass="clsavebutton" value="Print" onclick="javascript:view_data('PRINT')"/>
               </td>      
       </tr>
  <%} else {%>
      <tr align="left" class='table'> 
...
<script>
      ...
      function countCharacters(field, count, max) {
         if (document.getElementById(field).value.length > max)
             ...
             count.value = max - document.getElementById(field).value.length;
      }
</script> 
</body>
</html:html>
