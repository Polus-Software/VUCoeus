<%--
    Document   : RightPersonCertify
    Created on : Dec 16, 2010, 2:25:34 PM
    Author     : anishk
--%>
...
<html:html>
    ...
    <script>
        ...
        function processData(operation){
            ...
            document.dynaBeanList.action = "<%=request.getContextPath()%>/saveQuestData.do?operation="+operation;
        ...
        function view_data(operation){
          ...
          <% if (mess != null && mess.equals("COMPLETED")) { %>
            ...
            window.open("<%=request.getContextPath()%>/saveQuestData.do?operation="+operation+"&printQuestions="+printQuestion+"&printAllQuestions="+printAllQuestion);
            //4287 End
         <%                  }
            else
                {%>
                   alert('Please save the Questionnaire');
              <%  }%>
              }
        ...
        function coiCertifyPrompt(){
            if(confirm("Do you want disclose this project?")){ 
                alert('<%=proposalNumber%>');            
            document.dynaBeanList.action = "<%=request.getContextPath()%>/coiMailAction.do?proposalNo="+'<%=proposalNumber%>'+"&personId="+'<%=prop_personId%>'+"&moduleItemKey="+'<%=proposalNumber%>'+"&moduleCode=3";
            document.dynaBeanList.submit();
            }
    ...
    <body>
        <logic:present name="displayJSP">
            
           <%-- <% %>--%>
        <html:form action="/saveQuestData"   method="POST">


             <logic:present name="firstQuest">
            <%String url= (String)request.getAttribute("firstQuest") ;%>
            ...
            <!-- 4272: Maintain History of Questionnaires - Start -->
                ...
                <%if (CoeusLiteConstants.YES.equalsIgnoreCase(newQuestionnaireVersion)) {%>
                    ...
                    // COEUSDEV-230: Answered questionnaire says it is not Answered in Approval in Progress Proposal - Start
                    document.dynaBeanList.action = "<%=request.getContextPath()%>/getQuest.do?questionnaireId=<%=questionnaireId%>&menuId=<%=menuId%>&actionFrom=<%=actionFrom%>&newVersionMsgDisplayed=true&questionaireLabel=<%=questionnaireLabel%>";
                    document.dynaBeanList.submit();
                    // COEUSDEV-230: Answered questionnaire says it is not Answered in Approval in Progress Proposal - End
                <%}%>
            </script>
            ...
            <script>
                <%if (questionnaireModuleObject.getModuleItemCode() == ModuleConstants.PROPOSAL_DEV_MODULE_CODE) {%>
                    alert('Questionnaire \"<%=session.getAttribute("questionaireLabel")%>\" is  Completed');
                <%}%>
               <%if (request.getAttribute("ppcPersonCertifyComplete")!=null) {%>
                   javascript:coiCertifyPrompt();
                 <%}%>   
            </script>
            <%}%>
                                    ...
                                    <font color='red'>
                                        ...
                                        <logic:empty name="questionsList" property="list" scope="session">
                                            <%String errorMessage = "";
                                                isQnrPresent = false;
                                            %>        
                                            <%errorMessage = "Proposal certification questions are not answered";%>
...
<!--                                    &nbsp;&nbsp;&nbsp;&nbsp;<html:button property="print" styleClass="clsavebutton" value="Print" onclick="javascript:view_data('PRINT')"/>-->
        ...
        <script>
            ...
            LINK = "<%=request.getContextPath()%>//saveQuest.do?operation=SAVE";
