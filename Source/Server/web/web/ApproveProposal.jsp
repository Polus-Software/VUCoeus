...
<coeusPrivilege:checkLogin
	requestedURL= 'ApproveProposalAction.do?proposalNumber=<%=request.getParameter("proposalNumber") %>'
...
<template:insert template='HNCTemplate.jsp'>
  ...
  <template:put name='title'> <bean:message key='approveProposal.title'/> </template:put>
  ...
  <!-- No navigation bar since the page has a cancel button which serves as a back button -->
<%--  <template:put name='navbar' content='CoeusLeftPane.jsp' />--%>
  ...
  <template:put name='actualContent' content='ApproveProposalContent.jsp'/>
</template:insert>
