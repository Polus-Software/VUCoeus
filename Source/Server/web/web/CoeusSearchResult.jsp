<%--
/*
 * @(#)CoeusSearchResult.jsp	1.0	2002/06/11	23:12:30
 *
...
function populateResultData(keyVal){
    	var obj = eval("window.opener.document.forms[0].<%=fieldName%>");
        obj.value = keyVal;
        window.close();
}
</script>
...
<%--
    <logic:iterate id='values' name='resList' type='java.util.Vector' scope='page'>
        <% colIndex =0; %>
    <tr bgColor="<%= ((++index)%2==0?"#F7EEEE":"#FFFFFF") %>" valign="top">
        <logic:iterate id='value' name='values' type='java.lang.String'>
        --%>
            <%-- <logic:equal name="colIndex"  value="0" > --%>
            <%--
            <% if (colIndex ==0) { %>
                <td><div><a href ="JavaScript:populateResultData('<%=value%>');"><%=UtilFactory.dispEmptyStr(value)%></a></div></td>
                <%} else {%>
                --%>
            <%-- </logic:equal>
            <logic:notEqual name="colIndex"  value="0" >       --%>
            <%--
...
<%
		for(int colIndex=0; colIndex<colNames.size(); colIndex++){
			...
			if (colIndex ==0) {
%>
                		<td><div><a href ="JavaScript:populateResultData('<%=value%>');">
