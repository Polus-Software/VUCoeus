...
<coeusPrivilege:checkLogin 
		requestedURL="getInboxMessages.do?messageType=unresolved" 
...
<template:insert template='HNCTemplateNoLeftPane.jsp'>
  ...
  <template:put name='title'> <bean:message key='unresolved.title'/> </template:put>

  <%-- Supply the Header information to the HNCTemplate.jsp --%>
  <template:put name='header' content='CoeusHeader.jsp' />


  <%-- In order to conserve page real estate for Inbox messages display pages, a navigation bar
    	at the top and bottom of the pages is substituted for the left pane nav bar.  --%>
  ...
  <template:put name='actualContent' content='UnresolvedMessagesContent.jsp' />
