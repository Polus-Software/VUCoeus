/*
 * FinEntityAction.java
 *
 * Created on December 26, 2005, 3:39 PM
 */
 ...
 * @author  vinayks
 */
public class FinEntityAction extends COIBaseAction{
    ...
    /** Creates a new instance of FinEntityAction */
    public FinEntityAction() {
    }
    /*
    *Fix # 2866
    ...
    */
    ...
    HttpServletResponse response) throws Exception {
       ...
       // String name  = ((HttpServletRequest)context.getRequest()).getParameter("actionFrom");
        ...
        String actionFrom = (String)dynaFinEntityForm.get("actionFrom");
        ...
        dynaFinEntityForm.set("statusCode",ACTIVE_STATUS);
        dynaFinEntityForm.set("relatedToOrgFlag",DEFAULT_ORG_RELATIONSHIP);
        dynaFinEntityForm.set("updtimestamp",dbTimestamp.toString());
        ...
        if(actionMapping.getPath().equals("/deactivateFinEnt") ){
        ...
        }else if(actionMapping.getPath().equals("/deactivateFinEntSubmit")){
            ...
            if(vecData!=null && vecData.size()>0){
                ...
                webTxnBean.getResults(request,"updFinancialEntity",dynaForm);
        ...
        else if(actionMapping.getPath().equals("/activateFinEnt")){
            String entityNo = request.getParameter("entityNumber");
            HashMap hmData =new HashMap();
            webTxnBean = new WebTxnBean();
            hmData.put("entityNumber",entityNo);
        ...
        //Case 3838 - START
        String desc = (String)dynaFinEntityForm.get("entityName");
        if(desc != null && desc.trim().length() > 60) {
            messages.add("entityNameLength", new ActionMessage("error.relEntityLength"));
            saveMessages(request, messages);
            isValid = false;
        }
        desc = (String)dynaFinEntityForm.get("orgRelnDesc");
        if(desc != null && desc.trim().length() > 2000) {
            messages.add("orgRelnDescLength", new ActionMessage("error.DescRelation.length"));
            saveMessages(request, messages);
            isValid = false;
        }
        desc = (String)dynaFinEntityForm.get("orgRelnDesc");
        if(desc != null && desc.trim().length() == 0) {
            messages.add("orgRelnDesc", new ActionMessage("error.orgRelnDescMandatory"));
            saveMessages(request, messages);
            isValid = false;
        }
        
        desc = (String)dynaFinEntityForm.get("relationShipDesc");
        if(desc != null && desc.trim().length() > 2000) {
            messages.add("relationShipDesc", new ActionMessage("error.EntityRelDesc.length"));
            saveMessages(request, messages);
            isValid = false;
        }
        String relTypeCode = (String)dynaFinEntityForm.get("entityRelTypeCode");
        if(relTypeCode != null && (relTypeCode.trim().equals("4") || relTypeCode.trim().equals("5")) && desc.trim().length() == 0) {
            messages.add("relationShipDescMandatory", new ActionMessage("error.relationShipDescMandatory"));
            saveMessages(request, messages);
            isValid = false;
        }
        //refill Create User and CreateTimestamp
        String sequenceNum, entityNumber;
        sequenceNum = (String)dynaFinEntityForm.get("sequenceNum");
        entityNumber = (String)dynaFinEntityForm.get("entityNumber");
        if (sequenceNum!= null && Integer.parseInt(sequenceNum) != 1) {
                ...
                request.setAttribute("createTimestamp", dynaFinEntityForm.get("updtimestamp"));
                request.setAttribute("createUser", dynaFinEntityForm.get("updtimestamp"));
            }
        //Case 3838 - END
        if(isValid) {
    ...
    public void addUpdFinEntity(HttpServletRequest request,
    ...
    DynaValidatorForm dynaValidatorForm) throws Exception{
        ...
        orginalDesc =(String)formData.get("orgRelnDesc");
        relnDesc = (String)dynaValidatorForm.get("orgRelnDesc");
        orginalDesc = (orginalDesc == null ) ? EMPTY_STRING : orginalDesc.trim();
        relnDesc = (relnDesc == null ) ? EMPTY_STRING : relnDesc.trim();
        if(!orginalDesc.equals(relnDesc)){
            isEntityChanged =true;
