...
import edu.mit.coeus.iacuc.bean.AreaOfResearchTxnBean;
...
// private WebTxnBean webTxnBean;
 private static final String SAVE_MENU = "AC004";
 ...
 private static final String UPDATE_USER = "areaUser";
 private static final String GET_PROTO_RESEARCH_AREAS="getIacucResearchAreas";
 private static final String SUCCESS="success";
 private static final String UPD_AREAS_OF_RESEARCH="updateIacucAreasOfResearch";
 private static final String AREA_RESEARCH="iacucAreaResearch";
    ...
    private boolean saveAreasofResearch(HttpSession session,HttpServletRequest request, 
       ...
       Timestamp dbTimestamp = prepareTimeStamp();
        ...
        if(vcArea!= null && vcArea.size() > 0){
           ...
           session.setAttribute("iacucAreaData", vcArea);
           isSuccess=true;
           return isSuccess;
       }
        ...
        vcArea = (vcArea == null)? new Vector() : vcArea;
        if(isGenerateSequence(request) ){
            ...
            updateRecordsToNewSequence(vcArea, AREA_TIME_STAMP, UPDATE_USER, UPD_AREAS_OF_RESEARCH, request);
       ...
       // updateSaveStatusToSession(session,isMenuSaved);
        session.setAttribute("iacucAreaData", htAreaData.get(GET_PROTO_RESEARCH_AREAS));
        ...
        if(vcAreaData!=null && vcAreaData.size()>1){
            for(int i=0;i<vcAreaData.size();i++){
                ...
                if(rACode.equals(code)){
                    ...
                    //added for indicator logic begin
                    vcAreaData.remove(i);
                     //Added for sequence number logic start
                  
                    boolean isSequenceUpdated = false;
                    if(isGenerateSequence(request) ){
                        //Modified for COEUSQA-3064 Unable to save funding source type in Lite if it is other than base types supplied 
                        isSequenceUpdated = updateRecordsToNewSequence(vcAreaData, AREA_TIME_STAMP, UPDATE_USER, UPD_AREAS_OF_RESEARCH, request);
                   ...
                   // webTxnBean.getResults(request, UPD_AREAS_OF_RESEARCH, dynaAreaForm);
                    //Added for sequence number logic end
                    ...
                    // Update the Menu status to the session.
                    // updateSaveStatusToSession(session,isMenuSaved);
                    session.setAttribute("iacucAreaData", htArea.get(GET_PROTO_RESEARCH_AREAS));
                    break;
