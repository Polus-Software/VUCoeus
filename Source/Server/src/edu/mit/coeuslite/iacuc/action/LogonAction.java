...
import edu.mit.coeuslite.iacuc.form.MyLogonForm;
...
public class LogonAction extends ValidateUserAction{
        ...
        try{
            if(url != null){
                ...
                if(loginMode.equals("USERID")) {
                    ActionForward forward = new ActionForward("/coeuslite/mit/iacuc/cwLogon.jsp");
                    return forward;
                }else {
    ...
    private void getHeaderDetails(HttpServletRequest request) {
        ...
        if (headerVector==null || headerVector.size()==0) {
            headerVector = readProtocolDetails.readXMLDataForMainHeader("/edu/mit/coeuslite/iacuc/xml/MainMenu.xml");
