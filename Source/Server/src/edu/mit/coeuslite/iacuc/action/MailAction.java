...
public class MailAction extends CoeusBaseAction implements ModuleConstants{
    ...
    private static final String PROTOCOL_ACTIONS = "showIacucAction";
    private static final String PROTOCOL_DETAILS = "getIacucDetails";
    ...
    private static final String MAIL_ACTION = "/iacucMailAction";
    ...
    private static final String PROTOCOL_FUNDING_SOURCE = "protocolFundingSource";
...
//            DynaValidatorForm mailForm = (DynaValidatorForm) actionForm;
            ...
            if(request.getParameter(IACUC_ACTION_CODE) != null) {
                actionId = (String) request.getParameter(IACUC_ACTION_CODE);
            }
            ...
            if(moduleCode != null && actionId.equals("0")) {
                ...
                session.setAttribute(IACUC_ACTION_CODE, actionId);
            ...
            if(moduleCode == null || actionId == null) {
                ...
                actionId = (String) session.getAttribute(IACUC_ACTION_CODE);
                if(moduleCode == null || actionId == null) {
...
//            if(validAction != null && validAction.equals("false")) {
            ...
            if(hmContent != null && hmContent.size() > 0) {
                ...
                forward = forwardAction(actionId, moduleCode, request, response);
                return actionMapping.findForward(forward);
            }
            //COEUSDEV-75:End
...
//          ex.printStackTrace();
    ...
    private HashMap getIRBInfo(HttpServletRequest request) throws Exception {
        ...
        if(session.getAttribute(CoeusLiteConstants.IACUC_PROTOCOL_NUMBER+session.getId())!=null &&
            ...
            moduleItem = (String)session.getAttribute(CoeusLiteConstants.IACUC_PROTOCOL_NUMBER+session.getId());
...
//    }
     ...
     * @return String
     */
    private String forwardAction(String actionId, String moduleCode, HttpServletRequest request, HttpServletResponse response) throws java.io.IOException {
        String forward = null;
        if(moduleCode != null) {
...
//                forward = "devproposal";
            ...
            } else if(moduleCode.equals(String.valueOf(IACUC_MODULE_CODE))) {
                ...
                HttpSession session = request.getSession();
                String submitted = (String) session.getAttribute(IACUC_ACTION_CODE);
                ...
                // Case# 3783: No 'Do you want to save' When creating a new protocol and moving from General Info tab to Org -End
                    if(submitted != null && submitted.equals("101")){
                        forward = "submitted";
                    } else if(actionId.equalsIgnoreCase("552")||actionId.equalsIgnoreCase("553")){
                      forward = PROTOCOL_FUNDING_SOURCE;
...
//    }
    ...
    private HashMap getModuleData(String moduleCode, HttpServletRequest request) throws Exception {
        ...
        switch(module) {
            case IACUC_MODULE_CODE: //for IRB module
...
//                Vector iacucmenuItemsVector = readXMLData.readXMLDataForMenu("/edu/mit/coeuslite/iacuc/xml/IacucMenu.xml");
//                session.setAttribute(CoeusliteMenuItems.IACUC_MENU_ITEMS, iacucmenuItemsVector);
                session.removeAttribute(CoeusliteMenuItems.IACUC_MENU_ITEMS);
                ...
                mapMenuList.put("menuCode",CoeusliteMenuItems.IACUC_PROTOCOL_SEND_MAIL_MENU);
    ...
    protected void readSavedStatus(HttpServletRequest request) throws Exception{
        ...
        if(menuData!= null && menuData.size() > 0){
            ...
            session.removeAttribute(CoeusliteMenuItems.IACUC_MENU_ITEMS);
            session.setAttribute(CoeusliteMenuItems.IACUC_MENU_ITEMS, menuData);
    ...
    public void readProtocolMenus(HttpServletRequest request) throws Exception{
        HttpSession session = request.getSession();
        ReadXMLData readXMLData = new ReadXMLData();
        Vector menuItemsVector = (Vector) session.getAttribute(CoeusliteMenuItems.IACUC_MENU_ITEMS);
        HashMap hmData = null ;
        if ( menuItemsVector == null || menuItemsVector.size()==0) {
            ...
            session.setAttribute(CoeusliteMenuItems.IACUC_MENU_ITEMS, menuItemsVector);
        }else{
