...
//    private Timestamp dbTimestamp;
    ...
    private static final String SAVE_MENU = "AC007";
    ...
    private static final String GET_PROTOCOL_SPECIAL_REVIEW = "getIacucSpecialReview";
    ...
    private static final String GET_SPECIAL_REVIEW_NMUBER = "getIacucNextSpecialReviewNumber";
    private static final String UPDATE_MENU_CHECKlIST = "updateMenuCheckList";
    private static final String UPDATE_SPECIAL_REVIEW = "updIacucSpecialReview";
...
//    private ActionMessages messages ;
    ...
    public ActionForward performExecute(ActionMapping mapping, 
        ...
        //COEUSQA-1724-Added for Expiration date column- start
        date=specialReviewForm.get(EXPIRATION_DATE).toString();
        date = dateUtils.formatDate(date,":/.,|-","MM/dd/yyyy");
        specialReviewForm.set(EXPIRATION_DATE,date);
        //COEUSQA-1724-Added for Expiration date column- end
        
        ActionMessages messages = new ActionMessages();
        //start--2
        protocolNumber = 
            (String)session.getAttribute(CoeusLiteConstants.IACUC_PROTOCOL_NUMBER+session.getId());
        seqNum = 
        ...
        Boolean fundingSourceMode = (Boolean) session.getAttribute("MODIFY_IACUC_SPECIAL_RVW"+session.getId());
    ...
    private boolean saveSpecialReview(HttpServletRequest request, 
        ...
        }else if(specialReviewForm.get(AC_TYPE).toString().equalsIgnoreCase(AC_TYPE_UPDATE)){
            // Update the form
            String  formSeqNumber = (String)specialReviewForm.get(SEQUENCE_NUMBER);
            if(isGenerateSequence(request) && !seqNum.equals(formSeqNumber)){
                specialReviewForm.set(AC_TYPE, TypeConstants.INSERT_RECORD);
             
            }
            
            for(int index=0; index<vecSplRev.size(); index++){
                DynaValidatorForm form = (DynaValidatorForm) vecSplRev.get(index);
                if(form.get("protocolNumber").equals(protocolNumber) &&
                        form.get("specialReviewNumber").equals(specialReviewForm.get("specialReviewNumber"))){
                    vecSplRev.remove(index);
                    break;
                }
            }
        }
        ...
        if(isGenerateSequence(request)) {
             setFormatedDatesToForm(vecSplRev);
             //Modified for COEUSQA-3064 Unable to save funding source type in Lite if it is other than base types supplied 
             updateRecordsToNewSequence(vecSplRev, UPDATE_TIMESTAMP, UPDATE_USER,UPDATE_SPECIAL_REVIEW, request);
        }
      //Added for indicator logic end
       ...
       txnBean.getResults(request, UPDATE_PROTOCOL_INDICATOR, hmpIndMap);
        ...
        Hashtable hsSpRev = 
            (Hashtable)txnBean.getResults(request, "iacucSpecialReview", hmpSpRev);
...
//        session.setAttribute("splReview", hsSpRev.get(GET_SPECIAL_REVIEW_CODE));
        ...
        setFormatedDatesToForm(vecData);
        //The following commented code refacted to a method setFormatedDatesToForm
...
//                String approvalDate = (String)dynaValidatorForm.get(APPROVAL_DATE);
//                //COEUSQA-1724-Added for Expiration date column-start
//                String expirationDate = (String)dynaValidatorForm.get(EXPIRATION_DATE);
//                //COEUSQA-1724-Added for Expiration date column-end
//                if(applDate != null){
...
//                    dynaValidatorForm.set(EXPIRATION_DATE,expirationDateValue);
//
//                }else{
//                    dynaValidatorForm.set(EXPIRATION_DATE,"");
//                }
//                //COEUSQA-1724-Added for Expiration date column-end
//            }
//        }
...
//        txnBean.getResults(request, UPDATE_SPECIAL_REVIEW, specialReviewForm);       
        Hashtable hsSpRev = (Hashtable)txnBean.getResults(request, "iacucSpecialReview", hmpSpRev);        
        ...
        for(int index=0; index<formData.size(); index++){
            DynaValidatorForm form = (DynaValidatorForm) formData.get(index);
            if(form.get("protocolNumber").equals(protocolNumber) &&
                    form.get("specialReviewNumber").equals(specialReviewForm.get("specialReviewNumber"))){
                // Added with keerthy - start
                form.set(AC_TYPE,AC_TYPE_DELETE);
                formData.remove(index);
                
                boolean isSequenceUpdated = false;
                if(isGenerateSequence(request)) {
                    setFormatedDatesToForm(formData);
                    //Modified for COEUSQA-3064 Unable to save funding source type in Lite if it is other than base types supplied 
                    isSequenceUpdated = updateRecordsToNewSequence(formData, UPDATE_TIMESTAMP, UPDATE_USER, UPDATE_SPECIAL_REVIEW, request);
                }
       ...
       //txnBean.getResults(request, UPDATE_MENU_CHECKlIST, hmSaveStatus);
        ...
        session.setAttribute("iacucSplReview", vecSpecialReviewDetails);
//        session.setAttribute("splReview", hsSpRev.get(GET_SPECIAL_REVIEW_CODE));
        //Added for Coeus 4.3 Enhancement -Ends
       //Modified for indicater logic begin
        hsSpRev = (Hashtable)txnBean.getResults(request, "iacucSpecialReview", hmpSpRev); 
        session.setAttribute("approval", hsSpRev.get(GET_REVIEW_APPROVAL_TYPE));
        Vector vecData = (Vector)hsSpRev.get(GET_PROTOCOL_SPECIAL_REVIEW);
...
//        }
        ...
        //Modified for indicater logic end
        session.setAttribute("ReviewList",vecData); 
        readSavedStatus(request);
      return isSuccess;             
   ...
   /*This method returns a HashMap which is used to update the Save Status */
    ...
    private boolean validateRowData(DynaValidatorForm dynaSpecialForm, HttpServletRequest request) throws Exception{
        ...
        //COEUSQA-1724-Added for Expiration date column- start
        String exprnDate = (String)dynaSpecialForm.get(EXPIRATION_DATE) ;
        //COEUSQA-1724-Added for Expiration date column- end
        ...
        if(stData != null){            
            ...
            if(arDateFlag != null){
             ...
             //COEUSQA-1724-Added for Expiration date column- start
            if(expDateFlag != null){
                if(expDateFlag.trim().equalsIgnoreCase("Y")) {
                    if(exprnDate == null || exprnDate.trim().length() <=0){
                        messages.add("error.specialReview.expirationDate",new ActionMessage("error.specialReview.expirationDate"));
                        saveMessages(request, messages);
                        isErrorPresent = true ;
                    }
                }
    ...
    private String[] formValidateString(HttpServletRequest request) throws Exception{
        ...
        if(vecValidateRules != null){
            ...
            String expirationDateFlag = null;
            int approvalCode = -1;
            ...
            for(int index = 0; index < size ; index++){
                ...
                if(dynaValidForm != null){                    
                    ...
                    expirationDateFlag = (String)dynaValidForm.get("expirationDateFlag");
                                      
                    validate = ""+specialReviewCode + approvalCode + protocolFlag 
                                    + applicationDateFlag + approvalDateFlag + expirationDateFlag;
    ...
    private DynaValidatorForm resetFormDatas(DynaValidatorForm specialReviewForm)throws Exception {
        ...
        specialReviewForm.set(EXPIRATION_DATE, "");
    ...
    private DynaValidatorForm editSpecialReview(HttpServletRequest request,
        ...
        if(vecSpecialReview!=null && vecSpecialReview.size()>0){
            for(int index=0; index<vecSpecialReview.size(); index++){
                ...
                if(form.get("protocolNumber").equals(protocolNumber) && 
                   // form.get("sequenceNumber").equals(sequesnceNumber) && //Commented for indicator logic
                    form.get("specialReviewNumber").equals(dynaForm.get("specialReviewNumber"))){
                    //Added for indicator logic
                    dynaForm.set("sequenceNumber", form.get("sequenceNumber"));
                    ...
                    dynaForm.set("applicationDate", form.get("applicationDate"));
                            //COEUSQA-1724-Added for Expiration date column- start
                    dynaForm.set(EXPIRATION_DATE, form.get(EXPIRATION_DATE));
                            //COEUSQA-1724-Added for Expiration date column- end
    ...
    /**
     * This method will format the date files int the form and return  the list of forms
     * with formated date.
     * @param specialReviewList Vector
     */
    
    private void setFormatedDatesToForm(Vector specialReviewList){
        DateUtils dateUtils = new DateUtils();        
        DynaValidatorForm dynaValidatorForm = null;
        
        if(specialReviewList != null){
            for(Object obj : specialReviewList){
                dynaValidatorForm = (DynaValidatorForm)obj;
