...
import edu.mit.coeus.iacuc.bean.ProtocolRoleInfoBean;
 ...
 * @author  nadhgj
 */
public class IacucGeneralInfoAction extends ProtocolBaseAction{
...
//    private Timestamp dbTimestamp;
    private static final String NO =  CoeusConstants.INDICATOR_NOT_PRESENT_NOT_MODIFIED;
    //Removing instance variable case# 2960    
...
//    private String organizationId;
    ...
    private static final String DEFAULT_IACUC_ORGANIZATION_ID = "DEFAULT_IACUC_ORGANIZATION_ID";
    private static final String DEFAULT_IACUC_ORGANIZATION_TYPE = "DEFAULT_IACUC_ORGANIZATION_TYPE";
    ...
    private static final String UPD_AREAS_OF_RESEARCH="updateIacucAreasOfResearch";
    private static final String ALL_RESEARCH_AREAS = "000001";
    // Code added for 4.3 Enhancement - ends
     
    /**
     * Creates a new instance of IacucGeneralInfoAction
     */
    public IacucGeneralInfoAction() {
    }
     public ActionForward performExecute(ActionMapping mapping,
        ...
        DynaValidatorForm iacucGeneralInfoForm = (DynaValidatorForm)form;
        //Added for Coeus4.3 header changes enhacement - Start 
        session.removeAttribute("newIacucFlag");
        if (iacucGeneralInfoForm.get("approvalDate") == null || iacucGeneralInfoForm.get("approvalDate").equals(EMPTY_STRING)){// modified for fixing approval date format changing bug 
        ...
        String date=iacucGeneralInfoForm.get("applicationDate").toString();
        //Modified for instance variable case#2960.
        DateUtils dateUtils = new DateUtils();        
        date = dateUtils.formatDate(date,":/.,|-","MM/dd/yyyy");
        iacucGeneralInfoForm.set("applicationDate",date);
        //Modified for instance variable case#2960.
//        organizationId = (String)iacucGeneralInfoForm.get("organizationId");
        
        //Commented For PMD Check
        //String organizationId = (String)iacucGeneralInfoForm.get("organizationId");
        iacucGeneralInfoForm.set("isBillable", request.getParameter("isBillable"));
        ...
        */
//        InitialiseIndicators(iacucGeneralInfoForm,session);
        ...
        if(mode.trim().equals(CoeusLiteConstants.ADD_MODE) &&type!= null){
           //String acType =  (String)iacucGeneralInfoForm.get("acType");
           //if(acType== null || acType.trim().equals("")){
            iacucGeneralInfoForm.set("acType","I");
            InitialiseIndicators(iacucGeneralInfoForm,session);
           //}
        }else if(mode.trim().equals(CoeusLiteConstants.MODIFY_MODE)){
            iacucGeneralInfoForm.set("acType","U");
        }
        WebTxnBean txnBean= new WebTxnBean();
        HashMap hmpVulData = new HashMap();
        
        int seqNum = -1;
        String protocolNum = (String)session.getAttribute(CoeusLiteConstants.IACUC_PROTOCOL_NUMBER+session.getId());
        String sequenceNumber = (String)session.getAttribute(CoeusLiteConstants.IACUC_SEQUENCE_NUMBER+session.getId());
        String versionNumber = (String)session.getAttribute(CoeusLiteConstants.IACUC_VERSION_NUMBER+session.getId());
        ...
        iacucGeneralInfoForm.set("protocolNumber",protocolNum);
        iacucGeneralInfoForm.set("sequenceNumber",sequenceNumber);
        iacucGeneralInfoForm.set("versionNumber",versionNumber);
            ...
            if(!isLockExists && lockBean.getUnitNumber().equals(lockData.getUnitNumber()) || protocolNum == null) {             
                if(actionPerformed != null && actionPerformed.equalsIgnoreCase("organization")){
                    ...
                    if(iacucGeneralInfoForm.get("orgAcType").toString().equalsIgnoreCase("U")){
                        CoeusFunctions coeusFunctions = new CoeusFunctions();
                        Timestamp dbTimestamp = coeusFunctions.getDBTimestamp();
                        iacucGeneralInfoForm.set("locationUpdateTimestamp",dbTimestamp.toString());
                        
                        Hashtable hsTable = (Hashtable)txnBean.getResults(request, "getIacucList", hmpData);
                        Vector vecOrganizations=(Vector)hsTable.get("getIacucList");
                        boolean isPresent = false ;
                        if(vecOrganizations!= null && vecOrganizations.size() > 0){                            
                           isPresent = isUpdateOrganizations(vecOrganizations,iacucGeneralInfoForm);
                        }
                        if(isPresent){
                            txnBean.getResults(request, "updIacucOrganizations", iacucGeneralInfoForm);
                        }
                        if(!isOrgIdPresent(vecOrganizations , iacucGeneralInfoForm)){
                           iacucGeneralInfoForm.set("orgAcType" , "I");
                           txnBean.getResults(request, "updIacucOrganizations", iacucGeneralInfoForm); 
                        }
                        
                        Hashtable hTableData = (Hashtable)txnBean.getResults(request, "getIacucList", hmpData);
                        Vector cvOrg=(Vector)hTableData.get("getIacucList");
                        request.getSession().setAttribute("organizations", cvOrg);
                        
                    }else if(iacucGeneralInfoForm.get("orgAcType").toString().equalsIgnoreCase("D")){
                        Hashtable hsData = (Hashtable)txnBean.getResults(request, "updIacucOrganizations", iacucGeneralInfoForm);
                        Hashtable hTableData = (Hashtable)txnBean.getResults(request, "getIacucList", hmpData);
                        Vector cvOrg=(Vector)hTableData.get("getIacucList");
                        request.getSession().setAttribute("organizations", cvOrg);
                    ...
                    if(iacucGeneralInfoForm.get("acType").toString().equalsIgnoreCase("U")){
			//Modified  for case id COEUSQA-2868/COEUSQA-2869 begin
                        ...
                        if(newSeq!=null && newSeq.equals("incremented")){
                            iacucGeneralInfoForm.set("acType", "I");
                            session.setAttribute("iacucNewSeq"+session.getId(), EMPTY_STRING);
                        }
                        Timestamp dbTimestamp = prepareTimeStamp(iacucGeneralInfoForm);
                        iacucGeneralInfoForm.set("newUpdateTimestamp",dbTimestamp.toString());
                        
                        //Modified  for case id COEUSQA-2868/COEUSQA-2869 end
//                        iacucGeneralInfoForm.set("protocolStatusCode",request.getParameter("statusCode"));
                        // if value of "statusCode" from request is null, then use the value of "protocolStatusCode" from session
                        if (request.getParameter("statusCode") != null && !request.getParameter("statusCode").equals("null")){
                            iacucGeneralInfoForm.set("protocolStatusCode",request.getParameter("statusCode"));
                        }else{
                            if(session.getAttribute("protocolStatusCode") != null){
                                iacucGeneralInfoForm.set("protocolStatusCode",session.getAttribute("protocolStatusCode"));
                            }
                        }
                        ...
                        String formStatusCode = iacucGeneralInfoForm.get("protocolStatusCode").toString();
                        if(!headerStatusCode.equals(formStatusCode)){       
                            //Modified for internal issue fix 1889 start
                            if(session.getAttribute("protocolStatusCode") != null){
                                iacucGeneralInfoForm.set("protocolStatusCode", session.getAttribute("protocolStatusCode"));
                            }else{
                                iacucGeneralInfoForm.set("protocolStatusCode",headerStatusCode);
                            }
                           //Modified for internal issue fix 1889 end 
                        }
                        ...
                        if(protoHeaderBean.getExpirationDate() != null){
                         ...
                         String value = dateUtils.formatDate(expirDate,edu.mit.coeuslite.utils.DateUtils.MM_DD_YYYY);
                          iacucGeneralInfoForm.set("expirationDate",value);
                        }else{
                                iacucGeneralInfoForm.set("expirationDate","");
                            }
                        // end                         
                        Hashtable hsData = (Hashtable)txnBean.getResults(request, "updIacuc", iacucGeneralInfoForm);
                        Hashtable hTableData = (Hashtable)txnBean.getResults(request, "generalIacucInfo", hmpData);
                        Vector protoTypes =(Vector)hTableData.get("getIacucTypes");
                ...
                //        session.setAttribute("ProtocolTypes", protoTypes);// commented for case#3016
                        ...
                        //HashMap hmSaveMap = updateSaveStatus(iacucGeneralInfoForm,session,protocolNum);
                        //Hashtable htMenu = (Hashtable)txnBean.getResults(request, "updateMenuCheckList", hmSaveMap);
                        //updateSaveStatusToSession(session); 
                        
                       // session.setAttribute("IacucTypes", protoTypes);
                        Vector protoStatus =(Vector)hTableData.get("getIacucStatus");
                        ...
                        request.getSession().setAttribute("organizations", cvOrg);
                        Vector cvProtoData=(Vector)hTableData.get("getIacucInfo");
                      if(cvProtoData!= null && cvProtoData.size() >0){
                            ...
                            session.setAttribute("IacucTypes", vecFinalProtoTypes);
                    ...
                    }else if(iacucGeneralInfoForm.get("acType").toString().equalsIgnoreCase("I")){
                        
                        
                        //if(organizationId != null && !organizationId.equals(EMPTY_STRING)){
                            // prepare the time stamp
                            Timestamp dbTimestamp = prepareTimeStamp(iacucGeneralInfoForm);
                            iacucGeneralInfoForm.set("createTimestamp",dbTimestamp.toString());
                            iacucGeneralInfoForm.set("newUpdateTimestamp",dbTimestamp.toString());
                            Hashtable htProtocolNumber = (Hashtable)txnBean.getResults(request, "generateIacucNumber", null);
                            HashMap mappedData = (HashMap)htProtocolNumber.get("generateIacucNumber");
                            String newProtocolNumber = (String)mappedData.get("protocolNumber");
                            HashMap hmSaveMap = updateSaveStatus(iacucGeneralInfoForm,request,newProtocolNumber);

                            iacucGeneralInfoForm.set("protocolNumber",newProtocolNumber);
                            iacucGeneralInfoForm.set("sequenceNumber","1");
                            iacucGeneralInfoForm.set("versionNumber","1");
                            iacucGeneralInfoForm.set("protocolStatusCode","100");
                            iacucGeneralInfoForm.set("createTimestamp",dbTimestamp.toString());
                            iacucGeneralInfoForm.set("createUser",userId);
                            //Code added for Case#2785 - Protocol Routing - starts
                            sequenceNumber = "1";
                            //Code added for Case#2785 - Protocol Routing - ends
                            hmpData.put("protocolNumber",iacucGeneralInfoForm.get("protocolNumber"));
                            ...
                            //iacucGeneralInfoForm.set("organizationId",session.getAttribute("organizationId"));
                            //iacucGeneralInfoForm.set("locationUpdateTimestamp",prepareTimeStamp().toString());
                            ...
                            hmDefaultOrgId.put("defaultOrgId", DEFAULT_IACUC_ORGANIZATION_ID);
                            ...
                            iacucGeneralInfoForm.set("organizationId",(String)hmDefaultOrgId.get("ls_value"));
...
//                            hmDefaultOrgType.put("defaultOrgId", DEFAULT_IACUC_ORGANIZATION_TYPE);
...
//                            iacucGeneralInfoForm.set("protocolOrgTypeCode",(String)hmDefaultOrgType.get("ls_value"));
//                            //default rolodexid
//                            iacucGeneralInfoForm.set("rolodexId",(getRolodexId((String)iacucGeneralInfoForm.get("organizationId"),request))); 
//                            //end
//
//                            iacucGeneralInfoForm.set("locationUpdateTimestamp",prepareTimeStamp().toString());
//                            iacucGeneralInfoForm.set("orgAcType",TypeConstants.INSERT_RECORD); 
//
...
//                            //added to insert organization end
                            ...
                            //Hashtable htMenu = (Hashtable)txnBean.getResults(request, "updateMenuCheckList", hmSaveMap);
                            //updateSaveStatusToSession(session); 
                            Hashtable hTable = (Hashtable)txnBean.getResults(request, "generalIacucInfo", hmpData);

                            //Update the Protocol User Roles                        
                            updateProtocolUserRoles(request,iacucGeneralInfoForm);

                            Vector protoStatus =(Vector)hTable.get("getIacucStatus");
                            session.setAttribute("IacucStatusTypes", protoStatus);
                            //Vector protoOrg =(Vector)hTable.get("getProtocolOrgTypes");
                            //session.setAttribute("OrganizationTypes", protoOrg);
                            Vector cvOrg=(Vector)hTable.get("getIacucList");
                            request.getSession().setAttribute("organizations", cvOrg);
                            Vector cvProtoData=(Vector)hTable.get("getIacucInfo");
                            ...
                            hmAreaOfResearch.put("protocolNumber", iacucGeneralInfoForm.get("protocolNumber"));
                            hmAreaOfResearch.put("sequenceNumber", iacucGeneralInfoForm.get("sequenceNumber"));
                            ...
                            //Code added for Case#2785 - Protocol Routing - starts
                            sequenceNumber = (String) iacucGeneralInfoForm.get("sequenceNumber");
                            //Code added for Case#2785 - Protocol Routing - ends
                            //Added for Coeus4.3 Correspondents Page enhancement - start - 1
                            HashMap inputData = new HashMap();
                            inputData.put("protocolNumber", iacucGeneralInfoForm.get("protocolNumber"));
                            inputData.put("sequenceNumber", iacucGeneralInfoForm.get("sequenceNumber"));
                            inputData.put("orgOrUnitId", iacucGeneralInfoForm.get("organizationId"));
                            inputData.put("investigatorFlag", new Boolean(false));
                            try{
                            updateCorrespondents(inputData, request, iacucGeneralInfoForm);        
                            ...
                            if(newProtocolNumber!=null){
                                ...
                                session.setAttribute(CoeusLiteConstants.IACUC_SEQUENCE_NUMBER+session.getId(),"1");
                            }
                            ...
                            session.setAttribute("statusCode", iacucGeneralInfoForm.get("protocolStatusCode"));
    ...
    //    }
        ...
        if(vecProtocolHeader!=null && vecProtocolHeader.size()>0){
            ...
            if(iacucGeneralInfoForm.get("acType").equals("I")){
...
//        if(iacucGeneralInfoForm.get("acType").equals("I"))
            if(iacucGeneralInfoForm.get("acType").equals("I") && iacucGeneralInfoForm.get("protocolStatusCode").toString().equals("100")) {
                //COEUSDEV-358 : End
                session.setAttribute("MODULE_CODE", ModuleConstants.IACUC_MODULE_CODE+"");  
                session.setAttribute("IACUC_ACTION_CODE", "100"); // Action Code for Protocol Creation
                //COEUSQA-1724 : Email Notification For All Actions - Enable the Email-Notification functionality in Lite 
                // Added for COEUSQA-3163 Exception on Investigator link in IACUC -start
                 readSavedStatus(request);
                // Added for COEUSQA-3163 - end 
        ...
        iacucGeneralInfoForm.reset(mapping,request);
...
//        setMenuForAmendRenew(protocolNumber, request);
        ...
        return actionForward;
            /*
            IrbGeneralInfoForm iacucGeneralInfoForm = (IrbGeneralInfoForm)form;
            ...
            try{
                ...
                if(!mode.equals("D")) {
                    iacucGeneralInfoForm.setAcType(mode.equals("A") ? "I" : "U");
                    generalTxnBean.addUpdGeneralInfo(iacucGeneralInfoForm);
                }
                if(protocolNum != null ) {
                    iacucGeneralInfoForm = generalTxnBean.getGeneralInfoDetails(protocolNum);
                    orgList = generalTxnBean.getOrgList(protocolNum,iacucGeneralInfoForm.getSequenceNumber());
                }
                iacucGeneralInfoForm.setProtocolNumber(protocolNum);
                ...
                session.setAttribute(CoeusLiteConstants.GENERAL_INFO_FORM,iacucGeneralInfoForm);
    ...
    // Initialise the indicators in the new mode.
     private void InitialiseIndicators(DynaValidatorForm form,HttpSession session){
         ...
         if(acType.trim().equals("I")){
             ...
             form.set(CoeusLiteConstants.SCIENTIFIC_JUSTIF_INDICATOR,NO);
             form.set(CoeusLiteConstants.ALTERNATIVE_SEARCH_INDICATOR,NO);
             form.set(CoeusLiteConstants.SPECIES_STUDY_GROUP_INDICATOR,NO);
            //session.setAttribute(CoeusLiteConstants.PROTOCOL_INDICATORS,indicatorMap);
             ...
             String scientificIndicator = (String)indicatorMap.get(CoeusLiteConstants.SCIENTIFIC_JUSTIF_INDICATOR);
             String alterSearchIndicator = (String)indicatorMap.get(CoeusLiteConstants.ALTERNATIVE_SEARCH_INDICATOR);
             String speciesStudyGroupIndicator = (String)indicatorMap.get(CoeusLiteConstants.SPECIES_STUDY_GROUP_INDICATOR);
             ...
             if(CoeusConstants.INDICATOR_NOT_PRESENT_MODIFIED.equals(scientificIndicator)){
                 scientificIndicator =  CoeusConstants.INDICATOR_NOT_PRESENT_NOT_MODIFIED;
             }else if(CoeusConstants.INDICATOR_PRESENT_MODIFIED.equals(scientificIndicator)){
                 scientificIndicator = CoeusConstants.INDICATOR_PRESENT_NOT_MODIFIED;
             }
             
             if(CoeusConstants.INDICATOR_NOT_PRESENT_MODIFIED.equals(speciesStudyGroupIndicator)){
                 speciesStudyGroupIndicator =  CoeusConstants.INDICATOR_NOT_PRESENT_NOT_MODIFIED;
             }else if(CoeusConstants.INDICATOR_PRESENT_MODIFIED.equals(speciesStudyGroupIndicator)){
                 speciesStudyGroupIndicator = CoeusConstants.INDICATOR_PRESENT_NOT_MODIFIED;
             }
             
             if(CoeusConstants.INDICATOR_NOT_PRESENT_MODIFIED.equals(alterSearchIndicator)){
                 alterSearchIndicator =  CoeusConstants.INDICATOR_NOT_PRESENT_NOT_MODIFIED;
             }else if(CoeusConstants.INDICATOR_PRESENT_MODIFIED.equals(alterSearchIndicator)){
                 alterSearchIndicator = CoeusConstants.INDICATOR_PRESENT_NOT_MODIFIED;
             }
             ...
             form.set(CoeusLiteConstants.SCIENTIFIC_JUSTIF_INDICATOR,scientificIndicator);
             form.set(CoeusLiteConstants.ALTERNATIVE_SEARCH_INDICATOR,alterSearchIndicator);
              form.set(CoeusLiteConstants.SPECIES_STUDY_GROUP_INDICATOR,speciesStudyGroupIndicator);
     ...
     private void updateIndicators(Vector protoData,HttpSession session){
        ...
        if(protoData!= null && protoData.size() >0){
            ...
            String scientific = (String)dynaValidatorForm.get(CoeusLiteConstants.SCIENTIFIC_JUSTIF_INDICATOR);
            String alterSearch = (String)dynaValidatorForm.get(CoeusLiteConstants.ALTERNATIVE_SEARCH_INDICATOR);
...
//       return isPresent;
    ...
    private void updateProtocolUserRoles(HttpServletRequest request, DynaValidatorForm dynaForm)throws Exception{
        ...
        hmUserRoles.put("roleId", new Integer(ProtocolRoleInfoBean.AGGREGATOR_ROLE_ID));
        hmUserRoles.put("updateTimestamp",dbTimestamp.toString());
        hmUserRoles.put("acType",dynaForm.get("acType"));
        webTxnBean.getResults(request, "updateIacucUserRoles", hmUserRoles);        
    } 
