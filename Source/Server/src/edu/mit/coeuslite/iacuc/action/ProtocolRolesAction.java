...
public class ProtocolRolesAction extends ProtocolBaseAction {
    ...
    private static final String GET_PROTOCOL_ROLES_DATA = "/getIacucRoles";
    private static final String ADD_USER = "/addIacucUser";
    private static final String DELETE_USER ="/deleteIacucUser";
    private static final String GET_PROTOCOL_ROLES = "getIacucRoles";
    private static final String GET_USER_FOR_PROTOCOL_ROLE = "getUserForIacucRole";
    private static final String GET_USER_INFO = "getUserInfo";
    private static final String UPDATE_PROTOCOL_USER_ROLES = "updateIacucUserRoles";
    // Added for COEUSQA-2692_Allow an investigator to abandon an IRB protocol that has never been approved _start
    private static final int PROTOCOL_ABANDON_STATUS = 309;
    ...
    HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        session.setAttribute("iacucRolesMode"+session.getId(), EMPTY_STRING);
        ...
        int statusCode = 0;
        if(protocolHeaderDetailsBean != null){
            statusCode = protocolHeaderDetailsBean.getProtocolStatusCode();
        }
        ...
        session.setAttribute("iacucRolesMode"+session.getId(),CoeusLiteConstants.MODIFY_MODE);
        ...
        mapMenuList.put(MENU_CODE,CoeusliteMenuItems.IACUC_PROTOCOL_ROLES_MENU);
...
//            navigator = getProtocolRolesDetails(request,isError);
    ...
    private String deleteProtocolRolesUsers(HttpServletRequest request, boolean isError)throws Exception{
        ...
        TreeMap tmProtocolData = (TreeMap)session.getAttribute("iacucRolesDetails");
        HashMap hmRoleNameId = (HashMap)session.getAttribute("iacucRoleNameId");
        ...
        session.setAttribute("iacucRolesDetails", tmProtocolData);
        return navigator;
    }
    ...
    private String addProtocolRolesUsers(HttpServletRequest request,boolean isError)throws Exception{
        ...
        FormBeanConfig formConfig = moduleConfig.findFormBeanConfig("iacucRolesForm");
...
//                ProtocolDataTxnBean protocolDatsTxnBean = new ProtocolDataTxnBean();
                ...
                session.setAttribute("iacucRolesDetails", tmProtocolData);
                session.setAttribute("iacucRoleNameId", hmRoleNameId);
...
//    
    ...
    private boolean checkMaintainProtocolRights(HttpServletRequest request)throws Exception{
        ...
        String MAINTAIN_IACUC_PROTOCOL_ACCESS = "MAINTAIN_IACUC_PROTOCOL_ACCESS";
        String MAINTAIN_ANY_IACUC_PROT_ACCESS = "MAINTAIN_ANY_IACUC_PROT_ACCESS";
        HttpSession session = request.getSession();
        String protocolNumber = (String)session.getAttribute(CoeusLiteConstants.IACUC_PROTOCOL_NUMBER+session.getId());
        UserInfoBean userInfoBean = (UserInfoBean)session.getAttribute(USER+session.getId());
        ProtocolHeaderDetailsBean protocolHeaderDetailsBean = (ProtocolHeaderDetailsBean)session.getAttribute("iacucHeaderBean");
        ...
        canMaintain =  txnData.getUserHasIACUCProtocolRight(loggedinUser, MAINTAIN_IACUC_PROTOCOL_ACCESS, protocolNumber);
        //If no rights check at Unit level right
        if(unitNumber !=null && !canMaintain){
            canMaintain = txnData.getUserHasRight(loggedinUser, MAINTAIN_ANY_IACUC_PROT_ACCESS, unitNumber);
        }
        return canMaintain;
    }
        ...
        if(isLockExists || !lockBean.getSessionId().equals(lockData.getSessionId())) {
            if(mode!=null && mode.equalsIgnoreCase(CoeusLiteConstants.DISPLAY_MODE)){
                ...
                if(isLockExists){
                    ...
                    String lockId = CoeusLiteConstants.IACUC_PROTO_LOCK_STR+protocolNumber;
...
//                        serverLockedBean.getModuleNumber()));
    ...
    private Vector getUsersForAllProtocolRoles(HttpServletRequest request, String protocolNumber, String sequenceNumber) throws  CoeusException, DBException, Exception {
        ...
        hmProtocolDetails = (Hashtable)webTxnBean.getResults(request, "getAllUserForIacucRole" ,hmProtocolDetails);
        if(hmProtocolDetails !=null && hmProtocolDetails.size()>0){
            vecAllUsersForProto = (Vector) hmProtocolDetails.get("getAllUserForIacucRole");
        }
    ...
    private void copyProtocolRolesUsersToCurrentSequence(HttpServletRequest request) throws  CoeusException, DBException, Exception{
        ...
        String strLatestSeqNumber = (String)session.getAttribute(CoeusLiteConstants.IACUC_SEQUENCE_NUMBER+session.getId());
