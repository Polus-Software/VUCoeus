 ...
 *
 */
public class UserDetailsBeanCoiV2 {

    private DBEngineImpl dbEngine;

    /** Creates new UserDetailsBean */
    public UserDetailsBeanCoiV2() {
...
//        String personId = getPersonID(userId);
    ...
    public UserInfoBean getUserInfo(String userId)
        ...
        if (result != null && !result.isEmpty()) {
            ...
            userInfo.setNonEmployee(personRow.get(
                    ...
                    : (personRow.get("NON_MIT_PERSON_FLAG").toString().equalsIgnoreCase("y") ? true : false));
    ...
    public Vector getUnitsForUser(String userId, String rightId)
        ...
        if (listSize > 0) {
            ...
            for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
                ...
                comboBoxBean.setCode((String) rowPerson.get("UNIT_NUMBER"));
                comboBoxBean.setDescription((String) rowPerson.get("UNIT_NAME"));
     ...
     *  @exception DBException
     */
    public int getCOIV2Privilege(String userId)
            throws DBException, CoeusException, org.okip.service.shared.api.Exception {
        int privilege = 0;
        final String VIEW_RIGHT_STR = "VIEW_CONFLICT_OF_INTEREST";
        final String EDIT_RIGHT_STR = "MAINTAIN_CONFLICT_OF_INTEREST";
        final String MAINTAIN_COI_STR = "MAINNTAI_COI";
        final String MAINTAIN_DISCLOSURE_ACCESS_STR = "MAINTAIN_DISCLOSURE_ACCESS";
        final String VIEW_COI_STR = "VIEW_COI";
        final String VIEW_COI_ATTACHMENTS_STR = "VIEW_COI_ATTACHMENTS";
        final String VIEW_COI_NOTES_STR = "VIEW_COI_NOTES";

        final String MAINTAIN_COI_ATTACHMENTS_STR = "MAINTAIN_COI_ATTACHMENTS";
        final String MAINTAIN_COI_NOTES_STR = "MAINTAIN_COI_NOTES";
    ...
    public int getCoiDisclRightCoiv2(String userID) throws DBException, CoeusException, org.okip.service.shared.api.Exception {
        boolean isAdmin = isAdmin(userID);
        if (isAdmin) {
            return 2;
        } else if (hasOSPRight(userID, CoiConstants.MAINTAIN_COI_DISCL_STR)) {
            return 2;
        } else if (hasOSPRight(userID, CoiConstants.VIEW_COI_DISCL_STR)) {
           // return 1; commented since the view right is not all needed
           return 2;
        } else {
            return 0;
        }
    ...
    public int getCoiDisclAttachmentCoiv2(String userID) throws DBException, CoeusException, org.okip.service.shared.api.Exception {
        boolean isAdmin = isAdmin(userID);
        if (isAdmin) {
            return 2;
        } else if (hasOSPRight(userID, CoiConstants.MAINTAIN_COI_DISCL_ATTACHMENTS_STR)) {
            return 2;
        } else if (hasOSPRight(userID, CoiConstants.VIEW_COI_DISCL_ATTACHMENT_STR)) {
            return 1;
        } else {
            return 0;
        }
    ...
    public int getCoiDisclNotesCoiv2(String userID) throws DBException, CoeusException, org.okip.service.shared.api.Exception {

        boolean isAdmin = isAdmin(userID);
        if (isAdmin) {
            return 2;
        } else if (hasOSPRight(userID, CoiConstants.MAINTAIN_COI_DISCL_NOTES_STR)) {
            return 2;
        } else if (hasOSPRight(userID, CoiConstants.VIEW_COI_DISCL_NOTES_STR)) {
            return 1;
        } else {
            return 0;
        }
    ...
    public Map getRightsCoiv2(String userId) throws DBException, CoeusException, org.okip.service.shared.api.Exception {
        Map allRights = null;
        allRights = new HashMap();
        allRights.put(CoiConstants.DISCL, getCoiDisclRightCoiv2(userId));
        allRights.put(CoiConstants.ATTACHMENT, getCoiDisclAttachmentCoiv2(userId));
        allRights.put(CoiConstants.NOTES, getCoiDisclNotesCoiv2(userId));
        return allRights;
    }

    public boolean isAdmin(String userID) throws DBException, CoeusException, org.okip.service.shared.api.Exception {
        boolean state = false;
        state = hasOSPRight(userID, "COI_ADMIN");
        return state;
    }

    public boolean isViewer(String userID) throws DBException, CoeusException, org.okip.service.shared.api.Exception {
        boolean state = false;
        state = hasOSPRight(userID, "COI_VIEWER");
        return state;
    }
...
//        hasApproveProposalRight = hasOSPRight(userId, APPROVE_PROPOSAL);
        //Case#3587 - End
        UserMaintDataTxnBean userMaintTxn = new UserMaintDataTxnBean();
        hasViewPendingDiscRight = userMaintTxn.getUserHasRightInAnyUnit(userId, VIEW_ALL_PENDING_DISCLOSURES);
