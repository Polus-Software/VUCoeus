 ...
 *
 * @author Sony
 * To fetch the list of all protocols for the corresponding logged in user
 * This would further be used for Creating a Protocol Based Disclosure
 */
public class ProtocolBasedCoiDisclAction extends COIBaseAction {
    private static String PROJECT_TYPE ="Protocol";
    ...
    public ActionForward performExecuteCOI(ActionMapping actionMapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        coiInfoBean.setModuleCode(ModuleConstants.PROTOCOL_MODULE_CODE);
        coiInfoBean.setEventType(ModuleConstants.COI_EVENT_PROTOCOL);
        coiInfoBean.setMenuType("NewIRBProtocol");
    ...
    private Vector getAllProtocolList(HttpServletRequest request,String disclosureNumber) throws Exception {
        ...
        if (protocolDet != null && protocolDet.size() > 0) {
            ...
            request.getSession().setAttribute("protocolListForAttachment", protocolDet);
        }
