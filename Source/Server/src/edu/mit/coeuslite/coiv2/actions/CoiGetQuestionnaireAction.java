 ...
 * @author Lijo Joy
 */
public class CoiGetQuestionnaireAction extends COIBaseAction {

    private static final String GET_QUESTIONNAIRE_DATA = "/getCoiQuestionnaireCoiv2";
    private static final String SAVE_QUESTIONNAIRE_DATA = "/saveCoiQuestionnaireCoiv2";
...
//                seqNum = Integer.parseInt(seq);
                ...
                seqNumApplicable = (Integer)session.getAttribute("currentSequence");
                session.setAttribute("DisclNumber", moduleKey);
                setApprovedDisclosureDetails(moduleKey,seqNumApplicable,personId,request);
                String StrSeqNumber =null;
                if(seqNum!=null){
                StrSeqNumber = seqNum.toString();
                }
                    questionnaireModuleObject.setModuleSubItemKey(StrSeqNumber);
                questionnaireModuleObject.setApplicableModuleSubItemKey(seqNumApplicable);
            }
       ...
       // String check=(String) session.getAttribute("extcodeModuleKey");
        String check=(String) session.getAttribute("qnranswerd");
      // NEW MENU CHANGE 
      ...
      // NEW MENU CHANGE       
        if (actionMapping.getPath().equals(GET_QUESTIONNAIRE_DATA)) {           
           ...
           session.setAttribute("qnranswerd", "Y");
            ...
            if(check!=null){
                session.setAttribute("getinonce", "Y");
                 String operation = "";
           ...
           // questionnaireModuleObject = (QuestionnaireAnswerHeaderBean) session.getAttribute("questionnaireModuleObject");
            ...
            if (operation.equals("SAVE") && validateAnswers(coeusDynaBeanList, request)) {
               ...
               // String qstnAnsFlag = (String)hmQuestionnaireData.get("qstnAnsFlag");
        ...
        Hashtable htanswerYes = (Hashtable) webTxnBean.getResults(request, "getQuestionAnswerYes", hmData2);
        ...
        if (ansCount>0){
              String qstnAnsFlag="true";
        request.setAttribute("qstnAnsFlag", qstnAnsFlag);
                }else{
            String qstnAnsFlag="false";
        request.setAttribute("qstnAnsFlag", qstnAnsFlag);
                }
                   // request.setAttribute("qstnAnsFlag", qstnAnsFlag);

//                    HashMap hmp=new HashMap();
//              String coiDisclosureNumber = (String) session.getAttribute("DisclNumber");
//               Integer sequenceNumber = (Integer) session.getAttribute("DisclSeqNumber");
//                 hmp.put("sequenceNumber",sequenceNumber);
//                hmp.put("coiDisclosureNumber",coiDisclosureNumber);
//                Hashtable discNotesList = (Hashtable) webTxnBean.getResults(request, "checkCoiAnswer", hmp);
//                HashMap hmCoiAnsDet = (HashMap)discNotesList.get("checkCoiAnswer");
//                String answer = (String)hmCoiAnsDet.get("ll_is_ans_no");
//  if(answer.equalsIgnoreCase("1"))
//   {
//           return actionMapping.findForward("continue");
//   }
//  else{
//           return  actionMapping.findForward("notes");
//     }
// }
...
//        } else if(check!=null){
//            actionMapping.findForward(SAVE_QUESTIONNAIRE_DATA);}
        } 
