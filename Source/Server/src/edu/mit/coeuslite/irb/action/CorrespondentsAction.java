...
//    private ActionMessages actionMessages ;    
    ...
    public void getCorrespondentsData(HttpServletRequest request)throws Exception {
        ...
        WebTxnBean webTxnBean = new WebTxnBean();
        Hashtable htCorrespondentsData = (Hashtable) webTxnBean.getResults(request, "getCorrespondentsDetails" , hmProtocolData);
        if(htCorrespondentsData!=null) {
            Vector vecCorrespondentsType = (Vector) htCorrespondentsData.get("getCorrespondentsType");
            List lstCorrespondentsData = (Vector) htCorrespondentsData.get("getCorrespondentsData");
            //Added for Case#3033-CoeusLite IRB - Correspondence screen - Start
    ...
    HttpServletRequest request)throws Exception {
        ...
        if(lstCorrespondentsData!=null && lstCorrespondentsData.size()>0) {
            for(int index=0; index<lstCorrespondentsData.size(); index++) {
                ...
                dynaForm.set(PROTOCOL_NUMBER,session.getAttribute(CoeusLiteConstants.PROTOCOL_NUMBER+session.getId()));
                dynaForm.set(SEQUENCE_NUMBER,session.getAttribute(CoeusLiteConstants.SEQUENCE_NUMBER+session.getId()));
