/*
 * PHS398ChecklistV1_3Stream.java
 *
...
import gov.grants.apply.forms.phs398_checklist_1_3_v1_3.*;
 ...
 */
 ...
 public class PHS398ChecklistV1_3Stream extends S2SBaseStream{
    private gov.grants.apply.forms.phs398_checklist_1_3_v1_3.ObjectFactory objFactory;
...
//    Attachment attachment = null;
  ...
  /** Creates a new instance of PHS398ChecklistV1_1Stream */
    public PHS398ChecklistV1_3Stream() {
        objFactory = new gov.grants.apply.forms.phs398_checklist_1_3_v1_3.ObjectFactory();
    ...
    private PHS398Checklist13Type getPHS398Checklist() throws CoeusXMLException,CoeusException,DBException{
        PHS398Checklist13Type checklistType = null;
        PHS398Checklist13Type.IncomeBudgetPeriodType incomeBudgetPeriodType = null;

        try{
           checklistType = objFactory.createPHS398Checklist13();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
             ...
             checklistType.setFormVersion("1.3");
             if (hmAppType != null)
...
/*commented for questionnaire questions modification starts
            //YNQ=22 Starts
             if (hmInfo.get("IS_CHANGE_PI") != null)
                if (hmInfo.get("IS_CHANGE_PI").toString().startsWith("Y"))
                    checklistType.setIsChangeOfPDPI("Y: " + hmInfo.get("IS_CHANGE_PI").toString());
                else checklistType.setIsChangeOfPDPI("N: " + hmInfo.get("IS_CHANGE_PI").toString());
             ...
             if (hmInfo.get("IS_INST_CHANGE") != null)
                ...
                if (hmInfo.get("IS_INST_CHANGE").toString().startsWith("Y"))
                    checklistType.setIsChangeOfInstitution("Y: " + hmInfo.get("IS_INST_CHANGE").toString());
                else checklistType.setIsChangeOfInstitution("N: " + hmInfo.get("IS_INST_CHANGE").toString());

             if (hmInfo.get("FORMER_INST_NAME") != null){
                 String fomerInstitutionName = hmInfo.get("FORMER_INST_NAME").toString();
                 if (fomerInstitutionName.length() > 60 )
                    checklistType.setFormerInstitutionName (fomerInstitutionName.substring(0, 60));
                 else checklistType.setFormerInstitutionName (fomerInstitutionName);
             }  //YNQ=23 Ends

             //YNQ=16 Starts
             if (hmInfo.get("IS_INVENTIONS") !=  null)
                  //v1.1 changed enumeration. (from v1.0 "Yes/No" to  V1.1 "Y Yes/N No")
                if (hmInfo.get("IS_INVENTIONS").toString().startsWith("Y"))
                    checklistType.setIsInventionsAndPatents("Y: " + hmInfo.get("IS_INVENTIONS").toString());
                else checklistType.setIsInventionsAndPatents("N: " + hmInfo.get("IS_INVENTIONS").toString());

            if (hmInfo.get("IS_PREV_REPORTED") != null)
                  //v1.1 changed enumeration. (from v1.0 "Yes/No" to  V1.1 "Y Yes/N No")
                if (hmInfo.get("IS_PREV_REPORTED").toString().startsWith("Y"))
                    checklistType.setIsPreviouslyReported("Y: " + hmInfo.get("IS_PREV_REPORTED").toString());
                else checklistType.setIsPreviouslyReported("N: " + hmInfo.get("IS_PREV_REPORTED").toString());
               //YNQ=16 Ends
...
//           PHS398Checklist13Type.CertificationExplanationType certAttachment = getCertExplanationType();
...
//               checklistType.setCertificationExplanation(certAttachment);
            ...
            if (hmInfo.get("PROGRAM_INCOME") != null)
                 //v1.1 changed enumeration. (from v1.0 "Yes/No" to  V1.1 "Y Yes/N No")
                if (hmInfo.get("PROGRAM_INCOME").toString().startsWith("Y"))
                    checklistType.setProgramIncome("Y: " + hmInfo.get("PROGRAM_INCOME").toString());
                else checklistType.setProgramIncome("N: " + hmInfo.get("PROGRAM_INCOME").toString());
            ...
            if (hmInfo.get("PROGRAM_INCOME").toString().equals("Yes")){
                ...
                for (int i=1; i<=numPeriods; i++){
                    ...
                    if (hmProgIncome.get("AMOUNT") != null) {
                      incomeBudgetPeriodType = objFactory.createPHS398Checklist13TypeIncomeBudgetPeriodType();
             ...
             checklistType.setDisclosurePermission(getYNQAnswer("29", cvQuestions));

        }catch (JAXBException jaxbEx){
            utilFactory.log(jaxbEx.getMessage(),jaxbEx,"PHS398ChecklistV1_3Stream","getPHS398Checklist()");
    ...
    // Yes/No questions
    private String getYNQAnswer(String questionID, Vector vecYNQ) throws CoeusException,DBException {
        String question;
        String answer = null;
    ...
    /*private ProposalDevelopmentFormBean getPropDevData() throws DBException,CoeusException{
        ...
        ProposalDevelopmentTxnBean propDevTxnBean;
  ...
  private Vector<ProposalYNQBean> getQuestionAnswers()
        throws CoeusXMLException, CoeusException, DBException, JAXBException{
...
//
//         }catch (JAXBException jaxbEx){
//            utilFactory.log(jaxbEx.getMessage(),jaxbEx,"PHS398ChecklistV1_1Stream","getCertExplanationType()");
//            throw new CoeusXMLException(jaxbEx.getMessage());
//         }
//    }
...
private gov.grants.apply.system.attachments_v1.AttachedFileDataType getAttachedFileType(Attachment attachment)
    ...
    try{
        ...
        ex.printStackTrace();
        UtilFactory.log(ex.getMessage(),ex, "PHS398ChecklistV1_1Stream", "getAttachedFileType()");
