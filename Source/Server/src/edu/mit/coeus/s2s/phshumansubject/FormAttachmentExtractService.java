...
import edu.mit.coeus.s2s.phshumansubject.bean.PHSHumanSubjectFormAttachmentBean;
...
public class FormAttachmentExtractService {
    ...
    private static final String PHS_HUMANSUBJECTS_AND_CLINICALTRIALSINFO = "PHSHumanSubjectsAndClinicalTrialsInfo";
    private static final String NAMESPACE_HUMAN_SUBJECT_V10 = "http://apply.grants.gov/forms/HumanSubjectStudy-V1.0";
    private static final String XFA_NS = "http://www.xfa.org/schema/xfa-data/1.0/";

    /**
     * @param args
     */
    public List processPdfForm(PHSHumanSubjectFormBean userAttachedFormBean) throws Exception {
        return processPdfForm(userAttachedFormBean, null);
    }

    public List processPdfForm(PHSHumanSubjectFormBean userAttachedFormBean, String schemaUrl) throws Exception {
    ...
    private Object[] unpackFile(PdfReader reader, PdfDictionary filespec) throws IOException {
        PHSHumanSubjectFormAttachmentBean userAttachedS2SFormAttachmentBean = new PHSHumanSubjectFormAttachmentBean();
    ...
    private void validateXml(PHSHumanSubjectFormBean userAttachedFormBean, String schemaUrl) throws Exception {
    ...
    private List extractAndPopulateXml(PdfReader reader, PHSHumanSubjectFormBean userAttachedFormBean, Map attachments) throws Exception {
        List formBeans = new ArrayList();
                    ...
                    if (selectedOptionalFormsCount > 0) {
                        ...
                        if (selectedFormsCount > 0) {
                            ...
                            if (!exceptions.isEmpty()) {
                                throw new S2SValidationException(exceptions.toString());
                            }
    ...
    private void addForm(List formBeans, Element form,
            PHSHumanSubjectFormBean userAttachedFormBean, Map attachments) throws Exception {
        String namespaceUri = null;
        String formname = null;
        namespaceUri = form.getNamespaceURI();
        formname = form.getLocalName();
        PHSHumanSubjectFormBean userAttachedForm = new PHSHumanSubjectFormBean();
        if (PHS_HUMANSUBJECTS_AND_CLINICALTRIALSINFO.equals(formname)) {
            Document doc = Converter.node2Dom(form);
            doc.getDocumentElement().normalize();
            doc = extractHumanSubject(doc);
            form = doc.getDocumentElement();
        }
        userAttachedForm = processForm(form, userAttachedFormBean, attachments);
        validateIfHumanSubjectForm(userAttachedForm);
        if (userAttachedForm != null) {
            formBeans.add(userAttachedForm);
        }
    }

    private PHSHumanSubjectFormBean processForm(Element form, PHSHumanSubjectFormBean userAttachedFormBean, Map attachments) throws Exception {
...
//		form.setAttribute("xmlns:glob", "\"http://apply.grants.gov/system/Global-V1.0\"");
        ...
        PHSHumanSubjectFormBean newUserAttachedFormBean = cloneUserAttachedForm(userAttachedFormBean);
    ...
    private PHSHumanSubjectFormBean cloneUserAttachedForm(
            PHSHumanSubjectFormBean userAttachedFormBean) {
        PHSHumanSubjectFormBean newUserAttachedFormBean = new PHSHumanSubjectFormBean();
        newUserAttachedFormBean.setFormNumber(userAttachedFormBean.getFormNumber());
        newUserAttachedFormBean.setAwformNumber(userAttachedFormBean.getAwformNumber());
    ...
    private void updateAttachmentNodes(Document document, PHSHumanSubjectFormBean userAttachedFormBean, Map attachments) throws Exception {
...
//            hashNode.setNodeValue(hashAlgorithm);
            ...
            PHSHumanSubjectFormAttachmentBean userAttachedFormAttachmentBean = new PHSHumanSubjectFormAttachmentBean();
            ...
            userAttachedFormAttachmentBean.setFormNumber(userAttachedFormBean.getFormNumber());
    ...
    /**
     * This will fetch the xml data from database for a form, unmarshal it to
     * JaxbObject and return it
     *
    ...
    public Object createJaxbObject(Document domDocument, String packageName) throws CoeusException, S2SValidationException,
            JAXBException {
...
//    	System.out.println(new String(userAttachedFormNodeBytes));
    ...
    public Document extractHumanSubject(Document doc) {
        try {
            if (doc == null) {
                return doc;
            }
            ...
            domParserFactory.setNamespaceAware(true);
            NodeList nodes = doc.getDocumentElement().getElementsByTagName("HumanSubjectStudy:HumanSubjectStudy");
            ...
            newXmlDocument = removeRootNode(newXmlDocument);
    ...
    private Document removeRootNode(Document document) {
        ...
        string = string.replace("</root>", "");
        document = convertStringToDocument(string);
        return document;
    }
    ...
    private void validateIfHumanSubjectForm(PHSHumanSubjectFormBean userAttachedForm) throws CoeusException {
        if(userAttachedForm != null){
         String namespace =  userAttachedForm.getNamespace();
         if(!NAMESPACE_HUMAN_SUBJECT_V10.equalsIgnoreCase(namespace)){
             throw new CoeusException("Please upload valid Human Subject form.");
         }         
