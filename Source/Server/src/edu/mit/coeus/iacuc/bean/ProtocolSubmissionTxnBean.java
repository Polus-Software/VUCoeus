...
public class ProtocolSubmissionTxnBean {
    ...
    private static final int FYI = 104;
    private static final int REQUEST_TO_LIFT_HOLD_SUB_TYPE = 103;
    private static final int REQUEST_TO_DEACTIVATE_SUB_TYPE = 102;
    ...
    public Vector getProtocolReviewerTypes() throws CoeusException,DBException{
        ...
        if(dbEngine != null){
            ...
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_PROTOCOL_REVIEWER_TYPES( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
    ...
    public Vector getProtocolSubmissionReviewType(String protocolId) throws CoeusException,DBException {
        ...
        if(dbEngine!=null){
            ...
            "{<<OUT INTEGER VALUE>>=call FN_GET_SUB_TYPE_FOR_AC_PROTO( "
                                        +"<< PROTOCOL_ID >> )}", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER VAL>>=call FN_GET_REV_TYPE_FOR_AC_PROTO( "
                                        +"<< PROTOCOL_ID >> )}", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public Vector getMatchingCommitteeForProtocol(String protocolNumber) 
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_MATCHING_COMM_FOR_AC_PROTO( <<PROTOCOL_NUMBER>> , "+
    ...
    public Vector getProtocolReviewers(String protocolNumber,int seqNumber, int submissionNumber) 
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
            "call GET_AC_PROTOCOL_REVIEWERS( <<PROTOCOL_NUMBER>>, <<SEQUENCE_NUMBER>>, <<SUBMISSION_NUMBER>>, "
            + " <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
...
//                DBEngineConstants.TYPE_STRING,protocolNumber));
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_SUB_REV_TYPES( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public Vector getProtocolVoteAbstainees(String protocolNumber,int seqNumber,
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
            "call GET_IACUC_VOTE_ABSTAINES( <<PROTOCOL_NUMBER>>, <<SEQUENCE_NUMBER>>,"
    ...
    public ProtocolSubmissionInfoBean getProtocolSubmissionDetails(
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
            "call GET_AC_PROTOCOL_SUBMISSIONS( <<PROTOCOL_NUMBER>> , "+
                            "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
...
//                                        submissionInfoBean.getSubmissionNumber()));
    ...
    public Vector getCommitteeMembersForCommittee(String committeeId, String protocolNumber)
        ...
        if(dbEngine!=null){
            result = new Vector(3,2);
            //Modified for case id COEUSQA-2772In IACUC when assigning reviewer, user can select a reviewer that is also PI start
//            result = dbEngine.executeRequest("Coeus",
//                "call GET_COMM_COMMITTEE_REVIEWERS ( <<AW_COMMITTEE_ID>> , <<AW_PROTOCOL_NUMBER>>, "
//                        +" <<OUT RESULTSET rset>>)", "Coeus", param);
//            
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_COMMITTEE_REVIEWERS ( <<AW_COMMITTEE_ID>> , <<AW_PROTOCOL_NUMBER>>, "
                        +" <<OUT RESULTSET rset>>)", "Coeus", param);
            //Modified for case id COEUSQA-2772In IACUC when assigning reviewer, user can select a reviewer that is also PI end
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        int listSize = result.size();
    ...
    public int checkReviewerForProtocol(String membershipId,String protocolNumber)
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER CHECK>>=call FN_CHECK_REVIEWER_FOR_IACUC( "
    ...
    public int getNumberOfSubmittedProtocols(String scheduleId)
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER COUNT>>=call FN_GET_AC_PROTO_SUB_COUNT( "
                                        +"<< SCHEDULE_ID >> )}", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public boolean checkMinutesForProtocol(String protocolNumber)
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER COUNT>>=call FN_CHECK_MINUTES_FOR_IACUC( "
    ...
    public int getSeqNumberForRegeneration(String protocolNumber,int seqNumber,
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER SEQUENCE_NUMBER>>=call FN_GET_AC_PROTO_CURR_SEQ_CORR( "
                                        +"<< PROTOCOL_NUMBER >> ," +
                                        "<< SEQUENCE_NUMBER >> ," +
                                        "<< ACTION_ID >> )}", param);
        }else{
     ...
     *  <li>To fetch the data, it uses FN_GET_AC_MAX_PROTO_SUB_NUM function.
     *
    ...
    public int getMaxSubmissionNumber(String protocolId)
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER COUNT>> = call FN_GET_AC_MAX_PROTO_SUB_NUM( "
    ...
    public Vector getProtocolExpeditedCheckList(String protocolNumber, int sequenceNumber, int submissionNumber) throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
            "call GET_IACUC_EXPIDITED_CHKLST ( <<PROTOCOL_NUMBER>>, <<SEQUENCE_NUMBER>>, <<SUBMISSION_NUMBER>>, <<OUT RESULTSET rset>> )",
            "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public Vector getProtocolExemptCheckList(String protocolNumber, int sequenceNumber, int submissionNumber) 
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
            "call GET_IACUC_PROTO_EXEMPT_CHKLST ( <<PROTOCOL_NUMBER>> , <<SEQUENCE_NUMBER>>, <<SUBMISSION_NUMBER>>, <<OUT RESULTSET rset>> )",
            "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000") ;
        }
     ...
     */
     public Vector getCommitteeReviewers(String scheduleId, String protocolNumber)
        ...
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                        DBEngineConstants.TYPE_STRING,protocolNumber));        
        if(dbEngine!=null){
            //Added for Issue #1562 - 2260 - Reviewer when he is also an Investigator - start
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_COMM_REVIEWERS ( <<SCHEDULE_ID>>, <<PROTOCOL_NUMBER>>, "
                        +" <<OUT RESULTSET rset>>)", "Coeus", param);
            //Added for Issue #1562 - 2260 - Reviewer when he is also an Investigator - end
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        int listSize = result.size();
    ...
    public ProtocolSubmissionInfoBean getSubmissionForSubmissionNumber(
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
            "call GET_AC_PROTO_SUB_FOR_SUB_NUM( <<PROTOCOL_NUMBER>> , <<SUBMISSION_NUMBER>> , "+
                            "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
...
//        boolean blnReturn = true;
        ...
        while(iterator.hasNext()){            
            ...
            if(dbEngine!=null){
                result = dbEngine.executeFunctions("Coeus",
                "{<<OUT INTEGER CHECK>>=call FN_CAN_ASIGN_TO_SHDUL_IACUC( "
    ...
    public synchronized byte[] getSubmissionDocument(String protocolNumber, String sequenceNumber,
        ...
        String selectQuery = "SELECT DOCUMENT FROM OSP$AC_PROTOCOL_SUBMISSION_DOC " +
    ...
    throws CoeusException, DBException{
        ...
        if(protocolActionDocumentBean.getAcType().trim().equals("I")){            
            sqlProtocolActionDocument.append("INSERT INTO OSP$AC_PROTOCOL_SUBMISSION_DOC(");
            ...
            sqlProtocolActionDocument.append(" MIME_TYPE , "); 
            sqlProtocolActionDocument.append(" DOCUMENT , ");
            sqlProtocolActionDocument.append(" DESCRIPTION , "); 
...
//            result = new Vector(3,2);
//            result = dbEngine.executeRequest("Coeus",
//                    "call GET_AC_REVIEWER_RECOMM_ACTIONS( <<OUT RESULTSET rset>> )",
//                    "Coeus", param);
//        }else{
//            throw new CoeusException("db_exceptionCode.1000");
//        }        
...
//    }
    ...
    public int getSubmissionTypeCodeForAction(int actionCode){
        ...
        if(actionCode == IacucProtocolActionsConstants.NOTIFY_IACUC){
             ...
             submissionTypeCode = REQUEST_TO_LIFT_HOLD_SUB_TYPE;
        }else if(actionCode == IacucProtocolActionsConstants.REQUEST_TO_DEACTIVATE){
             submissionTypeCode = REQUEST_TO_DEACTIVATE_SUB_TYPE;
        }
        return submissionTypeCode ;
    }
    //COEUSDEV-86 : END
    
    // Added for IACUC Reviewer recommended action changes - Start
    
    /*
     * Method to get actions based on the review type
     * @return cvRecommendedActions - CoeusVector
     */
    public HashMap getRecommendedActionsForAllReview() throws DBException, CoeusException{
        HashMap hmRecommendedActions = new HashMap();
        Vector result = null;
        Vector param= new Vector();
        HashMap hmAction = null;
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                        "call GET_AC_RECOM_ACT_FOR_ALL_REV( <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if (result != null && result.size() > 0){
            ...
            for(int rowIndex=0; rowIndex<actionCount; rowIndex++){
                hmAction = (HashMap)result.elementAt(rowIndex);
                String reviewTypeCode = hmAction.get("REVIEW_TYPE_CODE").toString();
                ComboBoxBean actionDetails = new ComboBoxBean(hmAction.get(
                            "PROTOCOL_ACTION_TYPE_CODE").toString(),
                            hmAction.get("DESCRIPTION").toString());
                if(hmRecommendedActions.containsKey(reviewTypeCode)){
                    Vector vecRecommendedAction = (Vector)hmRecommendedActions.get(reviewTypeCode);
                    vecRecommendedAction.add(actionDetails);
                }else{
                    Vector vecRecommendeActions = new Vector();
                    vecRecommendeActions.add(actionDetails);
                    hmRecommendedActions.put(reviewTypeCode,vecRecommendeActions);
                }
            }
        }
        return hmRecommendedActions;
    }
    // Added for IACUC Reviewer recommended action changes - End
    
    // Added for COEUSQA-3025 : Enhanced functionality to address the IACUC designated member review determination process - Start
    /**
     * Method to get the review determinations details
     ...
     */
    public Vector getReviewDeterminations(String protocolNumber, int sequenceNumber, int submissionNumber) throws DBException, CoeusException{
        Vector result = new Vector();
        Vector vecReviewDetermination = new Vector();
        Vector param= new Vector();
        HashMap hmReviewDetermination = null;
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        param.addElement(new Parameter("SEQUENCE_NUMBER",
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_REVIEW_DETERMINATION ( <<PROTOCOL_NUMBER>> , <<SEQUENCE_NUMBER>> , <<SUBMISSION_NUMBER>> ,<<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        if (result != null && !result.isEmpty()){
            for(int index=0;index< result.size();index++){
                hmReviewDetermination = (HashMap)result.elementAt(index);
                ProtocolReviewDeterminationBean reviewDeterminationDetails = new ProtocolReviewDeterminationBean();
                reviewDeterminationDetails.setProtocolNumber(hmReviewDetermination.get("PROTOCOL_NUMBER").toString());
                reviewDeterminationDetails.setSequenceNumber(Integer.parseInt(hmReviewDetermination.get("SEQUENCE_NUMBER").toString()));
                reviewDeterminationDetails.setSubmissionNumber(Integer.parseInt(hmReviewDetermination.get("SUBMISSION_NUMBER").toString()));
                reviewDeterminationDetails.setPersonId(hmReviewDetermination.get("PERSON_ID").toString());
                reviewDeterminationDetails.setPersonFullName(hmReviewDetermination.get("FULL_NAME").toString());
                reviewDeterminationDetails.setRecommendedReviewTypeCode(Integer.parseInt(hmReviewDetermination.get("RECOMMENDED_REVIEW_TYPE_CODE").toString()));
                reviewDeterminationDetails.setRecommendedReviewTypeDescriptione(hmReviewDetermination.get("PROTOCOL_NUMBER") == null ? "" :
                                                                                hmReviewDetermination.get("PROTOCOL_NUMBER").toString());
                reviewDeterminationDetails.setDeterminationDueDate( hmReviewDetermination.get("DETERMINATION_DUE_DATE") == null ? null :
                    new Date( ((Timestamp) hmReviewDetermination.get("DETERMINATION_DUE_DATE")).getTime()));
                reviewDeterminationDetails.setSendNotificationDate( hmReviewDetermination.get("SEND_NOTIFICATION_DATE") == null ? null :
                    new Date( ((Timestamp) hmReviewDetermination.get("SEND_NOTIFICATION_DATE")).getTime()));
                reviewDeterminationDetails.setUpdateTimestamp((Timestamp)hmReviewDetermination.get("UPDATE_TIMESTAMP"));
                reviewDeterminationDetails.setUpdateUser(hmReviewDetermination.get("UPDATE_USER").toString());
                reviewDeterminationDetails.setAwUpdateTimestamp((Timestamp)hmReviewDetermination.get("UPDATE_TIMESTAMP"));
                reviewDeterminationDetails.setAwUpdateUser(hmReviewDetermination.get("UPDATE_USER").toString());
                vecReviewDetermination.addElement(reviewDeterminationDetails);
            }
        }
...
//code added for getting COI disclosure details to displaying DisclosureStatusForm - starts
 public Vector getDisclosureStatusDetails(String protocolNumber)throws CoeusException, DBException{
        ...
        {
                    result = dbEngine.executeRequest("Coeus",
                    "call GET_IC_PROTOCOL_DISCL_STATUS( <<PROTOCOL_NUMBER>> , "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else
        {
            throw new CoeusException("db_exceptionCode.1000");
...
//                data.add((String)hmRow.get("UNIT_NAME"));
 ...
 //code added to get iacuc protocol investigator,key-persons
  public Vector getNotificationPersonsDetails(String proposalNumber)throws CoeusException, DBException{
        ...
        if(dbEngine !=null){
            //result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_IACUC_INV_KPS( <<PROTOCOL_NUMBER>> ,<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
...
//code added to get a person email id.
     ...
     public HashMap getProtoPersonDetailsForMail(String proposalNumber)throws CoeusException, DBException{
        ...
        if(dbEngine !=null){
            result = dbEngine.executeRequest("Coeus","call GET_IACUC_PROTO_DETS_SND_NOT( <<PROTOCOL_NUMBER>> , <<OUT RESULTSET rset>>) ", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(result!=null){
...
public Vector getPropPersonDetailsForNotif(String proposalNumber)throws CoeusException, DBException{
    ...
    Vector param= new Vector();
        ...
        if(dbEngine !=null){
            result = dbEngine.executeRequest("Coeus","call GET_IC_PROTO_NOTIFIC_DETAILS( <<PROTOCOL_NUMBER>> , <<OUT RESULTSET rset>>) ", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
       return result;
}
     //code added to get protocol person details
   public Vector getProtoPersonDetailsForMail(String proposalNumber,String seqno)throws CoeusException, DBException{
        ...
        if(dbEngine !=null){
            result = dbEngine.executeRequest("Coeus","call GET_IC_PROTO_NOTI_DETAILS_DEL( <<PROTOCOL_NUMBER>> ,<<SEQUENCE_NUMBER>>, <<OUT RESULTSET rset>>) ", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        int listSize = result.size();
   ...
   //code added to update last notification date
         ...
         param.addElement(new Parameter("PROTOCOL_NUMBER",
          ...
          param.addElement(new Parameter("module",
           ...
           if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{ <<OUT INTEGER COUNT>> = "
            +" call FN_UPD_LAST_NOTIFIC_IACUC_DATE(<< PERSON_ID >>,<<PROTOCOL_NUMBER>>, <<module>> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
            if(!result.isEmpty()){
    ...
    /**
     * Method to get the review determination types 
     * @throws edu.mit.coeus.exception.CoeusException 
     * @throws edu.mit.coeus.utils.dbengine.DBException 
     * @return Vector
     */
     public Vector getProtocolReviewerDeterminationTypes() throws CoeusException,DBException{
         Vector vecResult = null;
         Vector vecReviewerTypes = null;
         Vector vecParam= new Vector();
         HashMap reviewerType = null;
         if(dbEngine != null){
             vecResult = new Vector();
             vecResult = dbEngine.executeRequest("Coeus",
                     "call GET_AC_REVIEW_TYPE_DETER( <<OUT RESULTSET rset>> )",
                     "Coeus", vecParam);
         }else{
             throw new CoeusException("db_exceptionCode.1000");
         }
         int actionCount = vecResult.size();
         vecReviewerTypes = new Vector();
         if (actionCount > 0){
             for(int rowIndex=0; rowIndex<actionCount; rowIndex++){
                 reviewerType = (HashMap)vecResult.elementAt(rowIndex);
                 vecReviewerTypes.addElement(new ComboBoxBean(
                         reviewerType.get(
                         "PROTOCOL_REVIEW_TYPE_CODE").toString(),
                         reviewerType.get("DESCRIPTION").toString()));
             }
         }
         return vecReviewerTypes;
     }
