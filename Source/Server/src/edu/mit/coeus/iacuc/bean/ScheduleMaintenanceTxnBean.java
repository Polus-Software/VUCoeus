...
// Added for COEUSQA-2542_Allow Protocol Reviewer to upload Attachments_start
    ...
    public String getContingencyDesc(String contingencyCode)
        ...
        if(dbEngine!=null){
            ...
            "call GET_AC_PROTO_CONTINGENCY_DESC( <<CON_CODE>> , "+
     ...
     *  Schedule maintenance screen from OSP$AC_SCHEDULE_ACT_ITEM_TYPE.
     *  <li>To fetch the data, it uses the procedure GET_AC_SCHEDULE_ACT_ITEM_TYPES.
     *
    ...
    public Vector getScheduleActionTypes() throws CoeusException,DBException{
        ...
        if(dbEngine != null){
            ...
            //COEUSQA-2686_IACUC-CHANGES TO SCHEDULE MAINTENANCE FOR 4.4.3 RELEASE_start
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_SCHEDULE_ACT_ITEM_TYPES( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
            //COEUSQA-2686_IACUC-CHANGES TO SCHEDULE MAINTENANCE FOR 4.4.3 RELEASE_end
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public Vector getProtocolSubmissionTypes() throws CoeusException,DBException{
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_SUBMISSION_TYPES( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public Vector getProtocolSubmissionTypeQualifiers() 
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_SUBMISSION_TYPE_QUALS( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public Vector getProtocolSubmissionReviewTypes() 
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_PROTOCOL_REVIEW_TYPES( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public Vector getProtocolContingency() throws CoeusException,DBException{
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                "call GET_AC_PROTO_CONTINGENCY_LIST( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
   ...
   /**
     ...
     *  from OSP$COMM_SCHEDULE_ACT_ITEMS,OSP$AC_SCHEDULE_ACT_ITEM_TYPE.
     *  <li>To fetch the data, it uses GET_AC_COMM_SCHEDULE_ACT_ITEMS procedure.
     *
    ...
    public Vector getOtherActions(String scheduleId) 
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            // COEUSQA-2686 IACUC - Changes to schedule maintenance for 4.4.3 release_Start
            result = dbEngine.executeRequest("Coeus",
            "call GET_AC_COMM_SCHEDULE_ACT_ITEMS( <<SCHEDULE_ID>> , "+
                            "<<OUT RESULTSET rset>> )", "Coeus", param);
            // COEUSQA-2686 IACUC - Changes to schedule maintenance for 4.4.3 release_End
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        int listSize = result.size();
...
// Added for COEUSQA-2542_Allow Protocol Reviewer to upload Attachments_start
    ...
    throws CoeusException,DBException{
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call get_ac_reviewer_attach_for_sub( <<PROTOCOL_NUMBER>>, <<SUBMISSION_NUMBER>>, "
                    + " <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    public ReviewAttachmentsBean getReviewAttachmentInfo(String protocolNumber,int sequenceNumber,
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call get_ac_reviewer_attach_info( <<AW_PROTOCOL_NUMBER>>, <<AW_SEQUENCE_NUMBER>>,"
    ...
    public boolean checkReviewAttachmentPresent(String protocolNumber,int sequenceNumber, int submissionNumber) throws DBException, CoeusException{
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER COUNT>> = call FN_AC_IS_REVIEW_ATTACH_PRESENT ( "
     ...
     * <li>To fetch the data, it uses the function FN_AC_GENERATE_REVW_DOC_ID.
     * @param String protocolNumber
     * @param int sequenceNumber
     * @param int submissionNumber
    ...
    public int getNextReviewAttachmentID(String protocolNumber,int sequenceNumber, int submissionNumber) throws  DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER LL_ATTACHMENT_ID>> = call fn_ac_generate_review_attch_id("
    ...
    throws CoeusException, DBException{
        ...
        sql.append(" FROM OSP$AC_REVIEWER_ATTACHMENTS");
...
//            throw new CoeusException("exceptionCode.999999");
    ...
    public ScheduleDetailsBean getScheduleMaintenance(String scheduleId)
        ...
        if(scheduleDetailsBean.getCommitteeTypeCode() == CoeusConstants.IACUC_COMMITTEE_TYPE_CODE){
        ...
        scheduleDetailsBean.setNonDeferredProtocols(getProtocolsForScheduleMinutes(scheduleId));
        return scheduleDetailsBean;
    }
     ...
     **/
    public Vector getProtocolsForScheduleMinutes(String scheduleId)
        throws CoeusException, DBException {
        Vector result = null;
        Vector param= new Vector();
        ...
        if(dbEngine != null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_FOR_SCH_MINUTES( <<SCHEDULE_ID>> , "+
                            "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
     ...
     *  <li>To fetch the data, it uses FN_AC_PERSON_HAS_ALTER_ROLE function.
     *
    ...
    public int getHasAlternateRole(String personId, String scheduleId) 
        ...
        if(dbEngine != null){
            ...
            +"call FN_AC_PERSON_HAS_ALTER_ROLE(<< PERSON_ID >>, << SCHEDULE_ID >>) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
    ...
    public Vector getProtocolSubmissionTypeQualifiers(String submissionTypeCode) 
        ...
        if(dbEngine != null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_TYPE_QUAL_FOR_SUBTYPE( <<SUBMISSION_TYPE_CODE>> , "+
                            "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
...
//                    "<<OUT RESULTSET rset>> )", "Coeus", param);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_SUB_LIST_FOR_SCH( <<SCHEDULE_ID>> ,  <<PERSON_ID>>,"+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
            //Modified for the case#3282/3284-Reviewer Views and Comments-end
        }else{
            throw new CoeusException("db_exceptionCode.1000");
