...
//              } 
     ...
     *  <li>To fetch the data, it uses get_ac_comm_research_areas procedure.
     *
     *  @param committeeId this is given input parameter for the
     *  procedure to execute.
    ...
    public Vector getCommitteeResearchAreas(String committeeId)
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
            "call get_ac_comm_research_areas ( <<COMMITTEE_ID>> , "
    ...
    public boolean checkScheduleHasChildren(String scheduleId)
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
            "{<<OUT INTEGER HASCHILDREN>>=call FN_IACUC_CHK_SHDULE_HAS_CHILD( "
                                        +"<< SCHEDULE_ID >> )}", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
