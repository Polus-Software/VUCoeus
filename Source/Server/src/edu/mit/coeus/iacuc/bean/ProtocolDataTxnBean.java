...
public class ProtocolDataTxnBean implements TypeConstants, ModuleConstants,MailPropertyKeys{
    ...
    /*Added for COEUDQA-1724-In Comments on Action Details, indicate Renewal with Amendment for submissions created via New Am -start */
    private static final char NEW_AMMENDMENT_REVISION = 'L';
    private final char  NEW_CONTINUATION_REVIEW = '#';
    private final char  NEW_CONTINUATION_REVIEW_AMEND = '&';
    /*Added for COEUDQA-1724-In Comments on Action Details, indicate Renewal with Amendment for submissions created via New Am -end */
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    ...
                    +" call GET_AC_PROTO_SPECIAL_REV_NUM(<< PROTOCOL_NUMBER >> ,<< SEQUENCE_NUMBER >> ) }", param);
        }else{
...
//  Commented for COEUSQA-2320 Show in Lite for Special Review in Code table. Use
//  method getSpecialReviewTypesForModule from edu.mit.coues.irb.bean.ProtocolDataTxnbean
//    /**
...
//
    ...
    public Vector getProtocolTypes() throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_TYPES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public Vector getProtocolStatus() throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_STATUS ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_NOTES ( <<PROTOCOL_NUMBER>> , "
    ...
    public Vector getCorrespondentTypes() throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_CORRESPONDENT_TYPES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public ProtocolInfoBean getProtocolInfo(String protocolNumber,
        ...
        if(functionType==NEW_AMMENDMENT){
            rowId = rowId + CoeusConstants.IACUC_AMENDMENT;
        }else if( functionType==NEW_REVISION){
            rowId = rowId + CoeusConstants.IACUC_RENEWAL;
        }
        /*COEUSQA-1724-New condition Added to check Locking functionality for new Amendment/renewal Type - Start */
        else if( functionType==NEW_AMMENDMENT_REVISION){
            rowId = rowId + CoeusConstants.IACUC_RENEWAL_WITH_AMENDMENT;
        }else if( functionType==NEW_CONTINUATION_REVIEW){
            rowId = rowId + CoeusConstants.IACUC_CONTINUATION_CONTINUING_REVIEW;
        }else if( functionType==NEW_CONTINUATION_REVIEW_AMEND){
            rowId = rowId + CoeusConstants.IACUC_CONTI_CONTINUING_REVIEW_WITH_AMEND;
        }
        /*COEUSQA-1724-New condition Added to check Locking functionality for new Amendment/renewal Type - End */
    ...
    public ProtocolInfoBean getProtocolInfo(String protocolNumber,
        ...
        // If Amendment or Revision lock the row
        if(functionType==NEW_AMMENDMENT){
            protocolBean.setPendingAmmendRenewal(hasPendingAmendmentsRenewals(protocolNumber, Character.toString(CoeusConstants.IACUC_AMENDMENT)));
            rowId = rowId + CoeusConstants.IACUC_AMENDMENT;
        }
        // else if( functionType==NEW_REVISION)
        /*Added for case#4278- In Comments on Action Details, indicate Renewal with Amendment for submissions created via New Am -start*/
            ...
            /*Added for case#4278- In Comments on Action Details, indicate Renewal for submissions created via New Am -end*/
            protocolBean.setPendingAmmendRenewal(hasPendingAmendmentsRenewals(protocolNumber, Character.toString(CoeusConstants.IACUC_RENEWAL)));
            rowId = rowId + CoeusConstants.IACUC_RENEWAL;
        }
        /*COEUDQA-1724-New Condition check added for new Amendment/Renewal type - Start */
        else if (functionType == NEW_AMMENDMENT_REVISION) {
            /*Added for case#4278- In Comments on Action Details, indicate Renewal with Amendment for submissions created via New Am -end*/
            protocolBean.setPendingAmmendRenewal(hasPendingAmendmentsRenewals(protocolNumber, Character.toString(CoeusConstants.IACUC_RENEWAL_WITH_AMENDMENT)));
            rowId = rowId + CoeusConstants.IACUC_RENEWAL_WITH_AMENDMENT;
        }

        else if (functionType == NEW_CONTINUATION_REVIEW) {
            /*In Comments on Action Details, indicate Continuation/Continuing Review for submissions created via New Am -end*/
            protocolBean.setPendingAmmendRenewal(hasPendingAmendmentsRenewals(protocolNumber, Character.toString(CoeusConstants.IACUC_CONTINUATION_CONTINUING_REVIEW)));
            rowId = rowId + CoeusConstants.IACUC_CONTINUATION_CONTINUING_REVIEW;
        }else if (functionType == NEW_CONTINUATION_REVIEW_AMEND) {
            /*In Comments on Action Details, indicate Continuation/Continuing Review with Amendment for submissions created via New Am -end*/
            protocolBean.setPendingAmmendRenewal(hasPendingAmendmentsRenewals(protocolNumber, Character.toString(CoeusConstants.IACUC_CONTI_CONTINUING_REVIEW_WITH_AMEND)));
            rowId = rowId + CoeusConstants.IACUC_CONTI_CONTINUING_REVIEW_WITH_AMEND;
        }
        /*COEUDQA-1724-New Condition check added for new Amendment/Renewal type - End */
        if(dbEngine!=null){
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    ...
                    ", << OUT STRING EXPIRATION_DATE_FLAG >> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (listSize >0){
            ...
            //EOEUSQA-1724-Added for Expiration date-start
            sRApprovalInfoBean.setExpirationDateFlag((String)
            spApprovalRow.get("EXPIRATION_DATE_FLAG"));
            //EOEUSQA-1724-Added for Expiration date-end
    ...
    throws DBException{
        ...
        if (listSize >0){
            ...
            for(int rowIndex=0;rowIndex<listSize;rowIndex++){
                ...
                sRApprovalInfoBean.setApplicationDateFlag((String)
                srApprovalRow.get("EXPIRATION_DATE_FLAG"));
...
//                        protocolInfoBean.getSequenceNumber()));
                ...
                protocolInfoBean.setFundingSources(getProtocolFundingSources(
                        protocolInfoBean.getProtocolNumber(),
                        protocolInfoBean.getSequenceNumber()));
                //Added for IACUC amendment-start
                protocolInfoBean.setSpecies(getProtocolSpecies(
                        protocolInfoBean.getProtocolNumber(),
                        protocolInfoBean.getSequenceNumber()));
                protocolInfoBean.setAlternativeSearch(getIacucProtocolAlterDBSearches(
                        protocolInfoBean.getProtocolNumber(),
                        protocolInfoBean.getSequenceNumber()));
                //Added for IACUC amendment-end
                ...
                protocolInfoBean.setStudyGroups(getStudyGroups(
                        protocolInfoBean.getProtocolNumber(),
                        protocolInfoBean.getSequenceNumber()));
                //Commented for COEUSQA-2724-Exceptions should be moved from Scientific Justification in IACUC
//                protocolInfoBean.setScientJustPrinciples(getIacucProtocolExceptions(
//                        protocolInfoBean.getProtocolNumber(),
//                        protocolInfoBean.getSequenceNumber()));
...
//            e.printStackTrace();
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_INFO( <<PROTOCOL_NUMBER>> , "
        ...
        if(!result.isEmpty()){
            ...
            protocolInfoBean.setVersionNumber(Integer.parseInt(
                    infoProtocolRow.get("VERSION_NUMBER") == null ? "0"
                    :infoProtocolRow.get("VERSION_NUMBER").toString()));
            ...
            protocolInfoBean.setProjectTypeCode(Integer.parseInt(
                    infoProtocolRow.get("PROJECT_TYPE_CODE") == null ? "0"
                    :infoProtocolRow.get("PROJECT_TYPE_CODE").toString()));
            protocolInfoBean.setProjectTypeDesc((String)
            infoProtocolRow.get("PROJECT_TYPE_DESCRIPTION"));
            ...
            protocolInfoBean.setLayStmt1( infoProtocolRow.get("LAY_STATEMENT_1") == null ? ""
                    : infoProtocolRow.get("LAY_STATEMENT_1").toString());
            protocolInfoBean.setLayStmt2(infoProtocolRow.get("LAY_STATEMENT_2") == null ? ""
                    :infoProtocolRow.get("LAY_STATEMENT_2").toString());
...
//            infoProtocolRow.get("VULNERABLE_SUBJECT_INDICATOR"));
            ...
            protocolInfoBean.setSpeciesStudyGroupIndicator((String)
            infoProtocolRow.get("SPECIES_STUDY_GROUP_INDICATOR"));
            // Indicator logic implementation in Species-Study Groups- End
            // Added for Indicator logic implementation in Scientific Justificaiton and Alternative Search - Start
            protocolInfoBean.setAlterSearchIndicator((String)
            infoProtocolRow.get("ALTERNATIVE_SEARCH_INDICATOR"));
            protocolInfoBean.setScientificJustIndicator((String)
            infoProtocolRow.get("SCIENTIFIC_JUSTIF_INDICATOR"));
            // Added for Indicator logic implementation in Scientific Justificaiton and Alternative Search - End
...
//            infoProtocolRow.get("RELATED_PROJECTS_INDICATOR"));
            ...
            protocolInfoBean.setOverviewTimeline(infoProtocolRow.get("OVERVIEW_TIMELINE") == null ? ""
                    : infoProtocolRow.get("OVERVIEW_TIMELINE").toString());
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_INFO_FOR_SEQ( << AW_PROTOCOL_NUMBER >> , << AW_SEQUENCE_NUMBER >> , "
                    +" <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
...
//            summaryDetails = new Vector();
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_STUDY_PERSON_LIST( <<PROTOCOL_NUMBER>> , "
...
//                keyPersonListRow.get("PERSON_ROLE"));
                protocolKeyPersonnelBean.setPersonRoleCode(
                        Integer.parseInt(keyPersonListRow.get("KEY_PERSON_ROLE_CODE").toString()));
                protocolKeyPersonnelBean.setPersonRoleDesc(
                        (String)keyPersonListRow.get("PERSON_ROLE"));
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_LOCATION_LIST( <<PROTOCOL_NUMBER>> , "
                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if(listSize >0){
            ...
            for(int rowIndex=0;rowIndex<listSize;rowIndex++){
                ...
                protocolLocationListBean.setAnimalWelfareAssurance((String)
                locationProtocolListRow.get("ANIMAL_WELFARE_ASSURANCE"));
...
//        if(dbEngine!=null){
//            result = dbEngine.executeRequest("Coeus",
//                    "call GET_IACUC_VULNERABLE_SUB_LIST( <<PROTOCOL_NUMBER>> , "
//                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )", "Coeus", param);
//        }else{
//            throw new DBException("DB instance is not available");
//        }
...
//    }
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_INVESTIGATORS( <<PROTOCOL_NUMBER>> , "
                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public Vector getProtocolInvestigatorsUnits(String protocolNumber,
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_INV_UNITS ( <<PROTOCOL_NUMBER>> , "
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_CORRESPONDENTS ( <<PROTOCOL_NUMBER>> , "
                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_RESEARCH_AREAS ( <<PROTOCOL_NUMBER>> , "
                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_FUNDING_SOURCES ( <<PROTOCOL_NUMBER>> , "
                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
...
//        Vector correspTypes = correspTxnBean.getProtoCorrespTemplates();
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_ACTIONS ( <<PROTOCOL_NUMBER>> , "
                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        ...
        if(listSize>0){
            ...
            for(int rowIndex=0;rowIndex<listSize;rowIndex++){
                ...
                protocolActionsBean.setVersionNumber(Integer.parseInt(
                        actionsProtocolRow.get("VERSION_NUMBER") == null ? "0"
                        : actionsProtocolRow.get("VERSION_NUMBER").toString()));
...
//        Vector correspTypes = correspTxnBean.getProtoCorrespTemplates();
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_ACTIONS_FOR_SUB ( <<PROTOCOL_NUMBER>> , "
...
//                protocolActionsBean.setIsCorresTemplateExists(hasTemplates(protocolActionsBean,correspTypes));
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_SP_REVIEW_LIST ( <<PROTOCOL_NUMBER>> , <<SEQUENCE_NUMBER>>, "
                    +" <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        ...
        if(listSize>0){
            ...
            for(int rowIndex=0;rowIndex<listSize;rowIndex++){
                ...
                protocolSpecialReviewFormBean.setExpirationDate(
                        reviewProtocolRow.get("EXPIRATION_DATE") == null ? null
                        :new Date( ((Timestamp) reviewProtocolRow.get(
                        "EXPIRATION_DATE")).getTime()) );
    ...
    public Vector getProtocolRoles()throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_ROLES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
...
//        String userId ="";
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_USERS_FOR_AC_PROTO_ROLE ( <<PROTOCOL_NUMBER>> , "
...
//            System.out.println("** Start Server side validation for edit ") ;
            ...
            if(dbEngine!=null){
                result = dbEngine.executeFunctions("Coeus",
                        "{<<OUT INTEGER SUCCESS>> = call FN_CAN_MODIFY_AC_PROTOCOL( "
...
//        String userId ="";
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_AFFILIATION_TYPES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
...
//        String userId ="";
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_ORG_TYPES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
...
//            String userId ="";
            ...
            if(dbEngine!=null){
                result = dbEngine.executeRequest("Coeus",
                        "call GET_AC_PROTO_REFERENCE_TYPES ( <<OUT RESULTSET rset>> )",
                        "Coeus", param);
            }else{
                throw new DBException("DB instance is not available");
            }
...
//            e.printStackTrace();
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    ...
                    +" call FN_GET_MAX_AC_PROTO_REF_NUM(<< PROTOCOL_NUMBER >> ,<< SEQUENCE_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_REFERENCE_LIST ( <<PROTOCOL_NUMBER>> , <<SEQUENCE_NUMBER>>, "
                    +" <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        ...
        if(listSize>0){
            ...
            for(int rowIndex=0;rowIndex<listSize;rowIndex++){
                ...
                protocolReferencesBean.setExpirationDate(
                        referenceRow.get("EXPIRATION_DATE") == null ? null
                        :new Date( ((Timestamp) referenceRow.get(
                        "EXPIRATION_DATE")).getTime()) );
    ...
    throws CoeusException, DBException{
        ...
        if(dbEngine !=null){
            ...
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_CUSTOM_DATA( <<PROTOCOL_NUMBER>>, <<SEQUENCE_NUMBER>>, <<MODULE_CODE>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
     ...
     * module by passing the code COEUS_MODULE_IACUC
     *
     * @return Vector collections of ProtocolCustomElementsInfoBean for others tab
    ...
    throws CoeusException, DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER COUNT>>=call FN_AC_PROTOCOL_HAS_CUSTOM_DATA ( "
        ...
        param.add(new Parameter("PARAM_NAME",
                DBEngineConstants.TYPE_STRING,"COEUS_MODULE_IACUC"));
        result = dbEngine.executeFunctions("Coeus",
    ...
    /**  This method used get max Protocol Version Number for the given protocol.
     *  <li>To fetch the data, it uses the function FN_GET_NEXT_AC_PROTO_VERSION.
     *
     * @return int version number for the protocol number
     * @param protocolNumber Protocol Number
     * @exception DBException if any error during database transaction.
     * @exception CoeusException if the instance of dbEngine is not available. */
    public int getMaxVersionNumber(String protocolNumber)
    throws CoeusException, DBException {
        int versionNumber = 0;
        Vector param= new Vector();
        Vector result = new Vector();
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    ...
                    +" call FN_GET_NEXT_AC_PROTO_VERSION(<< PROTOCOL_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
     ...
     *  <li>To fetch the data, it uses the function FN_GET_NEXT_AC_AMEND_VERSION.
     *
     * @return int version number for the protocol number
     * @param protocolNumber Protocol Number
     * @exception DBException if any error during database transaction.
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER VERSION_NUMBER>> = "
                    +" call FN_GET_NEXT_AC_AMEND_VERSION(<< PROTOCOL_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
     ...
     *  <li>To fetch the data, it uses the function FN_GET_NEXT_AC_REVISION_VER.
     *
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER VERSION_NUMBER>> = "
                    +" call FN_GET_NEXT_AC_REVISION_VER(<< PROTOCOL_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
    ...
    /**  This method used get max Amendment/renewal Number for the given protocol
     *  <li>To fetch the data, it uses the function FN_GET_NEXT_AC_REV_AMEND_VER.
     *
     * @return int revision number for the protocol number
     * @param protocolNumber Protocol Number
     * @exception DBException if any error during database transaction.
     * @exception CoeusException if the instance of dbEngine is not available. */
    public int getMaxAmendRevisionVersionNumber(String protocolNumber)
    throws CoeusException, DBException {
        int versionNumber = 0;
        Vector param= new Vector();
        Vector result = new Vector();
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER VERSION_NUMBER>> = "
                    +" call FN_GET_NEXT_AC_REV_AMEND_VER(<< PROTOCOL_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
    ...
    //COEUSQA-1724-Added for New amendment/renewals type imp-lementation -start
   /**  This method used get max Continuation/Continuing Review Number for the given protocol
     *  <li>To fetch the data, it uses the function FN_GET_NEXT_AC_CONT_REV_VER.
     *
     * @return int revision number for the protocol number
     * @param protocolNumber Protocol Number
     * @exception DBException if any error during database transaction.
     * @exception CoeusException if the instance of dbEngine is not available. */
    public int getMaxContinuingReviewNumber(String protocolNumber)
    throws CoeusException, DBException {
        int versionNumber = 0;
        Vector param= new Vector();
        Vector result = new Vector();
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER VERSION_NUMBER>> = "
                    +" call FN_GET_NEXT_AC_CONT_REV_VER(<< PROTOCOL_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
    ...
    /**  This method used get maxContinuation/Continuing Review with Amendment Number for the given protocol
     *  <li>To fetch the data, it uses the function FN_GET_NEXT_AC_CONT_REV_AM_VER.
     *
     * @return int revision number for the protocol number
     * @param protocolNumber Protocol Number
     * @exception DBException if any error during database transaction.
     * @exception CoeusException if the instance of dbEngine is not available. */
    public int getMaxContinuingReviewAmendNumber(String protocolNumber)
    throws CoeusException, DBException {
        int versionNumber = 0;
        Vector param= new Vector();
        Vector result = new Vector();
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER VERSION_NUMBER>> = "
                    +" call FN_GET_NEXT_AC_CONT_REV_AM_VER(<< PROTOCOL_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
    ...
    //COEUSQA-1724-Added for New amendment/renewals type imp-lementation -End
    ...
    throws CoeusException, DBException{
        ...
        if(dbEngine !=null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_ALL_AC_PROTO_AMEN_REVISION( <<PROTOCOL_NUMBER>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        ...
        if (listSize > 0){
            ...
            for(int rowIndex=0; rowIndex<listSize; rowIndex++){
                ...
                protocolAmendRenewalBean.setCreatedDate((Timestamp)protocolRow.get("DATE_CREATED"));
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER SUCCESS>> = call FN_CAN_AMEND_AC_PROTOCOL( "
                    + " << AS_PROTOCOL_NUMBER >> )}", param) ;
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER SUCCESS>> = call FN_CHECK_PEN_AC_AM_REN_PROTO( "
...
//        if(dbEngine!=null){
//            result = dbEngine.executeFunctions("Coeus",
//                    "{<<OUT INTEGER SUCCESS>> = call FN_CAN_RENEW_IACUC_PROTO( "
//                    + " << AS_PROTOCOL_NUMBER >> )}", param) ;
//        }else{
//            throw new CoeusException("db_exceptionCode.1000");
//        }
...
//        if(dbEngine!=null){
//            result = dbEngine.executeRequest("Coeus",
//                    "call GET_MOD_FOR_IACUC_PROTO_PROJ ( <<OUT RESULTSET rset>> )",
//                    "Coeus", param);
//        }else{
//            throw new DBException("DB instance is not available");
//        }
...
//        String newAgendaID = null ;
        ...
        String selectQuery = "SELECT CORRESPONDENCE FROM OSP$AC_PROTOCOL_CORRESPONDENCE " +
    ...
    throws DBException, CoeusException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_CORRESP_OF_ACTION ( <<PROTOCOL_NUMBER>> , "
    ...
    public boolean actionHasCorrespondence(String protocolNumber, int sequenceNumber, int actionId) {
        ...
        try {
            ...
            if(dbEngine!=null){
                result = dbEngine.executeFunctions("Coeus",
                        "{<<OUT INTEGER SUCCESS>> = call FN_AC_PROTO_ACTION_HAS_CORRESP( "
...
//            ex.printStackTrace() ;
    ...
    throws DBException, CoeusException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_NOT_RESTRICT_NOTE ( <<PROTOCOL_NUMBER>> , "
                    +" <<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    ...
                    +" call FN_GET_MAX_NOTE_NUM_AC_PROTO(<< PROTOCOL_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
...
//        HashMap hashUnits = new HashMap() ;
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_UNITS_IN_MAX_SEQ ( <<PROTOCOL_NUMBER>> , "
                    +" <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
    ...
    public Vector getCorrespondentTypesForProtocol() throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_CORR_TYPES_FOR_AC_PROTOCOL ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public Vector getUploadDocumentForProtocol(String protocolNumber) throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_DOCUMENT_LIST ( <<PROTOCOL_NUMBER>> , <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int ctypesCount = result.size();
...
//        Vector vecUploadDoc = null;
        ...
        if(dbEngine!=null){
            /*result = dbEngine.executeRequest("Coeus",
                    ...
                    ",<<OUT RESULTSET rset>> )","Coeus", param);*/
            String Statement ="SELECT * FROM OSP$AC_PROTOCOL_DOCUMENTS WHERE  OSP$AC_PROTOCOL_DOCUMENTS.PROTOCOL_NUMBER = <<PROTOCOL_NUMBER>> " +
                    "AND OSP$AC_PROTOCOL_DOCUMENTS.SEQUENCE_NUMBER = <<SEQUENCE_NUMBER>> AND OSP$AC_PROTOCOL_DOCUMENTS.DOCUMENT_TYPE_CODE = <<DOCUMENT_TYPE_CODE>> " +
                    "AND OSP$AC_PROTOCOL_DOCUMENTS.VERSION_NUMBER = <<VERSION_NUMBER>> AND OSP$AC_PROTOCOL_DOCUMENTS.DOCUMENT_ID = <<DOCUMENT_ID>> ";
...
//                ex.printStackTrace();
    ...
    public Vector getProtocolHistoryData(String protocolNumber, String documentId) throws DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_DOCUMENTS_HISTORY ( <<AV_PROTOCOL_NUMBER>> ,<<AV_DOCUMENT_ID>>, "
                    +" <<OUT RESULTSET rset>> )","Coeus", param);
    ...
    public CoeusVector getProtocolDocumetTypes() throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_DOCUMENT_TYPE ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    ...
                    +" call FN_GET_LEAD_UNIT_FOR_AC_PROTO(<< PROTOCOL_NUMBER >> ,<< SEQUENCE_NUMBER >> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
...
//                dbEx.printStackTrace();
  ...
  // for protocol Routing
          ...
          public LockingBean lockIacucProtocolRouting(
            String protocolNumber,String loggedinUser, String unitNumber) throws Exception{
...
//                dbEx.printStackTrace();
    ...
    public Vector getUnitsForProtocolLocations(String protoNum) throws DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_UNITS_FOR_AC_PROTO_LOC  ( <<AS_PROTOCOL_NUMBER>> ,<<OUT RESULTSET DETAILS>>)",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public boolean isDocumentAmendable(String protocolNumber, int documentId){
        ...
        try {
            ...
            if(dbEngine!=null){
                result = dbEngine.executeFunctions("Coeus",
                        "{<<OUT INTEGER AMENDABLE>> = call FN_CAN_AMEND_AC_PROTOCOL_DOC( "
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine !=null){
            ...
            moduleResult = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_AMEND_RENEW_MOD( <<PROTO_AMEND_RENEWAL_NUMBER>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
...
//            vecModuleData = new Vector();
    ...
    public Vector getAmendRenewEditableModules() throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_MODULES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
  ...
  /*  public static void main(String[] args){
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER ll_Ret>> = "
                    +" call FN_CAN_DELETE_AC_PROTOCOL(<< PROTOCOL_NUMBER >>) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
    ...
    public Vector getProtocolLinksData(String protocolNumber) throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_LINKS_FOR_AC_PROTOCOL( <<PROTOCOL_NUMBER>> , "
                    +" <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
...
//        if(dbEngine!=null){
//            result = dbEngine.executeRequest("Coeus",
//                    "call GET_IACUC_PROTO_RISK_LEVELS ( <<PROTOCOL_NUMBER>>, <<OUT RESULTSET rset>> )",
//                    "Coeus", param);
//        }else{
//            throw new DBException("DB instance is not available");
//        }
...
//    }
    ...
    throws CoeusException, DBException{
        ...
        if(dbEngine != null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_OTHER_DOCUMENTS(<<PROTOCOL_NUMBER>>, <<OUT RESULTSET rset>> ) ",
                    "Coeus", param);
        }
        if(result != null && result.size() > 0){
            HashMap hmRow = null;
    ...
    throws CoeusException, DBException{
        ...
        sql.append(" FROM OSP$AC_PROTOCOL_OTHER_DOCS");
    ...
    throws CoeusException,DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER DOCUMENT_ID>> = call FN_GET_AC_PROTO_OTHER_DOC_ID( "
    ...
    private byte[] convert(ByteArrayOutputStream baos){
        ...
        try {
            ...
            try {
                ...
                UtilFactory.log(ioex.getMessage());
            }
        }
        return byteArray;
    }
    ...
    public String getProtocolWatermarkText(String protocolNumber) throws  DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT STRING REPORT_VALUE>> = call FN_GET_AC_PROTO_WATERMARK_TEXT ("
                    +"<< PROTOCOL_NUMBER >> )}",param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public int isProtocolRenewalAmendment(String protocolNumber) throws CoeusException,DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER PROTO_TYPE>> = call fn_is_ac_protocol_renew_amend( "
                    + " << PROTOCOL_NUMBER >> )}", param) ;
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
...
//    case 4398 - end
    ...
    public boolean isUserProtocolInvestigator(String protocolNumber, int sequenceNumber, String userId) throws DBException, CoeusException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER IS_INV>> = call FN_IS_USER_AC_PROTO_INV( "
...
//                    + " << USER_ID >> , << UNIT_NUMBER >> )}", param) ;
    ...
    public boolean isProtocolReviwerReviewComplete(String protocolNumber,int sequenceNumber,int submissionNumber,String personId) throws DBException, CoeusException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER IS_REVIEWER_REVIEW_COMPLETE>> = call FN_REVIEWER_REV_COMP_AC_PROTO( "
    ...
    private Vector getUsersForAllProtocolRoles(String protocolNumber, int sequenceNumber) throws DBException {
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_USERS_FROM_ALL_AC_PRO_ROLE ( <<PROTOCOL_NUMBER>>,<<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    throws DBException,CoeusException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_ACTIONS_DOCUMENTS(<<PROTOCOL_NUMBER>> , <<SEQUENCE_NUMBER>>, <<OUT RESULTSET rset>> ) ",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    public HashMap getProtocolDetailsForMail(String protocolNumber) throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call get_ac_proto_details_for_mail( << AW_PROTOCOL_NUMBER >> , "
                    +" <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        //COEUSQA-1724 - Email Notifications For All Actions in IACUC Module - End
    ...
    public Vector getProtoAmendRenewEditableModules(String protoAmendRenewNumber) throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_EDITED_AC_PROTO_AM_REN_MOD( << AV_PROTO_AMEND_RENEWAL_NUMBER >> , "
                    +" <<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
    ...
    throws DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_DOCUMENTS ( <<PROTOCOL_NUMBER>>,<<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int ctypesCount = result.size();
    ...
    // IACUC Modification start
    //Study group
    /*
     * Method to get the Study Groups details with the loaction and other details
     * @param protocolNumber - String
     * @param sequenceNumber - int
     * @return vecStudyGroup - Vector
     */
    public Vector getStudyGroups(String protocolNumber,int sequenceNumber) throws CoeusException,DBException{

        Vector result = new Vector();
        Vector vecStudyGroup = new Vector();
        Vector param= new Vector();
        HashMap hmStudyGroup = null;
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        param.addElement(new Parameter("SEQUENCE_NUMBER",
                DBEngineConstants.TYPE_INTEGER,new Integer(sequenceNumber)));
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_STUDY_GROUPS ( <<PROTOCOL_NUMBER>>,<<SEQUENCE_NUMBER>> , <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (result != null && result.size() >0){
            for(int index=0;index< result.size();index++){
                hmStudyGroup = (HashMap)result.elementAt(index);
                ProtocolStudyGroupBean protocolStudyGroup = new ProtocolStudyGroupBean();
                protocolStudyGroup.setProtocolNumber((String)hmStudyGroup.get("PROTOCOL_NUMBER"));
                protocolStudyGroup.setSequenceNumber(Integer.parseInt(hmStudyGroup.get("SEQUENCE_NUMBER").toString()));
                protocolStudyGroup.setSpeciesCount(hmStudyGroup.get("COUNT") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("COUNT").toString()));
                protocolStudyGroup.setProcedureCategoryCode(hmStudyGroup.get("PROCEDURE_CATEGORY_CODE") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("PROCEDURE_CATEGORY_CODE").toString()));
                protocolStudyGroup.setProcedureCategoryName((String)hmStudyGroup.get("PROCEDURE_CATEGORY_NAME"));
                protocolStudyGroup.setPainCategoryCode(hmStudyGroup.get("PAIN_CATEGORY_CODE") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("PAIN_CATEGORY_CODE").toString()));
                protocolStudyGroup.setPainCategoryName((String)hmStudyGroup.get("PAIN_CATEGORY_NAME"));
                protocolStudyGroup.setProcedureCode(hmStudyGroup.get("PROCEDURE_CODE") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("PROCEDURE_CODE").toString()));
                protocolStudyGroup.setProcedureName((String)hmStudyGroup.get("PROCEDURE_NAME"));
                //Modified with CoeusQQA-2551:Rework how user enters species,study groups and procedures in IACUC protocols
                //Uncommented for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -Start
                protocolStudyGroup.setSpeciesCode(hmStudyGroup.get("SPECIES_CODE") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("SPECIES_CODE").toString()));
                //Uncommented for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -End
                //Added for case id COEUSQA-2551 begin
                //uncommented for case id COEUSQA-2551
                protocolStudyGroup.setSpeciesGroupName((String)hmStudyGroup.get("SPECIES_GROUP"));
                protocolStudyGroup.setSpeciesName((String)hmStudyGroup.get("SPECIES_NAME"));
                //Added for case id COEUSQA-2551 end
                protocolStudyGroup.setStudyGroupId(hmStudyGroup.get("STUDY_GROUP_ID") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("STUDY_GROUP_ID").toString()));
                protocolStudyGroup.setSpeciesId(hmStudyGroup.get("SPECIES_ID") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("SPECIES_ID").toString()));

//                protocolStudyGroup.setStudyGroupName((String)hmStudyGroup.get("STUDY_GROUP_NAME"));
                //CoeusQA-2551:End
                protocolStudyGroup.setProcedureCategoryCode(hmStudyGroup.get("PROCEDURE_CATEGORY_CODE") == null ? 0 :
                    Integer.parseInt(hmStudyGroup.get("PROCEDURE_CATEGORY_CODE").toString()));
                protocolStudyGroup.setUpdateTimestamp((Timestamp)hmStudyGroup.get("UPDATE_TIMESTAMP"));
                protocolStudyGroup.setUpdateUser((String)hmStudyGroup.get("UPDATE_USER"));
                protocolStudyGroup.setLocations(
                        getStudyGroupLocations(protocolNumber,sequenceNumber,protocolStudyGroup.getStudyGroupId()));
                Vector vecOtherDetails =
                        getStudyGroupOthersDetails(protocolNumber,sequenceNumber,protocolStudyGroup.getStudyGroupId());
                if(vecOtherDetails != null && vecOtherDetails.size() > 0){
                    protocolStudyGroup.setOtherDetails(vecOtherDetails);
                    protocolStudyGroup.setOtherDetailsPresent(true);
                }else{
                    protocolStudyGroup.setOtherDetailsPresent(false);
                }
                //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -Start
                protocolStudyGroup.setPersonsResponsible(
                        getPersonsResponsibleData(protocolNumber,sequenceNumber,protocolStudyGroup.getStudyGroupId()
                        ,protocolStudyGroup.getSpeciesCode(),protocolStudyGroup.getProcedureCode()));
                //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -End
                vecStudyGroup.addElement(protocolStudyGroup);
            }
        }
    ...
    /*
     * Method to get the study group locations
     * @param protocolNumber - String
     * @param sequenceNumber - int
     * @param studyGroupId - int
     * @return vecStudyGroupLocation - Vector
     */
    public Vector getStudyGroupLocations(String protocolNumber,int sequenceNumber, int studyGroupId) throws DBException{
        Vector result = new Vector();
        Vector vecStudyGroupLocation = new Vector();
        Vector param= new Vector();
        ...
        param.addElement(new Parameter("STUDY_GROUP_ID",
                DBEngineConstants.TYPE_INTEGER,new Integer(studyGroupId)));

        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_STUDY_GROUP_LOCS ( <<PROTOCOL_NUMBER>>,<<SEQUENCE_NUMBER>>, <<STUDY_GROUP_ID>> , <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (result != null && result.size() >0){
            for(int index=0;index< result.size();index++){
                hmLocation = (HashMap)result.elementAt(index);
                ProtocolStudyGroupLocationBean locationBean = new ProtocolStudyGroupLocationBean();
                locationBean.setProtocolNumber((String)hmLocation.get("PROTOCOL_NUMBER"));
                locationBean.setSequenceNumber(Integer.parseInt(hmLocation.get("SEQUENCE_NUMBER").toString()));
                locationBean.setLocationId(hmLocation.get("LOCATION_ID") == null ? 0 :
                    Integer.parseInt(hmLocation.get("LOCATION_ID").toString()));
                locationBean.setLocationName((String)hmLocation.get("LOCATION_NAME"));
                locationBean.setLocationTypeCode(hmLocation.get("LOCATION_TYPE_CODE") == null ? 0 :
                    Integer.parseInt(hmLocation.get("LOCATION_TYPE_CODE").toString()));
                locationBean.setLocationTypeDesc((String)hmLocation.get("LOCATION_TYPE_DESCRIPTION"));
                locationBean.setStudyGroupId(hmLocation.get("STUDY_GROUP_ID") == null ? 0 :
                    Integer.parseInt(hmLocation.get("STUDY_GROUP_ID").toString()));
                locationBean.setStudyGroupLocationName((String)hmLocation.get("STUDY_GROUP_LOCATION_DESC"));
                // Added for COEUSQA-2625_Add another level of detail for Location in IACUC protocol_start
                locationBean.setLocationRoom((String)hmLocation.get("LOCATION_ROOM"));
                // Added for COEUSQA-2625_Add another level of detail for Location in IACUC protocol_end
                locationBean.setStudyGroupLocationId(hmLocation.get("STUDY_GROUP_LOCATION_ID") == null ? 0 :
                    Integer.parseInt(hmLocation.get("STUDY_GROUP_LOCATION_ID").toString()));
                locationBean.setUpdateTimestamp((Timestamp)hmLocation.get("UPDATE_TIMESTAMP"));
                locationBean.setUpdateUser((String)hmLocation.get("UPDATE_USER"));
                vecStudyGroupLocation.addElement(locationBean);
            }
        }
        return vecStudyGroupLocation;
    }

     /*
      * Method to get the study group other details(Custom elements)
      * @param protocolNumber - String
      * @param sequenceNumber - int
      * @param studyGroupId - int
      * @return vecOtherDetails - Vector
      */
    public Vector getStudyGroupOthersDetails(String protocolNumber, int sequenceNumber,int studyGroupId) throws CoeusException, DBException{
        Vector result = null;
        Vector param= new Vector();
        ProtocolCustomElementsInfoBean protocolCustomElementsInfoBean = null;
        HashMap othersRow = null;
        ...
        param.addElement(new Parameter("SEQUENCE_NUMBER",
                DBEngineConstants.TYPE_INT,new Integer(sequenceNumber).toString()));
        param.addElement(new Parameter("STUDY_GROUP_ID",
                DBEngineConstants.TYPE_INT,new Integer(studyGroupId).toString()));
        if(dbEngine !=null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_STUDY_CUSTOM_DATA( <<PROTOCOL_NUMBER>>, <<SEQUENCE_NUMBER>>, <<STUDY_GROUP_ID>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        int listSize = result.size();
        Vector vecOtherDetails = null;
        if (listSize > 0){
...
//                othersRow.get("DESCRIPTION"));
                ...
                othersRow.get("UPDATE_USER"));
                protocolCustomElementsInfoBean.setUpdateUser( (String)
                othersRow.get("AC_TYPE"));

                vecOtherDetails.add(protocolCustomElementsInfoBean);
            }
        }
        return vecOtherDetails;
    }

    public CoeusVector getSpeciesCodes() throws DBException{
        Vector result = new Vector(3,2);
        CoeusVector cvSepcies=null;
        HashMap hasSpecies=null;
        Vector param= new Vector();

        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_SPECIES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int typesCount = result.size();
        if (typesCount >0){
            cvSepcies = new CoeusVector();
            for(int types=0;types<typesCount;types++){
                hasSpecies = (HashMap)result.elementAt(types);
                cvSepcies.addElement(new ComboBoxBean(
                        hasSpecies.get("SPECIES_CODE").toString(),
                        hasSpecies.get("SPECIES_NAME").toString()));
            }
        }
        return cvSepcies;
    }
...
//    public Vector getProtocolSpecies(String protocolNumber)
//    throws DBException{
//
//        Vector result = new Vector(3,2);
//        Vector vecSpeciesData = null;
//        HashMap hmSpeciesData = null;
//        Vector param= new Vector();
//        param.addElement(new Parameter("PROTOCOL_NUMBER",
//                DBEngineConstants.TYPE_STRING,protocolNumber));
//
//        if(dbEngine!=null){
//            result = dbEngine.executeRequest("Coeus",
//                    "call GET_AC_PROT_SPECIES_FOR_MAXSEQ ( <<PROTOCOL_NUMBER>>,<<OUT RESULTSET rset>> )","Coeus", param);
//        }else{
//            throw new DBException("DB instance is not available");
//        }
//        int ctypesCount = result.size();
//        if (ctypesCount >0){
//            vecSpeciesData = new Vector();
//            for(int types=0;types<ctypesCount;types++){
//                hmSpeciesData = (HashMap)result.elementAt(types);
//                ProtocolSpeciesBean protocolSpeciesBean = new ProtocolSpeciesBean();
//
//                protocolSpeciesBean.setProtocolNumber((String)hmSpeciesData.get("PROTOCOL_NUMBER"));
//                protocolSpeciesBean.setSequenceNumber(Integer.parseInt(hmSpeciesData.get("SEQUENCE_NUMBER").toString()));
//                String strUscdaCovered = (String)hmSpeciesData.get("IS_USDA_COVERED");
//                protocolSpeciesBean.setUsdaCovered("Covered".equalsIgnoreCase(strUscdaCovered) ? true : false);
//                //SPECIES_GROUP col added with COEUSQA-2551:Rework how user enters species, study groups, and procedures in IACUC protocols
//                protocolSpeciesBean.setSpeciesGroupName((String)hmSpeciesData.get("SPECIES_GROUP"));
//                protocolSpeciesBean.setSpeciesCode(Integer.parseInt(hmSpeciesData.get("SPECIES_CODE").toString()));
//                protocolSpeciesBean.setSpeciesName((String)hmSpeciesData.get("SPECIES_NAME"));
//                if(hmSpeciesData.get("SPECIES_COUNT") != null){
//                    protocolSpeciesBean.setSpeciesCount(Integer.parseInt(hmSpeciesData.get("SPECIES_COUNT").toString()));
//                } else {
//                    protocolSpeciesBean.setSpeciesCount(0);
//                }
//                protocolSpeciesBean.setSpeciesId(Integer.parseInt(hmSpeciesData.get("SPECIES_ID").toString()));
//                protocolSpeciesBean.setStrain((String)hmSpeciesData.get("STRAIN"));
//
//                // Added for COEUSQA-2627_Move pain category in IACUC protocol to Protocol Species/Group level_start
//                protocolSpeciesBean.setPainCategoryCode(hmSpeciesData.get("PAIN_CATEGORY_CODE") == null ? 0 :
//                    Integer.parseInt(hmSpeciesData.get("PAIN_CATEGORY_CODE").toString()));
//                protocolSpeciesBean.setPainCategoryName((String)hmSpeciesData.get("PAIN_CATEGORY_NAME"));
//                // Added for COEUSQA-2627_Move pain category in IACUC protocol to Protocol Species/Group level_end
//
//                protocolSpeciesBean.setAwSpeciesId(Integer.parseInt(hmSpeciesData.get("SPECIES_ID").toString()));
//                protocolSpeciesBean.setUpdateUser((String)hmSpeciesData.get("UPDATE_USER"));
//                protocolSpeciesBean.setUpdateTimestamp((Timestamp)hmSpeciesData.get("UPDATE_TIMESTAMP"));
//
//                vecSpeciesData.addElement(protocolSpeciesBean);
//            }
//        }
//        return vecSpeciesData;
//    }




    public Vector getIacucProtocolPrinciples(String protocolNumber, int sequenceNumber)
    throws DBException{

        Vector result = new Vector(3,2);
        Vector vecprinciples = new Vector();
        HashMap hmPrinceiples = null;
        Vector param= new Vector();
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        param.addElement(new Parameter("SEQUENCE_NUMBER",
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_PRINCIPLES_DET ( <<PROTOCOL_NUMBER>>,  <<SEQUENCE_NUMBER>> ,<<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int ctypesCount = result.size();
        if (ctypesCount >0){
            // vecprinciples = new Vector();
            for(int types=0;types<ctypesCount;types++){
                hmPrinceiples = (HashMap)result.elementAt(types);
                ProtocolPrinciplesBean principlesBean = new ProtocolPrinciplesBean();

                principlesBean.setProtocolNumber((String)hmPrinceiples.get("PROTOCOL_NUMBER"));
                principlesBean.setSequenceNumber(Integer.parseInt(hmPrinceiples.get("SEQUENCE_NUMBER").toString()));

                principlesBean.setPrincipleOfReduction((String)hmPrinceiples.get("REDUCTION_PRINCIPLE"));
                principlesBean.setPrincipleOfRefinement((String)hmPrinceiples.get("REFINEMENT_PRINCIPLE"));
                principlesBean.setPrincipleOfReplacement((String)hmPrinceiples.get("REPLACEMENT_PRINCIPLE"));
                principlesBean.setExceptionPresent("Y".equalsIgnoreCase((String)hmPrinceiples.get("EXCEPTIONS_PRESENT")) ? true: false);


                principlesBean.setUpdateUser((String)hmPrinceiples.get("UPDATE_USER"));
                principlesBean.setUpdateTimestamp((Timestamp)hmPrinceiples.get("UPDATE_TIMESTAMP"));

                vecprinciples.addElement(principlesBean);
            }
        }
     ...
     /*
      * Method to get Species Exceptions
      * @param protocolNumber - String
      * @param sequenceNumber - int
      * @param speciesId - int
      * @return vecExceptions - Vector
      */
    public Vector getIacucProtocolExceptions(String protocolNumber, int sequenceNumber, int speciesId)
    throws DBException{

        Vector result = new Vector(3,2);
        Vector vecExceptions = new Vector();
        HashMap hmExceptions = null;
        Vector param= new Vector();
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        param.addElement(new Parameter("SEQUENCE_NUMBER",
        ...
        param.addElement(new Parameter("SPECIES_ID",
                DBEngineConstants.TYPE_INT,speciesId));

        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_EXCEPTIONS ( <<PROTOCOL_NUMBER>>,  <<SEQUENCE_NUMBER>>, <<SPECIES_ID>>, <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int ctypesCount = result.size();
        if (ctypesCount >0){
            for(int types=0;types<ctypesCount;types++){
                hmExceptions = (HashMap)result.elementAt(types);
                ProtocolExceptionBean exceptionsBean = new ProtocolExceptionBean();

                exceptionsBean.setProtocolNumber((String)hmExceptions.get("PROTOCOL_NUMBER"));
                exceptionsBean.setSequenceNumber(Integer.parseInt(hmExceptions.get("SEQUENCE_NUMBER").toString()));
                exceptionsBean.setSpeciesId(Integer.parseInt(hmExceptions.get("SPECIES_ID").toString()));
                exceptionsBean.setExceptionCategoryCode(Integer.parseInt(hmExceptions.get("EXCEPTION_CATEGORY_CODE").toString()));
                exceptionsBean.setExceptionDescription((String)hmExceptions.get("EXCEPTION_DESCRIPTION"));
                exceptionsBean.setExceptionId(Integer.parseInt(hmExceptions.get("EXCEPTION_ID").toString())) ;
                exceptionsBean.setExceptionCategoryDesc((String)hmExceptions.get("EXCEPTION_CATEGORY_DESC"));

                exceptionsBean.setUpdateUser((String)hmExceptions.get("UPDATE_USER"));
                exceptionsBean.setUpdateTimestamp((Timestamp)hmExceptions.get("UPDATE_TIMESTAMP"));

                vecExceptions.addElement(exceptionsBean);
            }
        }
        return vecExceptions;
    }

    public Vector getIacucProtocolAlterDBSearches(String protocolNumber, int sequenceNumber)
    throws DBException{

        Vector result = new Vector(3,2);
        Vector vecAlterSearches = new Vector();
        //Added for COEUSQA-2714 In the Alternative Search in IACUC-Start
        Vector vecAlterDatabaseSearches = new Vector();
        //Added for COEUSQA-2714 In the Alternative Search in IACUC-End
        HashMap hmAlterSearches = null;
        Vector param= new Vector();
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        param.addElement(new Parameter("SEQUENCE_NUMBER",
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_ALTER_SEARCHES ( <<PROTOCOL_NUMBER>>,  <<SEQUENCE_NUMBER>> ,<<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int ctypesCount = result.size();
        if (ctypesCount >0){
            //vecAlterSearches = new Vector();
            for(int types=0;types<ctypesCount;types++){
                hmAlterSearches = (HashMap)result.elementAt(types);
                ProtocolAlternativeSearchBean alternativeSearchBean = new ProtocolAlternativeSearchBean();

                alternativeSearchBean.setProtocolNumber((String)hmAlterSearches.get("PROTOCOL_NUMBER"));
                alternativeSearchBean.setSequenceNumber(Integer.parseInt(hmAlterSearches.get("SEQUENCE_NUMBER").toString()));
                alternativeSearchBean.setAlternativeSearchId(Integer.parseInt(hmAlterSearches.get("ALTERNATIVE_SEARCH_ID").toString()));
                alternativeSearchBean.setDatabaseSearchedCode(Integer.parseInt(hmAlterSearches.get("ALT_DB_SEARCH_ID").toString()));
                alternativeSearchBean.setComments((String)hmAlterSearches.get("COMMENTS"));
                alternativeSearchBean.setKeyWordsSearched((String)hmAlterSearches.get("KEYWORDS_SEARCHED"));
                alternativeSearchBean.setYearsSearched((String)hmAlterSearches.get("YEARS_SEARCHED"));
                alternativeSearchBean.setDatabaseSeartched((String)hmAlterSearches.get("DESCRIPTION"));
                alternativeSearchBean.setSearchDate( hmAlterSearches.get("SEARCH_DATE") == null ? null :
                    new Date( ((Timestamp) hmAlterSearches.get(
                        "SEARCH_DATE")).getTime()) );

                alternativeSearchBean.setAwAlternativeSearchId(Integer.parseInt(hmAlterSearches.get("ALTERNATIVE_SEARCH_ID").toString()));
                alternativeSearchBean.setUpdateUser((String)hmAlterSearches.get("UPDATE_USER"));
                alternativeSearchBean.setUpdateTimestamp((Timestamp)hmAlterSearches.get("UPDATE_TIMESTAMP"));
                //Added for COEUSQA-2714 In the Alternative Search in IACUC-Start
                vecAlterDatabaseSearches = getIacucProtocolAltSearchesDB(alternativeSearchBean.getProtocolNumber(),
                        alternativeSearchBean.getSequenceNumber(),alternativeSearchBean.getAlternativeSearchId());
                if(vecAlterDatabaseSearches !=null && vecAlterDatabaseSearches.size()>0){
                    ProtocolAlterDatabaseSearchBean protocolAltDatabaseSearch = new ProtocolAlterDatabaseSearchBean();
                    int index = 0;
                    StringBuilder dataBaseSearched = new StringBuilder();
                    for(Object obj:vecAlterDatabaseSearches){
                        protocolAltDatabaseSearch = (ProtocolAlterDatabaseSearchBean)obj;
                        protocolAltDatabaseSearch.setSelected(true);
                        if(index !=0 && index < vecAlterDatabaseSearches.size()){
                            dataBaseSearched.append(", ");
                            dataBaseSearched.append(protocolAltDatabaseSearch.getDescription());
                        }else{
                            dataBaseSearched.append(protocolAltDatabaseSearch.getDescription());
                        }
                        index++;
                    }
                    alternativeSearchBean.setDatabaseSeartched(dataBaseSearched.toString());
                    alternativeSearchBean.setChkDatabaseSearchedCode(vecAlterDatabaseSearches);
                    alternativeSearchBean.setChkDatabaseOldSearchedCode(vecAlterDatabaseSearches);
                }
                //Added for COEUSQA-2714 In the Alternative Search in IACUC-End

                vecAlterSearches.addElement(alternativeSearchBean);
            }
        }
        return vecAlterSearches;
    }

    /**
     * Method to get Exception Category types
     * @return vecProcCategory - Vector
     *
     */
    public CoeusVector getIacucProtocolExceptionsCategory() throws DBException{
        Vector result = new Vector();
        CoeusVector vecExceptionCategory = new CoeusVector();
        HashMap hmExceptionCategoryCode=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_EXCEPTION_CATEGORY ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmExceptionCategoryCode = (HashMap)result.elementAt(types);
                vecExceptionCategory.addElement(new ComboBoxBean(
                        hmExceptionCategoryCode.get("EXCEPTION_CATEGORY_CODE").toString(),
                        hmExceptionCategoryCode.get("EXCEPTION_CATEGORY_DESC").toString()));
            }
        }
        return vecExceptionCategory;
    }

    /**
     * This method populates the list box meant to retrieve the project Type
     * in the protocol detail screen.
     * <li>To fetch the data, it uses the procedure get_protocol_types.
     *
     * @return Vector map of all protocol type as key
     ...
     * @exception DBException if the instance of a dbEngine is null.
     */
    public Vector getProjectTypes() throws DBException{
        Vector result = new Vector(3,2);
        Vector vecProtocolTypes=null;
        HashMap hasProtcolType=null;
        Vector param= new Vector();
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_PROJECT_TYPE ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            ...
            for(int types=0;types<typesCount;types++){
                hasProtcolType = (HashMap)result.elementAt(types);
                vecProtocolTypes.addElement(new ComboBoxBean(
                        hasProtcolType.get("PROJECT_TYPE_CODE").toString(),
                        hasProtcolType.get("DESCRIPTION").toString()));
            }
        }
        return vecProtocolTypes;
    ...
    /**
     * Method to get the person role types
     * @exception DBException if the instance of a dbEngine is null.
     */
    public Vector getPersonRoleTypes() throws DBException{
        Vector result = new Vector();
        Vector vecPersonRoleTypes = new Vector();
        HashMap hasAffiliationType=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_KEY_PER_ROLE ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hasAffiliationType = (HashMap)result.elementAt(types);
                vecPersonRoleTypes.addElement(new ComboBoxBean(
                        hasAffiliationType.get("KEY_PERSON_ROLE_CODE").toString(),
                        hasAffiliationType.get("DESCRIPTION").toString()));
            }
        }
        return vecPersonRoleTypes;
    }

    /**
     * Method to get procedure category types
     * @return cvProcCategory - CoeusVector
     *
     */
    public CoeusVector getProcedureCategoryTypes() throws DBException{
        Vector result = new Vector();
        CoeusVector cvProcCategory = new CoeusVector();
        HashMap hmProcCategoryCode=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROCEDURE_CATEGORY ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmProcCategoryCode = (HashMap)result.elementAt(types);
                cvProcCategory.addElement(new ComboBoxBean(
                        hmProcCategoryCode.get("PROCEDURE_CATEGORY_CODE").toString(),
                        hmProcCategoryCode.get("PROCEDURE_CATEGORY").toString()));
            }
        }
        return cvProcCategory;
    }


    /**
     * Method to get procedure types
     *
     *
     * @return vecProcedureType - Vector
     */
    public Vector getProcedureTypes() throws DBException{
        Vector result = new Vector();
        Vector vecProcedureType = new Vector();
        HashMap hmProcedureType=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROCEDURES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmProcedureType = (HashMap)result.elementAt(types);
                vecProcedureType.addElement(new ComboBoxBean(
                        hmProcedureType.get("PROCEDURE_CATEGORY_CODE").toString(),
                        hmProcedureType.get("PROCEDURE_CATEGORY").toString()));
            }
        }
        return vecProcedureType;
    }


    public Vector getProtocolSpecies(String protocolNumber, int sequenceNumber)throws DBException{
        Vector result = new Vector();
        Vector vecProtocolSpecies = new Vector();
        HashMap hmProtocolSpecies = null;
        Vector param= new Vector();
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        param.addElement(new Parameter("SEQUENCE_NUMBER",
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTOCOL_SPECIES ( <<PROTOCOL_NUMBER>>,  <<SEQUENCE_NUMBER>> ,<<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int ctypesCount = result.size();
        if (ctypesCount >0){
            for(int types=0;types<ctypesCount;types++){
                hmProtocolSpecies = (HashMap)result.elementAt(types);

                ProtocolSpeciesBean protocolSpeciesDetail = new ProtocolSpeciesBean();
                protocolSpeciesDetail.setProtocolNumber((String)hmProtocolSpecies.get("PROTOCOL_NUMBER"));
                protocolSpeciesDetail.setSequenceNumber(Integer.parseInt(hmProtocolSpecies.get("SEQUENCE_NUMBER").toString()));
                if(hmProtocolSpecies.get("IS_USDA_COVERED") != null
                        && hmProtocolSpecies.get("IS_USDA_COVERED").toString().equalsIgnoreCase("Covered")){
                    protocolSpeciesDetail.setUsdaCovered(true);
                }else{
                    protocolSpeciesDetail.setUsdaCovered(false);
                }
                //SPECIES_GROUP col added with COEUSQA-2551:Rework how user enters species, study groups, and procedures in IACUC protocols
                protocolSpeciesDetail.setSpeciesGroupName((String)hmProtocolSpecies.get("SPECIES_GROUP"));
                protocolSpeciesDetail.setSpeciesCode(Integer.parseInt(hmProtocolSpecies.get("SPECIES_CODE").toString()));
                protocolSpeciesDetail.setSpeciesName((String)hmProtocolSpecies.get("SPECIES_NAME"));
                if(hmProtocolSpecies.get("SPECIES_COUNT") != null){
                    protocolSpeciesDetail.setSpeciesCount(Integer.parseInt(hmProtocolSpecies.get("SPECIES_COUNT").toString()));
                }
                // Added for COEUSQA-2627_Move pain category in IACUC protocol to Protocol Species/Group level_start
                protocolSpeciesDetail.setPainCategoryCode(hmProtocolSpecies.get("PAIN_CATEGORY_CODE") == null ? 0 :
                    Integer.parseInt(hmProtocolSpecies.get("PAIN_CATEGORY_CODE").toString()));
                protocolSpeciesDetail.setPainCategoryName(hmProtocolSpecies.get("PAIN_CATEGORY_NAME") == null ? "":
                    (String)hmProtocolSpecies.get("PAIN_CATEGORY_NAME"));
                // Added for COEUSQA-2627_Move pain category in IACUC protocol to Protocol Species/Group level_end
                protocolSpeciesDetail.setSpeciesId(Integer.parseInt(hmProtocolSpecies.get("SPECIES_ID").toString()));
                protocolSpeciesDetail.setStrain((String)hmProtocolSpecies.get("STRAIN"));
                protocolSpeciesDetail.setUpdateUser((String)hmProtocolSpecies.get("UPDATE_USER"));
                protocolSpeciesDetail.setUpdateTimestamp((Timestamp)hmProtocolSpecies.get("UPDATE_TIMESTAMP"));
                //Added for-COEUSQA-2798 Add count type to species/group screen-Start
                protocolSpeciesDetail.setSpeciesCountTypeCode(hmProtocolSpecies.get("SPECIES_COUNT_CODE") == null ? 0 :
                    Integer.parseInt(hmProtocolSpecies.get("SPECIES_COUNT_CODE").toString()));
                protocolSpeciesDetail.setSpeciesCountTypeName(hmProtocolSpecies.get("DESCRIPTION") == null ? "":
                    (String)hmProtocolSpecies.get("DESCRIPTION"));
                //Added for-COEUSQA-2798 Add count type to species/group screen-End

                //COEUSQA-2724-Exceptions should be moved from Scientific Justification in IACUC
                if(hmProtocolSpecies.get("EXCEPTIONS_PRESENT") != null
                        && hmProtocolSpecies.get("EXCEPTIONS_PRESENT").toString().equalsIgnoreCase("Y")){
                    protocolSpeciesDetail.setExceptionsPresent(true);
                }else{
                    protocolSpeciesDetail.setExceptionsPresent(false);
                }

                protocolSpeciesDetail.setSpeciesExceptions(
                        getIacucProtocolExceptions(protocolSpeciesDetail.getProtocolNumber(),
                                                     protocolSpeciesDetail.getSequenceNumber(),
                                                     protocolSpeciesDetail.getSpeciesId()));
                vecProtocolSpecies.addElement(protocolSpeciesDetail);
    ...
    public Vector getProtocolSpeciesType(String protocolNumber, int sequenceNumber)throws DBException{
        Vector vecProtoSpecies =  getProtocolSpecies(protocolNumber,sequenceNumber);
        Vector vecProtocolSpeciesType = new Vector();
        if(vecProtoSpecies != null && vecProtoSpecies.size() > 0){
            for(int index = 0; index <vecProtoSpecies.size();index++){
                ProtocolSpeciesBean protocolSpeciesDetail =  (ProtocolSpeciesBean)vecProtoSpecies.get(index);
                vecProtocolSpeciesType.add(new ComboBoxBean(
                        protocolSpeciesDetail.getSpeciesCode()+"",
                        protocolSpeciesDetail.getSpeciesName()));
            }
        }
        return vecProtocolSpeciesType;
    }

    // IACUC Modification End
    public CoeusVector getAltSearchDB() throws DBException{

        Vector result = new Vector();
        CoeusVector vecAltSearchDB = new CoeusVector();
        HashMap hmAltSearchDB=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_ALT_DB_SEARCH ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmAltSearchDB = (HashMap)result.elementAt(types);
                vecAltSearchDB.addElement(new ComboBoxBean(
                        hmAltSearchDB.get("ALT_DB_SEARCH_ID").toString(),
                        hmAltSearchDB.get("DESCRIPTION").toString()));
            }
        }
        return vecAltSearchDB;
    }

    /*
     * Method to get the study group other details(Custom elements) for procedure category code
     * @param procedureCategoryCode - int
     * @return vecOtherDetails - Vector
     */
    public Vector getProcedureCatCustomData(int procedureCategoryCode)
    throws CoeusException, DBException{
        Vector result = null;
        Vector param= new Vector();
        ProtocolCustomElementsInfoBean protocolCustomElementsInfoBean = null;
        ...
        param.addElement(new Parameter("PROCEDURE_CATEGORY_CODE",
                DBEngineConstants.TYPE_INT,new Integer(procedureCategoryCode).toString()));
        if(dbEngine !=null){
            result = new Vector(3,2);
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROC_CAT_CUSTOM_DATA(<<PROCEDURE_CATEGORY_CODE>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        ...
        if (listSize > 0){
            ...
            for(int rowIndex=0; rowIndex<listSize; rowIndex++){
                ...
                protocolCustomElementsInfoBean.setUpdateUser( (String)
                othersRow.get("UPDATE_USER"));
                protocolCustomElementsInfoBean.setAcType(TypeConstants.INSERT_RECORD);
                protocolCustomElementsInfoBean.setNewCustomElement(true);
                vecOtherDetails.add(protocolCustomElementsInfoBean);
            }
        }
        return vecOtherDetails;
    ...
    /**
     * Method to get Study group location type
     * @return vcLocationType - CoeusVector
     */
    public CoeusVector getStudyGroupLocationTypes() throws DBException{
        Vector result = new Vector();
        CoeusVector vcLocationType = new CoeusVector();
        HashMap hmProcedureType=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_LOCATION_TYPES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmProcedureType = (HashMap)result.elementAt(types);
                vcLocationType.addElement(new ComboBoxBean(
                        hmProcedureType.get("LOCATION_TYPE_CODE").toString(),
                        hmProcedureType.get("LOCATION").toString()));
            }
        }
        return vcLocationType;
    }

    /**
     * Method to get Study group location name
     *
     * @return cvLocationName - CoeusVector
     */
    public CoeusVector getStudyGroupLocationNames() throws DBException{
        Vector result = new Vector();
        CoeusVector cvLocationName = new CoeusVector();
        HashMap hmProcedureType=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_LOCATION_NAMES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmProcedureType = (HashMap)result.elementAt(types);
                cvLocationName.addElement(new ComboBoxBean(
                        hmProcedureType.get("LOCATION_ID").toString(),
                        hmProcedureType.get("LOCATION_NAME").toString()));
            }
        }
        return cvLocationName;
    }


    /**
     * Method to get Study group pain categories
     * @return cvPainCategory - CoeusVector
     */
    public CoeusVector getStudyGroupPainCategory() throws DBException{
        Vector result = new Vector();
        CoeusVector cvPainCategory = new CoeusVector();
        HashMap hmProcedureType=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PAIN_CATEGORIES ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmProcedureType = (HashMap)result.elementAt(types);
                cvPainCategory.addElement(new ComboBoxBean(
                        hmProcedureType.get("PAIN_CATEGORY_CODE").toString(),
                        hmProcedureType.get("PAIN_CATEGORY").toString()));
            }
        }
        return cvPainCategory;
    }
     ...
     * @return cvPainCategory - CoeusVector
     */
    public CoeusVector getSpeciesCountType() throws DBException{
        Vector result = new Vector();
        CoeusVector cvSpeciesCountType = new CoeusVector();
        HashMap hmSpeciesCountType=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_SPECIES_COUNT_TYPE ( <<OUT RESULTSET rset>> )",
                    "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmSpeciesCountType = (HashMap)result.elementAt(types);
                cvSpeciesCountType.addElement(new ComboBoxBean(
                        hmSpeciesCountType.get("SPECIES_COUNT_CODE").toString(),
                        hmSpeciesCountType.get("DESCRIPTION").toString()));
            }
        }
        return cvSpeciesCountType;
    }
    ...
    /**
     * Method to get Study group pain categories
     * @return cvProcedure - CoeusVector
     */
    public CoeusVector getProcedureForCategory(int procedureCategoryCode) throws DBException{
        Vector result = new Vector();
        CoeusVector cvProcedure = new CoeusVector();
        HashMap hmProcedure=null;
        Vector param= new Vector();
        param.addElement(new Parameter("PROCEDURE_CATEGORY_CODE",
                DBEngineConstants.TYPE_INT,new Integer(procedureCategoryCode).toString()));
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROCEDURES_FOR_PROC_CAT( <<PROCEDURE_CATEGORY_CODE>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmProcedure = (HashMap)result.elementAt(types);
                cvProcedure.addElement(new ComboBoxBean(
                        hmProcedure.get("PROCEDURE_CODE").toString(),
                        hmProcedure.get("PROCEDURE_DESCRIPTION").toString()));
            }
        }
        return cvProcedure;
    }
    ...
    // IACUC Modification End
    //COEUSQA-1724-Implemented for new Amendment type - Start
    /*The Method checkFunctiontype is use to check the function type for Amendment/Renewal
     *@return isfunctionTypeTrue true/false.
     */
    private boolean checkFunctiontype(){
        boolean isfunctionTypeTrue = false;
        if(functionType != NEW_AMMENDMENT
        && functionType != NEW_REVISION
        && functionType != NEW_AMMENDMENT_REVISION
        && functionType != NEW_CONTINUATION_REVIEW
        && functionType != NEW_CONTINUATION_REVIEW_AMEND)
        {
            isfunctionTypeTrue = true;
        }
        return isfunctionTypeTrue;
    }

    //COEUSQA-1724-Implemented for new Amendment type - End

    //Added for-COEUSQA-2508 Some schools do not use pain category-Start
    /**
     *  This method used get the pain category parameter value
     *  <li>To fetch the data, it uses the function GET_PARAMETER_VALUE.
     *
     *  @return String rparameter value
     *  @param String protocol number  and sequence number
     *  @exception DBException if any error during database transaction.
     *  @exception CoeusException if the instance of dbEngine is not available.
     *  @return String value
     */
    public String getPainCategoryParameterValue()
    throws CoeusException, DBException {
        String parameterValue = "";
        Vector param= new Vector();
        Vector result = new Vector();
        param.add(new Parameter("PARAMETER",
                DBEngineConstants.TYPE_STRING,"IACUC_IS_PAIN_CATEGORY_MANDATORY"));

        /* calling stored function */
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                "{<<OUT STRING MOD_VALUE>>=call GET_PARAMETER_VALUE ( "
                + " << PARAMETER >>)}", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
        if(!result.isEmpty()){
            HashMap rowParameter = (HashMap)result.elementAt(0);
            parameterValue = rowParameter.get("MOD_VALUE").toString();
        }
     ...
     *  @return cvIacucCorrForOrg - Vector List of default organization correspondence
     */
    public CoeusVector getIacucCorrespForOrg(Vector organizationID)
    throws CoeusException, DBException{
        Vector result = new Vector();
        CoeusVector cvIacucCorrForOrg = new CoeusVector();
        HashMap hmIacucOrgCorr=null;
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_ORG_CORRESPONDENCE( <<AV_ORGANIZATION_ID>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                ...
                hmIacucOrgCorr = (HashMap)result.elementAt(types);
                protocolCorrespondentsBean.setPersonId((String) hmIacucOrgCorr.get("PERSON_ID"));
                protocolCorrespondentsBean.setPersonName((String) hmIacucOrgCorr.get("PERSON_NAME"));
                protocolCorrespondentsBean.setNonEmployeeFlag(hmIacucOrgCorr.get("NON_EMPLOYEE_FLAG") == null ? false:
                    (hmIacucOrgCorr.get("NON_EMPLOYEE_FLAG").toString().equalsIgnoreCase("Y") ? true :false));
                protocolCorrespondentsBean.setCorrespondentTypeCode(hmIacucOrgCorr.get("CORRESPONDENT_TYPE_CODE") == null ? 0:
                    Integer.parseInt(hmIacucOrgCorr.get("CORRESPONDENT_TYPE_CODE").toString()));
                protocolCorrespondentsBean.setComments((String) hmIacucOrgCorr.get("COMMENTS"));
                cvIacucCorrForOrg.add(protocolCorrespondentsBean);
            }
        }
        }
        }
      return cvIacucCorrForOrg;
    }

    /**
     *  This method used get the Default Unit correspondence
     *  @param unitLists - Vector List of Unit number
     *  @return cvIacucCorrForUnit - Vector List of default Unit correspondence
     */
    public CoeusVector getIacucCorrespForUnit(Vector unitLists)
    throws CoeusException, DBException{
        Vector result = new Vector();
        CoeusVector cvIacucCorrForUnit = new CoeusVector();
        HashMap hmIacucOrgCorr=null;
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_UNIT_CORRESPONDENCE( <<AV_UNIT_NUMBER>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                ...
                protocolCorrespondentsBean.setCorrespondentTypeCode(hmIacucOrgCorr.get("CORRESPONDENT_TYPE_CODE") == null ? 0:
                    Integer.parseInt(hmIacucOrgCorr.get("CORRESPONDENT_TYPE_CODE").toString()));
                protocolCorrespondentsBean.setComments((String) hmIacucOrgCorr.get("COMMENTS"));
                cvIacucCorrForUnit.add(protocolCorrespondentsBean);
            }
        }
        }
        }
      return cvIacucCorrForUnit;
    }
     ...
     */
    public int isIacucProtocolCorrExist(ProtocolCorrespondentsBean protocolCorrespondentsBean) throws CoeusException,DBException{
        ...
        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{ <<OUT INTEGER SUCCESS>> = "
                    +" call FN_CHECK_AC_PROTO_CORR_EXIST(<< AS_PROTOCOL_NUMBER >> ,<< AS_SEQUENCE_NUMBER >> ,<<AS_CORRESPONDENT_TYPE_CODE>>, <<AS_PERSON_ID>> ) }", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
   ...
   //Added for COEUSQA-2714 In the Alternative Search in IACUC-Start
    public Vector getIacucProtocolAltSearchesDB(String protocolNumber, int sequenceNumber, int alternativeSearchId)
    throws DBException{

        Vector result = new Vector(3,2);
        Vector vecAlterSearchDB = new Vector();
        HashMap hmAlterSearches = null;
        Vector param= new Vector();
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        ...
        param.addElement(new Parameter("ALTERNATIVE_SEARCH_ID",
                DBEngineConstants.TYPE_INT,alternativeSearchId));

        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROTO_ALT_SEARCH_DB ( <<PROTOCOL_NUMBER>>,  <<SEQUENCE_NUMBER>>, <<ALTERNATIVE_SEARCH_ID>>, <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        int ctypesCount = result.size();
        if (ctypesCount >0){
             for(int types=0;types<ctypesCount;types++){
                hmAlterSearches = (HashMap)result.elementAt(types);
                ProtocolAlterDatabaseSearchBean alternativeDatabaseSearchBean = new ProtocolAlterDatabaseSearchBean();
                alternativeDatabaseSearchBean.setProtocolNumber((String)hmAlterSearches.get("PROTOCOL_NUMBER"));
                alternativeDatabaseSearchBean.setSequenceNumber(Integer.parseInt(hmAlterSearches.get("SEQUENCE_NUMBER").toString()));
                alternativeDatabaseSearchBean.setDatabaseSearchedCode(Integer.parseInt(hmAlterSearches.get("ALT_DB_SEARCH_ID").toString()));
                alternativeDatabaseSearchBean.setDescription((String) hmAlterSearches.get("DESCRIPTION"));
                vecAlterSearchDB.addElement(alternativeDatabaseSearchBean);
        }
        }
        return vecAlterSearchDB;
    }
   //Added for COEUSQA-2714 In the Alternative Search in IACUC-End
    ...
    throws CoeusException, DBException {
        ...
        if(dbEngine !=null){
            moduleResult = new Vector(3,2);
            moduleResult = dbEngine.executeRequest("Coeus",
                    "call GET_AC_AMEND_PROTOCOL_DOCUMENT( <<AV_PROTOCOL_NUMBER>>, <<AV_SEQUENCE_NUMBER>>, <<AV_DOCUMENT_ID>>,"+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    //Added for COEUSQA-2633 Ability to indicate which procedures IACUC study personnel will perform and training -Start
    /*
     * Method to get the study Persons responsible
     * @param protocolNumber - String
     * @param sequenceNumber - int
     * @param studyGroupId - int
     * @throws DBException, CoeusException
     ...
     */
    public Vector getPersonsResponsibleData(String protocolNumber,int sequenceNumber, int studyGroupId, int speciesCode, int procedureCode) throws DBException, CoeusException{
        Vector result = new Vector();
        Vector vecPersonsResponsible = new Vector();
        Vector param= new Vector();
        HashMap hmPersonResponsible = null;
        param.addElement(new Parameter("PROTOCOL_NUMBER",
                DBEngineConstants.TYPE_STRING,protocolNumber));
        param.addElement(new Parameter("SEQUENCE_NUMBER",
                DBEngineConstants.TYPE_INTEGER,new Integer(sequenceNumber)));
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PROC_PERSON_RESPONSIBLE ( <<PROTOCOL_NUMBER>>,<<SEQUENCE_NUMBER>>, <<STUDY_GROUP_ID>>  , <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (result != null && result.size() >0){
            for(int index=0;index< result.size();index++){
                hmPersonResponsible = (HashMap)result.elementAt(index);
                ProtocolPersonsResponsibleBean personsResponsibleBean = new ProtocolPersonsResponsibleBean();
                personsResponsibleBean.setProtocolNumber((String)hmPersonResponsible.get("PROTOCOL_NUMBER"));
                personsResponsibleBean.setSequenceNumber(Integer.parseInt(hmPersonResponsible.get("SEQUENCE_NUMBER").toString()));
                personsResponsibleBean.setStudyGroupId(hmPersonResponsible.get("STUDY_GROUP_ID") == null ? 0 :
                    Integer.parseInt(hmPersonResponsible.get("STUDY_GROUP_ID").toString()));
                personsResponsibleBean.setPersonId((String)hmPersonResponsible.get("PERSON_ID"));
                // Added for COEUSQA-3694 : Coeus4.5: IACUC Procedure Personnel Issue Remains - Start
                personsResponsibleBean.setNonEmployeeFlag(hmPersonResponsible.get(
                        "NON_EMPLOYEE_FLAG") == null ? false :
                            (hmPersonResponsible.get("NON_EMPLOYEE_FLAG").toString()
                            .equalsIgnoreCase("Y") ? true :false));
                // Added for COEUSQA-3694 : Coeus4.5: IACUC Procedure Personnel Issue Remains - End
                if(getSpeciesTrainingFlag(personsResponsibleBean.getPersonId(), speciesCode, procedureCode)){
                personsResponsibleBean.setTrained(true);
                }else{
                personsResponsibleBean.setTrained(false);
                }
                personsResponsibleBean.setPersonName((String)hmPersonResponsible.get("FULL_NAME"));
                personsResponsibleBean.setUpdateTimestamp((Timestamp)hmPersonResponsible.get("UPDATE_TIMESTAMP"));
                personsResponsibleBean.setUpdateUser((String)hmPersonResponsible.get("UPDATE_USER"));
                vecPersonsResponsible.addElement(personsResponsibleBean);
            }
        }
    ...
    /*
     * Method to get the study Persons specied training status
     ...
     */
    public boolean getSpeciesTrainingFlag(String personId, int speciesCode, int procedureCode) throws CoeusException,DBException{
        boolean success = false ;
        // get the new seq id and do the updation
        Vector param= new Vector();
        ...
        param.add(new Parameter("AS_PERSON_ID",
                DBEngineConstants.TYPE_STRING, personId )) ;
        param.add(new Parameter("AS_SPECIES_CODE",
                DBEngineConstants.TYPE_INT, speciesCode )) ;
        param.add(new Parameter("AS_PROCEDURE_CODE",
                DBEngineConstants.TYPE_INT, procedureCode )) ;

        if(dbEngine!=null){
            result = dbEngine.executeFunctions("Coeus",
                    "{<<OUT INTEGER SUCCESS>> = call FN_GET_SPECIES_TRAINING_STATUS( "
                    + " << AS_PERSON_ID >>,<<AS_SPECIES_CODE>>, <<AS_PROCEDURE_CODE>> )}", param) ;
        }else{
            throw new CoeusException("db_exceptionCode.1000");
        }
    ...
    /*
     * Method to get the Persons training details
     * @param personId - String
     * @return vecPersonsResponsible - Vector
     * @throws DBException, CoeusException
     */
    public Vector getSpeciesTrainingDetails(String personId) throws DBException, CoeusException{
        Vector result = new Vector();
        Vector vecPersonsResponsible = new Vector();
        Vector param= new Vector();
        HashMap hmPersonResponsible = null;
        ...
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_PERSON_TRAINING_DETAILS ( <<PERSON_ID>> , <<OUT RESULTSET rset>> )","Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        if (result != null && result.size() >0){
            for(int index=0;index< result.size();index++){
                hmPersonResponsible = (HashMap)result.elementAt(index);
                PersonResponsibleTrainingDetailBean personBean = new PersonResponsibleTrainingDetailBean();
                personBean.setTrainingDescription((String)hmPersonResponsible.get("DESCRIPTION"));
                personBean.setSpeciesType((String)hmPersonResponsible.get("SPECIES"));
                personBean.setProcedureType((String)hmPersonResponsible.get("PROCEDURE"));
                personBean.setDateRequested( hmPersonResponsible.get("DATE_REQUESTED") == null ? null :
                    new Date( ((Timestamp) hmPersonResponsible.get(
                        "DATE_REQUESTED")).getTime()) );
                personBean.setDateSubmitted( hmPersonResponsible.get("DATE_SUBMITTED") == null ? null :
                    new Date( ((Timestamp) hmPersonResponsible.get(
                        "DATE_SUBMITTED")).getTime()) );
                personBean.setDateAcknowledge( hmPersonResponsible.get("DATE_ACKNOWLEDGED") == null ? null :
                    new Date( ((Timestamp) hmPersonResponsible.get(
                        "DATE_ACKNOWLEDGED")).getTime()) );
                personBean.setFollowupDate( hmPersonResponsible.get("FOLLOWUP_DATE") == null ? null :
                    new Date( ((Timestamp) hmPersonResponsible.get(
                        "FOLLOWUP_DATE")).getTime()) );
                //COEUSQA:3537 - Coeus IACUC Training Revisions - Start
                personBean.setProcedureCategory((String) hmPersonResponsible.get("PROCEDURE_CATEGORY"));
                //COEUSQA:3537 - End
                vecPersonsResponsible.addElement(personBean);
            }
        }
        return vecPersonsResponsible;
    }
     ...
     * @return cvLocationtype - CoeusVector
     *
     */
    public CoeusVector getLocationTypes() throws DBException{
        Vector result = new Vector();
        CoeusVector cvLocationtype = new CoeusVector();
        HashMap hmLocationTypeCode=null;
        Vector param= new Vector();
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_LOCATION_TYPES ( <<OUT RESULTSET rset>> )",
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmLocationTypeCode = (HashMap)result.elementAt(types);
                cvLocationtype.addElement(new ComboBoxBean(
                        hmLocationTypeCode.get("LOCATION_TYPE_CODE").toString(),
                        hmLocationTypeCode.get("LOCATION").toString()));
            }
        }
    ...
    /**
     * Method to get Location type for locationtype
     * @return cvLocation - CoeusVector
     */
    public CoeusVector getLocationForType(int locationTypeCode) throws DBException{
        Vector result = new Vector();
        CoeusVector cvLocation = new CoeusVector();
        HashMap hmLocation=null;
        Vector param= new Vector();
        param.addElement(new Parameter("LOCATION_TYPE_CODE",
                DBEngineConstants.TYPE_INT,new Integer(locationTypeCode).toString()));
        if(dbEngine!=null){
            result = dbEngine.executeRequest("Coeus",
                    "call GET_AC_LOCATIONS_FOR_LOC_TYPE( <<LOCATION_TYPE_CODE>>, "+
                    "<<OUT RESULTSET rset>> )", "Coeus", param);
        }else{
            throw new DBException("DB instance is not available");
        }
        ...
        if (typesCount >0){
            for(int types=0;types<typesCount;types++){
                hmLocation = (HashMap)result.elementAt(types);
                cvLocation.addElement(new ComboBoxBean(
                        hmLocation.get("LOCATION_ID").toString(),
                        hmLocation.get("LOCATION_NAME").toString()));
            }
        }
        return cvLocation;
    }
