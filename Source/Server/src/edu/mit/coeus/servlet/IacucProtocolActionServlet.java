/*
 * @(#)IacucProtocolActionServlet.java 1.0 11/25/02
 *
 ...
 *
 */
public class IacucProtocolActionServlet extends CoeusBaseServlet {
...
//                    }
                ...
                // IACUC Parent case - Correspondence realted changes
            ...
            //prps end dec 15 2003
					...
					if (canUndo == 1) {
                        ...
                        String errMsg = coeusMessageResourcesBean.parseMessageKey(
                                                            "iacucUndoLastAction_exceptionCode." + canUndo);
                        ...
                        if ( (canUndo == 5091) || (canUndo == 5092) || (canUndo == 5093) ) {
                            responder.setDataObject(DIALOG_TYPE_POP_UP_FOR_ACTION);
                        }
                        //Addedfor COEUSQA-3144 : IRB and IACUC - Add warning to Undo Last Action that review comments will be lost - end
		}
                                //For the IRB Enhancement Assigning to agenda as the follow up action to Submitted To IRB step:2        
				}
            //Added for COEUSQA-3144 : IRB and IACUC - Add warning to Undo Last Action that review comments will be lost - start
                        else if (requester.getFunctionType() == UNDO_LAST_ACTION_CONFIRMATION) {
                String protocolId = requester.getId();
                ActionTransaction axnTxn = new ActionTransaction();
                ProtocolInfoBean protocolInfoBean;
                ProtocolDataTxnBean protocolDataTxnBean = new ProtocolDataTxnBean();
...
//            UtilFactory = new UtilFactory();
            ...
            { //No Action Rights message
                ...
                String errMsg = 
                coeusMessageResourcesBean.parseMessageKey(
                            "iacucProtocolAction_exceptionCode." + 5000);
                //Add action Description as well 
                responder.setMessage(errMsg + strActionDescription);         
                //Sending exception to client side. - Prasanna
            ...
            { // cannot perform this action
                ...
                String errMsg= 
                        coeusMessageResourcesBean.parseMessageKey(
                                    "iacucProtocolAction_exceptionCode." + authCode);
                responder.setMessage(errMsg);
                //Sending exception to client side. - Prasanna
                CoeusException ex = new CoeusException(errMsg,1);
                responder.setDataObject(ex);
         ...
         UserInfoBean userBean) throws ServletException, IOException   
    {
        ...
        ProtocolActionsBean lastActionBean = null;
	...
	userId = userBean.getUserId();
            ...
            if(actionCode == IacucProtocolActionsConstants.REQUEST_TO_DEACTIVATE ||
                ...
                else if(authCode == 5000){   //No Action rights message
                    responder.setResponseStatus(false);
                    CoeusMessageResourcesBean coeusMessageResourcesBean 
                        =new CoeusMessageResourcesBean();
                    String errMsg = 
                            ...
                            if(actionCode == 208 && actionBean.getProtocolNumber() != null
                                ...
                                if(vecProtocolLink != null && vecProtocolLink.size() > 0){
                                    for(int index = 0; index < vecProtocolLink.size(); index++){
                                        ...
                                        //Commented and modified for COEUSQA-3119- Need to implement IACUC link to Award, IP, Prop Dev, and IRB - start
                                        //When special review inserted or deleted Email Notification should be send to IACUC module
                                        ...
                                        //COEUSDEV-75:End
                                        //Commented and modified for COEUSQA-3119- Need to implement IACUC link to Award, IP, Prop Dev, and IRB - end
                                    }
                                }
                            }
            ...
            if (statusChange)
            {
                ...
                //String orginalProtocolNumber = actionBean.getProtocolNumber();
                //Coeus enhancement Case #1791 - step 1: start
                //condition modified for internal issue fix 1882
                // if(actionCode==204 || actionCode==205 || actionCode==208 || actionCode==209){ 
                //Added for COEUSQA-2431 
                //If Approval or Expedited or Designated Approval get Original Protocol Number
                if(actionCode==204 || actionCode==205 || actionCode==212){ 
...
//                vecActionDetails.add(actionTxn.getHashUserPromptFlag());
                ...
                if(actionBean.getSubmissionNumber() == 0){
                    ...
                    // Added for COEUSQA-1724 start
                    //When action is performed from submission window
                    if(IacucProtocolActionsConstants.WITHDRAWN == actionBean.getActionTypeCode() ){
                        actionBean.setActionTypeCode(IacucProtocolActionsConstants.ADMINISTRATIVELY_WITHDRAWN);
                    }else if(IacucProtocolActionsConstants.DEACTIVATED == actionBean.getActionTypeCode() ){
                        actionBean.setActionTypeCode(IacucProtocolActionsConstants.ADMINISTRATIVELY_DEACTIVATED);
                    }
                    // Added for COEUSQA-1724 end
                }                
                // COEUSQA-1724 : Email Notifications For All Actions In IACUC - Start
                // Changes done since some actions will be triggered internally 
                // Based on the last performed action follow-up actions will be fetched
                // Action type code is updated since an action will trigger some other actions
                // (For example: Withdrawn action from the submission window will trigger 'Administratively Withdrawn action)
                lastActionBean = actionTxn.getLastActionPerformed(actionBean.getProtocolNumber(), actionBean.getSequenceNumber());
                int actionTypeCode = actionBean.getActionTypeCode();
                if(lastActionBean != null) {
                    actionTypeCode = lastActionBean.getActionTypeCode();
                    String actionTypeDesc = lastActionBean.getActionTypeDescription();
                    protocolActionChangesBean.setActionTypeCode(actionTypeCode);
                    protocolActionChangesBean.setActionTypeDescription(actionTypeDesc);
                    actionBean.setActionTypeCode(actionTypeCode);
                    actionBean.setActionTypeDescription(actionTypeDesc);
                }
                actionTxn.setActionCode(actionTypeCode);
                vecActionDetails.add(actionTxn.getSubAction());
                vecActionDetails.add(actionTxn.getHashUserPrompt());
                vecActionDetails.add(actionTxn.getHashUserPromptFlag());
