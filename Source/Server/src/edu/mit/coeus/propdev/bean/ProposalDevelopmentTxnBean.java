...
/**
 * This class provides the methods for performing all procedure executions for a
 * Proposal development functionality. Various methods are used to fetch the
 * Proposal development details from the Database. All methods are used
 * <code>DBEngineImpl</code> singleton instance for the database interaction.
 *
...
public class ProposalDevelopmentTxnBean implements TypeConstants {
	...
	RolodexMaintenanceDataTxnBean rolodexDataTxnBean = new RolodexMaintenanceDataTxnBean();
	...
	public int checkAgencyType(String sponsorCode, String primeSponsorCode) throws CoeusException, DBException {
		int sponsorTypeCode = -1;
		Vector param = new Vector();
		Vector result = new Vector();
		param.addElement(new Parameter("SPONSOR_CODE", DBEngineConstants.TYPE_STRING, sponsorCode));
		param.addElement(new Parameter("PRIME_SPONSOR_CODE", DBEngineConstants.TYPE_STRING, primeSponsorCode));
		/* calling stored function */
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER SPONSOR_TYPE>> = "
							+ " call fn_check_fed_type_for_sponsors(<<SPONSOR_CODE>> , <<PRIME_SPONSOR_CODE>> ) }",
					param);
		} else {
	...
	/**
	 * Check the lock for the given propsoal number. If the doesn't exists then
	 * lock it and send the notification.
	 *
	 * @param proposalNumber,
	 *            loggedIn user, Unit number @ return LockingBean Case #1769 -
	 *            start
	 */
	public LockingBean checkAndLock(String proposalNumber, String loggedinUser, String unitNumber)
		...
		// COEUSQA-1579 : End
	/*
	 * Added for Case# COEUSQA-1675-ability to delete Proposal Development
	 * proposals - Start
	 */

	/**
	 * To check if the Proposal can be deleted for the given Proposal number
	 * Function FN_CAN_DELETE_Proposal is used to check if Proposal can be
	 * deleted
	 *
	 * @param String
	 *            ProposalNumber
	 ...
	 * @return int canDelete canDelete = 0 - Proposal is in one of the 3
	 *         required status and is not linked to any module canDelete = 2 -
	 *         Proposal not in either 'Pending in Progress'
	 */
	public int checkCanDeleteProposal(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER ll_Ret>> = " + " call FN_CAN_DELETE_PROPOSAL(<< PROPOSAL_NUMBER >>) }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	// Added for COEUSQA-2548_Delete Proposal action issue with unlinked
	...
	public String checkIsProposalInHierarchy(String proposalNumber) throws CoeusException, DBException {
		...
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("AV_PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		/* calling stored function */
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call GET_EPS_PROP_HEADER_DETAILS(<< AV_PROPOSAL_NUMBER >>,<<OUT RESULTSET rset>>)", "Coeus",
					param);
	...
	public boolean checkS2SSubTyp(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER RESULTS>> = call FN_CHECK_PROP_S2S_SUB_TYP(<< AS_PROPOSAL >>" + " ) }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	/**
	 * This method gets all Abstracts Types. The return is a Vector(Collection)
	 * from OSP$ABSTRACT_TYPE table.
	 ...
	 */
	public Vector getAbstractTypes() throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			...
			result = dbEngine.executeRequest("Coeus", "call DW_GET_ABSTRACT_TYPES( <<OUT RESULTSET rset>> )", "Coeus",
					param);
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				abstractTypeFormBean
						.setAbstractTypeId(Integer.parseInt(abstractRow.get("ABSTRACT_TYPE_CODE").toString()));
				abstractTypeFormBean.setDescription((String) abstractRow.get("DESCRIPTION"));
				abstractTypeFormBean.setUpdateTimestamp((Timestamp) abstractRow.get("UPDATE_TIMESTAMP"));
				abstractTypeFormBean.setUpdateUser((String) abstractRow.get("UPDATE_USER"));
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public CoeusVector getAllProposalRoleRights() throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call GET_ALL_PROPOSAL_ROLE_RIGHTS(" + "<<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				roleRightInfoBean.setRightId((String) rightRoleRow.get("RIGHT_ID"));
				roleRightInfoBean.setRoleId(Integer.parseInt(rightRoleRow.get("ROLE_ID").toString()));
				roleRightInfoBean.setDescendFlag(rightRoleRow.get("DESCEND_FLAG") == null ? false
						: (rightRoleRow.get("DESCEND_FLAG").toString().equalsIgnoreCase("y") ? true : false));
				roleRightInfoBean.setDescription((String) rightRoleRow.get("DESCRIPTION"));
				roleRightInfoBean.setRightType(rightRoleRow.get("RIGHT_TYPE").toString().charAt(0));
				roleRightInfoBean.setRoleDescendFlag(rightRoleRow.get("ROLE_DESCEND_FLAG") == null ? false
						: (rightRoleRow.get("ROLE_DESCEND_FLAG").toString().equalsIgnoreCase("y") ? true : false));
	...
	public Vector getCertifications(String sponsorCode, String primeSponsorCode) throws CoeusException, DBException {
		Vector param = new Vector();
		Vector result = new Vector();
	 ...
	 *
	 * @param proposalnumber
	 *            and person Id this is given as input parameter for the
	...
	public Vector getCertifyAnswers(String proposalNumber, String personId) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_PROP_PERS_YNQ_FOR_PP(<<PROPOSAL_NUMBER>>,<<PERSON_ID>> ," + "<<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				/*
				 * ynqBean = new YNQBean(); yNQRow =
				 * (HashMap)result.elementAt(rowIndex); ynqBean.setQuestionId(
				 * (String) yNQRow.get("QUESTION_ID")); ynqBean.setAnswer(
				 * (String) yNQRow.get("ANSWER")); ynqBean.setUpdateTimeStamp(
				...
				proposalYNQFormBean.setProposalNumber((String) yNQRow.get("PROPOSAL_NUMBER"));
				proposalYNQFormBean.setPersonId((String) yNQRow.get("PERSON_ID"));
				proposalYNQFormBean.setQuestionId((String) yNQRow.get("QUESTION_ID"));
				proposalYNQFormBean.setAnswer((String) yNQRow.get("ANSWER"));
				proposalYNQFormBean.setUpdateTimeStamp((Timestamp) yNQRow.get("UPDATE_TIMESTAMP"));
				proposalYNQFormBean.setUpdateUser((String) yNQRow.get("UPDATE_USER"));
	...
	/**
	 * Method used to get proposallocation list from OSP$EPS_PROP_LOCATION for a
	 * given proposalnumber .
	 * <li>To fetch the data, it uses get_prop_location_list procedure.
	 *
	 * @param proposalNumber
	 *            get list of proposalLoctionList for this id
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getCertifyQuestions(String questionType) throws CoeusException, DBException {
		...
		if (questionType != null) {
			...
			if (dbEngine != null) {
				result = dbEngine.executeRequest("Coeus",
						"call DW_GET_YNQ_FOR_QTYPE ( <<QUESTION_TYPE>> , <<OUT RESULTSET rset>> )", "Coeus", param);
			} else {
				throw new CoeusException("db_exceptionCode.1000");
			}
			...
			if (result != null) {
				...
				for (int i = 0; i < result.size(); i++) {
					...
					questionListBean.setNoOfAnswers(Integer.parseInt(orgDetailsRow.get("NO_OF_ANSWERS") == null ? "0"
							: orgDetailsRow.get("NO_OF_ANSWERS").toString()));
					questionListBean.setExplanationRequiredFor((String) orgDetailsRow.get("EXPLANATION_REQUIRED_FOR"));
					questionListBean.setDateRequiredFor(orgDetailsRow.get("DATE_REQUIRED_FOR") == null ? null
							: orgDetailsRow.get("DATE_REQUIRED_FOR").toString());
					/*
					 * Added by Geo on 01/02/07 updating the status to the bean
					 * for fixing #2698
					 */
	 ...
	 * @param integer
	 *            module code to get custom data element count
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public int getCustomDataElementCount(int moduleCode) throws CoeusException, DBException {
		int count = 0;
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("MODULE_CODE", DBEngineConstants.TYPE_INT, new Integer(moduleCode).toString()));
		/* calling stored function */
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER COUNT>> = " + " call FN_MODULE_HAS_CUSTOM_DATA(<< MODULE_CODE >> ) }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	// code added for getting proposal disclosure details to displaying
	// DisclosureStatusForm - starts
	public Vector getDisclosureStatusDetails(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call GET_PROPOSAL_DISCLOSRE_STATUS( <<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	// Added for COEUSQA-1579 : For Hierarchy Proposal, Approval in Progress,
	// cannot sync after narratives updated on child proposal - Start
	/*
	...
	public String getEmailIdForPerson(String proposalNumber, String personId) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			// result = new Vector(3,2);
			result = dbEngine.executeRequest("Coeus",
					" call DW_GET_EMAIL_ID( <<PERSON_ID>>, " + " <<PROPOSAL_NUMBER>>, <<OUT RESULTSET rset>>) ",
					"Coeus", param);
		...
		if (listSize > 0) {
			...
			// proposalStatusCode =
			// Integer.parseInt(proposalDevRow.get("CREATION_STATUS_CODE").toString());
		}
		return emailId;
	}
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public int getIndividualQuestionCount() throws CoeusException, DBException {
		int count = 0;
		Vector param = new Vector();
		Vector result = new Vector();
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER COUNT>> = " + " call FN_GET_INDIV_QUEST_COUNT() }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	public CoeusVector getInvCreditTypes() throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			try {
				result = dbEngine.executeRequest("Coeus", "{ call GET_INV_CREDIT_TYPE(" + "<<OUT RESULTSET rset>> ) }",
						"Coeus", param);
			} catch (DBException dbEx) {
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				invCreditTypeBean.setInvCreditTypeCode(Integer.parseInt(row.get("INV_CREDIT_TYPE_CODE").toString()));

				invCreditTypeBean
						.setDescription(row.get("DESCRIPTION") == null ? "" : row.get("DESCRIPTION").toString());

				invCreditTypeBean
						.setAddsToHundred(row.get("ADDS_TO_HUNDRED").toString().equalsIgnoreCase("Y") ? true : false);

				invCreditTypeBean.setUpdateTimestamp((Timestamp) row.get("UPDATE_TIMESTAMP"));

				invCreditTypeBean.setUpdateUser((String) row.get("UPDATE_USER"));
	...
	/**
	 * This method is used to check whether it is ok to copy the Proposal Budget
	 * for the given Proposal Number and Unit number.
	 * <li>To fetch the data, it uses the function
	 * FN_IS_OK_TO_COPY_PROPOSAL_BUD.
	 *
	 * @return int count for the proposal number.
	 * @param proposalNumber
	 *            is given as the first input parameter to the function.
	 * @param unitNumber
	 *            is given as the second input parameter to the function.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	...
	public int getIsOkToCopyProposalBudget(String proposalNumber, String unitNumber)
		...
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		param.add(new Parameter("TARGET_UNIT", DBEngineConstants.TYPE_STRING, unitNumber));
		/* calling stored function */
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER COUNT>> = "
							+ " call FN_IS_OK_TO_COPY_PROPOSAL_BUD(<< PROPOSAL_NUMBER >> , << TARGET_UNIT >> ) }",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	public Vector getLocationTypes() throws CoeusException, DBException {
		...
		HashMap hmRow = null;
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus", "call GET_LOCATION_TYPES( <<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (locationTypeCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < locationTypeCount; rowIndex++) {
				hmRow = (HashMap) result.elementAt(rowIndex);
				vecLocationTypes.addElement(new ComboBoxBean(hmRow.get("LOCATION_TYPE_CODE").toString(),
	...
	// code added for getting proposal disclosure details to displaying
	// DisclosureStatusForm - ends
	public boolean getMaintanenceCertification(String loggedinUser, String proposalNumber)
			throws CoeusException, DBException {
		...
		HashMap hmRow = null;
		param.addElement(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		param.addElement(new Parameter("USER_ID", DBEngineConstants.TYPE_STRING, loggedinUser));
		if (dbEngine != null) {
			// result = new Vector(3,2);
			result = dbEngine.executeRequest("Coeus",
					"call dw_get_prop_rights_for_user( <<PROPOSAL_NUMBER>> ,<<USER_ID>>, " + "<<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (maintanenceCertificationCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < maintanenceCertificationCount; rowIndex++) {
				hmRow = (HashMap) result.elementAt(rowIndex);
				maintanenceCertificationCountVector.add(hmRow.get("RIGHT_ID"));
			}

		}
		if (maintanenceCertificationCountVector != null
				&& maintanenceCertificationCountVector.contains("MAINTAIN_PERSON_CERTIFICATION")) {
			hasRight = true;
		} else if (maintanenceCertificationCountVector != null
				&& maintanenceCertificationCountVector.contains("NOTIFY_PROPOSAL_PERSONS")) {
			hasRight1 = true;
		} else if (maintanenceCertificationCountVector != null
				&& maintanenceCertificationCountVector.contains("MAINTAIN_DEPT_PERSONNEL_CERTIFICATION ")) {
			hasRight = true;
		}
		return hasRight;
	}

	public boolean getMaintanenceNotification(String loggedinUser, String proposalNumber)
			throws CoeusException, DBException {
		boolean hasRight = false;
		...
		param.addElement(new Parameter("AW_USER_ID", DBEngineConstants.TYPE_STRING, loggedinUser));
		if (dbEngine != null) {
			// result = new Vector(3,2);
			result = dbEngine.executeRequest("Coeus",
	...
	public CoeusVector getNarrativeModule(String proposalNumber) throws CoeusException, DBException {
	...
	public CoeusVector getNarrativeRights(String proposalNumber) throws CoeusException, DBException {
		ProposalNarrativeTxnBean proposalNarrativeTxnBean = new ProposalNarrativeTxnBean();
	...
	// Added for COEUSQA-2778-Submited Prop to Sponsor-Narrative Incomplete -
	// Start
	/*
	 ...
	 *
	 *
	 * /** This method gets the proposal details information for the proposal
	 * tab in Proposal details screen. @ return ProposalDevelopmentFormBean that
	 * contain the proposal infn.
	 */
	...
	public ProposalDevelopmentFormBean getNewProposalDetails(String unitNumber, String userId)
		...
		/**
		 * The following lines are commnted. The organization id has to be
		 * pulled by executing FN_GET_ORGANIZATION_ID not by getting from
		 * parameter table Bug fix #1576
		 */
		...
		/**
		 * Bug Fix #1576.start Get the Organization id by executing
		 * FN_GET_ORGANIZATION_ID
		 */
		...
		organizationAddressFormBean = organizationMaintenanceDataTxnBean.getOrganizationAddress(organizationId);
		...
		proposalBean.setRolodexDetailsBean(
				rolodexMaintenanceDataTxnBean.getRolodexMaintenanceDetails(Integer.toString(contactAddressId)));
		...
		// CoeusConstants.INSTITUTE_UNIT_NUMBER));
		proposalBean.setPerOrganizationAddressFormBean(
				organizationMaintenanceDataTxnBean.getOrganizationAddress(organizationId));
		...
		// proposalBean.setPerformingOrganizationId(
		// CoeusConstants.INSTITUTE_UNIT_NUMBER );
		...
		proposalBean
				.setUserRolesInfoBean(getProposalUserRoles(newProposalNumber, proposalBean.getOwnedBy(), "N", userId));
	...
	/**
	 * This method used to generate the new proposal number when ever new
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public String getNextProposalNumber() throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT STRING PROPOSAL_NUMBER>> = " + " call FN_GET_NEXT_PROPOSAL_NUMBER( ) }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	/**
	 * This method used get max specail review number for the given Proposal
	 * Number
	 ...
	 * @param proposalNumber
	 *            is given as the input parameter for the procedure.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public int getNextPropSpecialReviewNumber(String proposalNumber) throws CoeusException, DBException {
		int reviewNumber = 0;
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		...
		/* calling stored function */
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus", "{ <<OUT INTEGER REVIEWNUMBER>> = "
	...
	public int getNextSiteNumber(String proposalNumber) throws DBException, CoeusException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER SITE_NUMBER>> = " + " call FN_GET_NEXT_SITE_NUMBER(<< PROPOSAL_NUMBER >>) }",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	public Vector getNotificationPersonsDetails(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			// result = new Vector(3,2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_SEND_NOTIFICATION( <<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (result != null) {
			...
			if (proposalPersonsCertifyCount > 0) {
				...
				for (int rowIndex = 0; rowIndex < proposalPersonsCertifyCount; rowIndex++) {
					...
					data.add(false);
					data.add(hmRow.get("FULL_NAME"));
					data.add(hmRow.get("LAST_NOTIFICATION_DATE"));
					personid.add(hmRow.get("PERSON_ID"));
	 ...
	 * for a given proposal number and others details for the loggedin user
	 * module.
	 * <li>It uses DW_GET_EPS_PROP_CUSTOM_DATA to fetch others data for given
	 * proposal number and DW_GET_CUST_COLUMNS_FOR_MODULE to get the custom data
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getOthersDetails(String proposalNumber) throws CoeusException, DBException {
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{<<OUT INTEGER COUNT>>=call fn_dev_prop_has_custom_data ( " + " << PROPOSAL_NUMBER >>)}", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
		...
		param.add(new Parameter("PARAM_NAME", DBEngineConstants.TYPE_STRING, "COEUS_MODULE_DEV_PROPOSAL"));
		result = dbEngine.executeFunctions("Coeus",
		...
		if ((customPropCount > 0) || (customModCount > 0)) {
			...
			/*
			 * else{ // there is no custom data available for the proposal
			 * module. return null; }
			 */
	...
	public boolean getParameterCertification(String parameterName) throws CoeusException, DBException {
		...
		HashMap hmRow = null;
		Vector result = null;
		param.addElement(new Parameter("as_parameter", DBEngineConstants.TYPE_STRING, parameterName));
		if (dbEngine != null) {
			// result = new Vector(3,2);
			// result = dbEngine.executeFunctions("Coeus","{<<OUT INTEGER
			// COUNT>> = call
			// FN_GET_UPDATEUSER_FOR_PPC(<<AV_MODULE_ITEM_KEY>>)", "Coeus",
			// param);

			result = dbEngine.executeFunctions("Coeus",
					"{<<OUT STRING COUNT>> = call get_parameter_value(<<as_parameter>>)}", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	public Vector getParamValue() throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call get_parameter_value( <<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				requesterB.setParameterValue(paramValue.get("parameter"));

				// enableProposaltoProtocolLink =
	 ...
	 * @param String
	 *            person id to get biography number
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public int getPersonBiographyNumber(String proposalId, String personId) throws CoeusException, DBException {

		int number = 0;
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalId));
		...
		if (dbEngine != null) {
			result = dbEngine
					.executeFunctions("Coeus",
							"{ <<OUT INTEGER NUMBER>> = "
									+ " call GET_MAX_FOR_PROP_PER_BIO( <<PROPOSAL_NUMBER>>, << PERSON_ID >> ) }",
							param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	/**
	 * This Method is used to get Person Editable Columns from
	 * OSP$PERSON_EDITABLE_COLUMNS table.
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getPersonEditableColumns() throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		PersonEditableColumnsFormBean personEditableColumnsFormBean = null;
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call DW_GET_PERSON_EDITABLE_COLUMNS( <<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				personEditableColumnsFormBean.setColumnName((String) editColumnRow.get("COLUMN_NAME"));
				personEditableColumnsFormBean.setUpdateTimestamp((Timestamp) editColumnRow.get("UPDATE_TIMESTAMP"));
				personEditableColumnsFormBean.setUpdateUser((String) editColumnRow.get("UPDATE_USER"));
	...
	/**
	 * This method is used to get Person Names, Unit Name and Unit Number for
	 * the given Proposal Number. It returns Vector(Collection) from
	 * OSP$EPS_PROP_UNITS table.
	 ...
	 */
	public Vector getPersonUnitNumberName(String proposalNumber) throws CoeusException, DBException {
		// int hasRight = 0;
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public String getPIUserName(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT STRING PI_NAME>> = " + " call fn_get_eps_prop_pi_name(<< PROPOSAL_NUMBER >> ) }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	// Dev, and IRB - start
	/*
	 * This Method gets the IACUC protocol(Special Review) status of the
	 * proposal it uses procedure GET_PROP_SPRV_AC_PROTO_STATUS to get the
	 * status code and status description for the particular protocol
	 *
	 * @param proposalNumber
	 *
	 * @param protocolNumber
	 *
	 * @param specialReviewNumber returns specialReviewFormBean
	 */
	public SpecialReviewFormBean getPropIacucProtocolStatus(String proposalNumber, String protocolNumber,
			int specialReviewNumber) throws CoeusException, DBException {
		...
		param.addElement(new Parameter("AW_PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		param.addElement(new Parameter("AW_PROTOCOL_NUMBER", DBEngineConstants.TYPE_STRING, protocolNumber));
		param.addElement(new Parameter("AW_SPECIAL_REVIEW_NUMBER", DBEngineConstants.TYPE_INT, specialReviewNumber));
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
		...
		if (!result.isEmpty()) {
			...
			specialReviewFormBean
					.setProtocolStatusCode(Integer.parseInt(infoProtocolRow.get("PROTOCOL_STATUS_CODE").toString()));
			specialReviewFormBean
					.setProtocolStatusDescription((String) infoProtocolRow.get("PROTOCOL_STATUS_DESCRIPTION"));
		}
	...
	// start case 2358
	public HashMap getPropLeadUnitOSPAdminHomeUnit(String propNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call getPropLUOspAdminHomeUnit( <<PROPOSAL_NUMBER>> , " + " <<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	/**
	 * This method gets all the proposal abstracts for the given proposal
	 * number. The return Vector(Collection) from OSP$EPS_PROP_ABSTRACT table.
	 ...
	 * @param proposalNumber
	 *            is given as input parameter for the procedure.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalAbstract(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_PROP_ABSTRACTS(<<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalAbstractFormBean.setProposalNumber((String) abstractRow.get("PROPOSAL_NUMBER"));
				proposalAbstractFormBean
						.setAbstractTypeCode(Integer.parseInt(abstractRow.get("ABSTRACT_TYPE_CODE").toString()));
				proposalAbstractFormBean
						.setAwAbstractTypeCode(Integer.parseInt(abstractRow.get("ABSTRACT_TYPE_CODE").toString()));
				proposalAbstractFormBean.setAbstractText(abstractRow.get("ABSTRACT").toString());
				proposalAbstractFormBean.setUpdateTimestamp((Timestamp) abstractRow.get("UPDATE_TIMESTAMP"));
				proposalAbstractFormBean.setUpdateUser((String) abstractRow.get("UPDATE_USER"));
	...
	public Vector getProposalActivityTypes() throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call DW_GET_ACTIVITY_TYPE( <<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (actTypeCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < actTypeCount; rowIndex++) {
				...
				vecProposalActTypes
						.addElement(new ComboBoxBean(hasProposalActTypes.get("ACTIVITY_TYPE_CODE").toString(),
	...
	/**
	 * Gets the proposal admin Details for the given proposal number. It returns
	 * a ProposalAdminFormBean from OSP$PROPOSAL_ADMIN_DETAILS table.
	 * <li>To fetch the data, it uses the procedure DW_GET_PROP_ADMIN_DETAILS.
	 *
	 * @param String
	 *            proposal Number to get certify details for proposal screen
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public ProposalAdminFormBean getProposalAdminDetails(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_PROP_ADMIN_DETAILS( <<PROPOSAL_NUMBER>>, <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			proposalAdminFormBean.setDevProposalNumber((String) adminDetailRow.get("DEV_PROPOSAL_NUMBER"));
			proposalAdminFormBean.setInstProposalNumber((String) adminDetailRow.get("INST_PROPOSAL_NUMBER"));
			proposalAdminFormBean
					.setInstPropSeqNumber(Integer.parseInt(adminDetailRow.get("INST_PROP_SEQUENCE_NUMBER") == null ? "0"
							: adminDetailRow.get("INST_PROP_SEQUENCE_NUMBER").toString()));
			proposalAdminFormBean.setDateSubmittedByDept((Timestamp) adminDetailRow.get("DATE_SUBMITTED_BY_DEPT"));
			proposalAdminFormBean.setDateReturnedToDept((Timestamp) adminDetailRow.get("DATE_RETURNED_TO_DEPT"));
			proposalAdminFormBean.setDateApprovedByOsp((Timestamp) adminDetailRow.get("DATE_APPROVED_BY_OSP"));
			proposalAdminFormBean.setDateSubmittedToAgency((Timestamp) adminDetailRow.get("DATE_SUBMITTED_TO_AGENCY"));
			proposalAdminFormBean.setCreatedDate((Timestamp) adminDetailRow.get("INST_PROP_CREATE_DATE"));
			proposalAdminFormBean.setCreatedUser((String) adminDetailRow.get("INST_PROP_CREATE_USER"));
			proposalAdminFormBean.setSignedBy((String) adminDetailRow.get("SIGNED_BY"));
			proposalAdminFormBean.setSubmissionType(adminDetailRow.get("SUBMISSION_TYPE").toString().charAt(0));

			// get the created by user name
			UserDetailsBean userDetailsBean = new UserDetailsBean();
			UserInfoBean userInfoBean = userDetailsBean
					.getUserInfo((String) adminDetailRow.get("INST_PROP_CREATE_USER"));
	...
	/**
	 * Method used to get proposal science code details from
	 * OSP$EPS_PROP_SCIENCE_CODE for a given proposal number
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalBioGraphy(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_PROP_PERBIO_FOR_P( <<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	/**
	 * Gets all the proposal admin Details for proposal number . The return
	 * Vector(Collection) from OSP$PROPOSAL_INV_CERTIFICATION table.
	 * <li>To fetch the data, it uses the procedure
	 * DW_GET_ALL_INV_CERT_FOR_PROP.
	 *
	 * @param String
	 *            proposal Number to get certify details for proposal screen
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalCertifyDetails(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_ALL_INV_CERT_FOR_PROP( <<PROPOSAL_NUMBER>>, <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalCertificationFormBean.setProposalNumber((String) certifyDetailRow.get("PROPOSAL_NUMBER"));
				proposalCertificationFormBean.setPersonId((String) certifyDetailRow.get("PERSON_ID"));
				proposalCertificationFormBean.setPersonName((String) certifyDetailRow.get("PERSON_NAME"));
				proposalCertificationFormBean.setCertifyFlag(certifyDetailRow.get("CERTIFIED_FLAG") == null ? false
						: (certifyDetailRow.get("CERTIFIED_FLAG").toString().equalsIgnoreCase("y") ? true : false));
				proposalCertificationFormBean.setDateCertify(certifyDetailRow.get("DATE_CERTIFIED") == null ? null
						: new Date(((Timestamp) certifyDetailRow.get("DATE_CERTIFIED")).getTime()));
				proposalCertificationFormBean.setDateReceivedByOsp(certifyDetailRow.get("DATE_RECEIVED_BY_OSP") == null
						? null : new Date(((Timestamp) certifyDetailRow.get("DATE_RECEIVED_BY_OSP")).getTime()));
				proposalCertificationFormBean.setUpdateTimestamp((Timestamp) certifyDetailRow.get("UPDATE_TIMESTAMP"));
				proposalCertificationFormBean.setUpdateUser((String) certifyDetailRow.get("UPDATE_USER"));
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public int getProposalCount(String proposalNumber) throws CoeusException, DBException {
		int count = 0;
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER COUNT>> = " + " call FN_PROPOSAL_OVERRIDES_EXISTS(<< PROPOSAL_NUMBER >> ) }",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	 ...
	 * @return int count for the proposal number
	 * @param String
	 *            proposal number to get custom data count
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public int getProposalCustomCount(String proposalNumber) throws CoeusException, DBException {
		int count = 0;
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER COUNT>> = " + " call FN_DEV_PROP_HAS_CUSTOM_DATA(<< PROPOSAL_NUMBER >> ) }",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	// place
	/*
	 * This function fetches all relevent information for a proposalNumber into
	 * a hashmap. Calls get_eps_prop_details_for_mail
	 */
	public HashMap getProposalDetailsForMail(String proposalNumber) throws DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call get_eps_prop_details_for_mail( << AW_PROPOSAL_NUMBER >> , " + " <<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new DBException("DB instance is not available");
		}
	 ...
	 * @param proposalnumber
	 *            this is given as input parameter for the procedure to execute.
	 * @return ProposalDevelopmentFormBean with proposal details for
	...
	public ProposalDevelopmentFormBean getProposalDevelopmentDetails(String proposalNumber)
		...
		if (listSize > 0) {
			...
			proposalDevelopmentFormBean.setCfdaNumber((String) proposalDevRow.get("CFDA_NUMBER"));
			...
			proposalDevelopmentFormBean.setAgencyProgramCode((String) proposalDevRow.get("AGENCY_PROGRAM_CODE"));
			proposalDevelopmentFormBean.setAgencyDivCode((String) proposalDevRow.get("AGENCY_DIVISION_CODE"));
			// end ele addition 5-24-05

			// COEUSQA-3951
			proposalDevelopmentFormBean
					.setAgencyRoutingIdentifier((String) proposalDevRow.get("AGENCY_ROUTING_IDENTIFIER"));
			proposalDevelopmentFormBean.setPreviousGGTrackingID((String) proposalDevRow.get("PREV_GG_TRACKID"));
			// COEUSQA-3951
			...
			proposalDevelopmentFormBean
					.setMailingAddressId(Integer.parseInt(proposalDevRow.get("MAILING_ADDRESS_ID") == null ? "-1"
			...
			proposalDevelopmentFormBean.setCopiedFromPropNum((String) proposalDevRow.get("COPIED_FROM_PROP_NUM"));
			// JM END
			...
			proposalDevelopmentFormBean.setAwardTypeCode(Integer.parseInt(proposalDevRow.get("AWARD_TYPE_CODE") == null
					? "0" : proposalDevRow.get("AWARD_TYPE_CODE").toString()));
			proposalDevelopmentFormBean.setAwardTypeDesc((String) proposalDevRow.get("AWARD_TYPE_DESCRIPTION"));
			// Added for Case 2162 - adding Award Type - End
			proposalDevelopmentFormBean
					.setProposalOverrideExists(getProposalCount(proposalDevelopmentFormBean.getProposalNumber()));
			 ...
			 * ":"); strBffr.append(departmentTxnBean.getUnitName(
			...
			organizationMaintenanceDataTxnBean = new OrganizationMaintenanceDataTxnBean();
			rolodexMaintenanceDataTxnBean = new RolodexMaintenanceDataTxnBean();
			rolodexDetailsBean = new RolodexDetailsBean();
			...
			organizationAddressFormBean = getSiteAddress("Proposal Organization",
					proposalDevelopmentFormBean.getSites());
			proposalDevelopmentFormBean.setOrganizationAddressFormBean(organizationAddressFormBean);
			// Added for case 2406 - Organization and Locations - end
			...
			// settings the organization details for the performing organization
			// id
			// Commented for case 2406 - Organization and Locations - start
			...
			// getting the protocol investigators and adding to proposalform
			// bean
			...
			proposalDevelopmentFormBean
					.setInvestigators(getProposalInvestigatorDetails(proposalDevelopmentFormBean.getProposalNumber()));
					// getting the proposal key persone and adding to proposal
					// key person form bean
					// new code to add unit section

			// ppc certify flag for key person ends
			proposalDevelopmentFormBean
					.setKeyPersonCertifyParam(this.getParameterCertification(ENABLE_PROP_PERSON_SELF_CERTIFY));
					// ppc certify flag for key person ends

			// old code..
			proposalDevelopmentFormBean
					.setKeyStudyPersonnel(getProposalKeyPersonDetails(proposalDevelopmentFormBean.getProposalNumber()));
			// getting the proposal science code and adding the form bean
			proposalDevelopmentFormBean
					.setScienceCode(getProposalScienceCode(proposalDevelopmentFormBean.getProposalNumber()));
			...
			proposalDevelopmentFormBean.setOthers(getOthersDetails(proposalDevelopmentFormBean.getProposalNumber()));
			// Commented for case 2406 - Organization and Locations - start
			...
			/*
			 * proposalDevelopmentFormBean.setLeadUnit(
			 * getProposalLeadUnitDetails(
			 * proposalDevelopmentFormBean.getProposalNumber()));
			 */

			// set certify data
			proposalDevelopmentFormBean.setInvestigatorQuestions(getCertifyQuestions("I"));
			...
			for (int rowIndex = 0; rowIndex < size; rowIndex++) {
				ProposalInvestigatorFormBean proposalInvestigatorFormBean = (ProposalInvestigatorFormBean) investigators
						.elementAt(rowIndex);
				String personId = proposalInvestigatorFormBean.getPersonId();
				// pass the personid and proposal no to get the answers
				Vector answers = getCertifyAnswers(proposalDevelopmentFormBean.getProposalNumber(), personId);

				// hashtable will hold personId as key and vector of
				// ProposalYNQFormBean as value
			...
			proposalDevelopmentFormBean.setUserRolesInfoBean(
					getProposalUserRoles(proposalNumber, proposalDevelopmentFormBean.getOwnedBy(), "M", ""));
			UserDetailsBean userDetailsBean = new UserDetailsBean();
			// getting data for User's for this Unit Number
			proposalDevelopmentFormBean
					.setUserInfo(userDetailsBean.getUserForUnit(proposalDevelopmentFormBean.getOwnedBy()));
			// getting Special Review Data
			// Commented and Added for COEUSQA-2984 : Statuses in special review
			// - start
			...
			Vector vecReviewData = getProposalSpecialReview(proposalDevelopmentFormBean.getProposalNumber());
			CoeusFunctions coeusFunctions = new CoeusFunctions();
			String specialReviewCode = coeusFunctions.getParameterValue(CoeusConstants.SPL_REV_TYPE_CODE_HUMAN);
			String specialReviewCodeForAnimal = coeusFunctions
					.getParameterValue(CoeusConstants.IACUC_SPL_REV_TYPE_CODE);
			...
			if (vecReviewData != null && vecReviewData.size() > 0) {
				if (PROPOSAL_SUBMITTED_STATUS == proposalDevelopmentFormBean.getCreationStatusCode()) {
					for (int index = 0; index < vecReviewData.size(); index++) {
						ProposalSpecialReviewFormBean proposalSpecialReviewFormBean = (ProposalSpecialReviewFormBean) vecReviewData
								.get(index);
						if (Integer.parseInt(specialReviewCode.trim()) == proposalSpecialReviewFormBean
								.getSpecialReviewCode()) {
							specialReviewFormBean = getPropProtocolStatus(
							...
							if (specialReviewFormBean != null) {
								proposalSpecialReviewFormBean
										.setApprovalDescription(specialReviewFormBean.getProtocolStatusDescription());
							}
							// Added for COEUSQA-3119- Need to implement IACUC
							// link to Award, IP, Prop Dev, and IRB - start
							// If special review code is 2 then get the details
							// for IACUC protocol
						} else if (Integer.parseInt(specialReviewCodeForAnimal.trim()) == proposalSpecialReviewFormBean
								.getSpecialReviewCode()) {
							specialReviewFormBean = getPropIacucProtocolStatus(
									proposalSpecialReviewFormBean.getProposalNumber(),
									proposalSpecialReviewFormBean.getProtocolSPRevNumber(),
									proposalSpecialReviewFormBean.getSpecialReviewNumber());
							if (specialReviewFormBean != null) {
			...
			try {
				...
				// System.out.println("NullPointerException raised. It means
				// that the Answer List is Null.");
			}

			// Commented the below lines. The initialization will be taken care
			// at the client
			// by the QuestionForm class
			/*
			 * ProposalYNQBean[] yqnArray = null; if (vctAnswerList == null) {
			 * vctAnswerList = new Vector(); yqnArray = new
			 * ProposalYNQBean[vctQuestionList.size()]; for (int i = 0; i <
			 * vctQuestionList.size(); i++) { yqnArray[i] = new
			 * ProposalYNQBean();
			 * yqnArray[i].setQuestionId(((QuestionListBean)vctQuestionList.
			 * elementAt( i )).getQuestionId());
			 * yqnArray[i].setProposalNumber(proposalNumber);
			 * yqnArray[i].setAcType("I"); yqnArray[i].setAnswer("");
			...
			proposalDevelopmentFormBean
					.setPropYNQExplanationList(hstExplanation == null ? new Hashtable() : hstExplanation);
					// End of Yes No Questions

			// QUESTIONNAIRE SEGMENT MODIFICATION STARTS
			try {
				vctAnswerList = getProposalQuestionnaire(proposalNumber);
			} catch (NullPointerException npe) {
			}
			proposalDevelopmentFormBean
					.setPropQuestionnaireAnswers(vctAnswerList == null ? new Vector() : vctAnswerList);
			...
			// ProposalActionTxnBean proposalActionTxnBean = new
			// ProposalActionTxnBean();
			//
			// proposalApprovalMapBean =
			// proposalActionTxnBean.getProposalApprovalMapForPK(proposalDevelopmentFormBean.getProposalNumber(),
			// 999);
			...
			if (routingBean != null) {
				...
				if (cvRoutingMaps != null) {
					...
					if (cvRejectedMaps == null || cvRejectedMaps.size() == 0) {
						...
						RoutingMapBean routingMapBean = routingTxnBean
								.getRoutingApprovalMap(routingBean.getRoutingNumber());
			...
			ProposalNarrativeTxnBean proposalNarrativeTxnBean = new ProposalNarrativeTxnBean();
			CoeusVector vctNarrative = proposalNarrativeTxnBean
					.getProposalNarrative(proposalDevelopmentFormBean.getProposalNumber());
			proposalDevelopmentFormBean.setNarrativeUserRights(vctNarrative);

			vctNarrative = proposalNarrativeTxnBean
					.getProposalNarrativeModules(proposalDevelopmentFormBean.getProposalNumber());
			proposalDevelopmentFormBean.setNarrativeModules(vctNarrative);
			// Get Narrative All Modules users - end

			// check the opportunity already selected for s2s submission and set
			// it to bean

			S2SSubmissionDataTxnBean s2sSubTxnBean = new S2SSubmissionDataTxnBean();
			proposalDevelopmentFormBean.setS2sOppSelected(
					null != s2sSubTxnBean.getLocalOpportunity(proposalDevelopmentFormBean.getProposalNumber()));

			proposalDevelopmentFormBean
					.setS2sCandidate(s2sSubTxnBean.isS2SCandidate(proposalDevelopmentFormBean.getProposalNumber()));
	...
	/**
	 * To get the proposal development details for a particular proposal
	 * Number @ return ProposalDevelopmentFormBean that contains the proposal
	 * development details
	 */
	public ProposalDevelopmentFormBean getProposalDevelopmentDetails(String proposalNumber, char functionType)
			throws CoeusException, DBException {
	...
	public ProposalDevelopmentFormBean getProposalDevelopmentDetails(String proposalNumber, String loggedinUser,
			String unitNumber, char functionType) throws CoeusException, DBException, LockingException {
	...
	/**
	 * This method is used to get whether the given proposal number has any
	 * Budget.
	 * <li>To fetch the data, it uses the function FN_BUDGET_EXISTS.
	 *
	 * @return int count for the proposal number.
	 * @param proposalNumber
	 *            is given as input parameter to the function.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public int getProposalHasBudget(String proposalNumber) throws CoeusException, DBException {
		int count = 0;
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER COUNT>> = " + " call FN_BUDGET_EXISTS(<< PROPOSAL_NUMBER >> ) }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	// Case 2106 End
	public HashMap getProposalHeader(String proposalId) throws CoeusException, DBException {
		Vector param = new Vector();
		Vector result = new Vector();
		// String proposalTitle = "";

		param.addElement(new Parameter("AV_PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalId));

		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call GET_EPS_PROPOSAL_HEADER(<<AV_PROPOSAL_NUMBER>> , "
					+ " << OUT STRING PROPOSAL_TITLE>>," + " << OUT STRING PI_NAME>>)", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalInvestigatorDetails(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		/*
		 * if(dbEngine !=null){ result = new Vector(3,2); result =
		 * dbEngine.executeRequest("Coeus",
		 * "call GET_PROPOSAL_INVESTIGATORS_DW( <<PROPOSAL_NUMBER>> , "+
		 * "<<OUT RESULTSET rset>> )", "Coeus", param); }else{ throw new
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalInvestigatorFormBean.setMultiPIFlag(proposalInvRow.get("MULTI_PI_FLAG") == null ? false
						: (proposalInvRow.get("MULTI_PI_FLAG").toString().equalsIgnoreCase("y") ? true : false));

				// Addedd for Case# 2270 - Tracking of Effort - Start
				proposalInvestigatorFormBean
						.setAcademicYearEffort(Float.parseFloat(proposalInvRow.get("ACADEMIC_YEAR_EFFORT") == null ? "0"
								: proposalInvRow.get("ACADEMIC_YEAR_EFFORT").toString()));
				proposalInvestigatorFormBean
						.setSummerYearEffort(Float.parseFloat(proposalInvRow.get("SUMMER_YEAR_EFFORT") == null ? "0"
								: proposalInvRow.get("SUMMER_YEAR_EFFORT").toString()));
				proposalInvestigatorFormBean
						.setCalendarYearEffort(Float.parseFloat(proposalInvRow.get("CALENDAR_YEAR_EFFORT") == null ? "0"
								: proposalInvRow.get("CALENDAR_YEAR_EFFORT").toString()));
				// Addedd for Case# 2270 - Tracking of Effort - End
				proposalInvestigatorFormBean.setCertifyFlag(proposalInvRow.get("CERTIFICATION_FLAG") == null ? false
						: (proposalInvRow.get("CERTIFICATION_FLAG").toString().equalsIgnoreCase("y") ? true : false));
				/* JM 9-7-2015 added person status */
				proposalInvestigatorFormBean.setStatus((String) proposalInvRow.get("STATUS"));
				/* JM END */

				/* JM 1-8-2016 added isExternalPerson flag */
				proposalInvestigatorFormBean.setIsExternalPerson((String) proposalInvRow.get("IS_EXTERNAL_PERSON"));
				/* JM END */

				// Adding the Lead unit details to the investigator tab
				proposalInvestigatorFormBean.setInvestigatorUnits(getProposalLeadUnitDetails(
	...
	/**
	 * Method used to get proposal lead unit details from OSP$EPS_PROP_UNITS for
	...
	public CoeusVector getProposalKeyPersonDetails(String proposalNumber) throws DBException, CoeusException {
		Vector result = new Vector(3, 2);
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_PROPOSAL_KEY_PERSONS( <<PROPOSAL_NUMBER>>," + " <<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (!result.isEmpty()) {
			...
			if (recCount > 0) {
				...
				for (int rowIndex = 0; rowIndex < recCount; rowIndex++) {
					...
					row = (HashMap) result.elementAt(rowIndex);
					proposalKeyPersonFormBean.setProposalNumber((String) row.get("PROPOSAL_NUMBER"));
					proposalKeyPersonFormBean.setSequenceNumber(Integer.parseInt(
					...
					proposalKeyPersonFormBean.setFacultyFlag("Y".equals(row.get("FACULTY_FLAG")) ? true : false);
					proposalKeyPersonFormBean
							.setNonMITPersonFlag("Y".equals(row.get("NON_MIT_PERSON_FLAG")) ? true : false);
					proposalKeyPersonFormBean.setPercentageEffort(Float.parseFloat(
							row.get("PERCENTAGE_EFFORT") == null ? "0" : row.get("PERCENTAGE_EFFORT").toString()));
					proposalKeyPersonFormBean
							.setAcademicYearEffort(Float.parseFloat(row.get("ACADEMIC_YEAR_EFFORT") == null ? "0"
									: row.get("ACADEMIC_YEAR_EFFORT").toString()));
					proposalKeyPersonFormBean.setSummerYearEffort(Float.parseFloat(
							row.get("SUMMER_YEAR_EFFORT") == null ? "0" : row.get("SUMMER_YEAR_EFFORT").toString()));
					proposalKeyPersonFormBean
							.setCalendarYearEffort(Float.parseFloat(row.get("CALENDAR_YEAR_EFFORT") == null ? "0"
									: row.get("CALENDAR_YEAR_EFFORT").toString()));
					proposalKeyPersonFormBean.setUpdateTimestamp((Timestamp) row.get("UPDATE_TIMESTAMP"));
					proposalKeyPersonFormBean.setUpdateUser((String) row.get("UPDATE_USER"));
					proposalKeyPersonFormBean.setKeyPersonsUnits(getProposalKeyPersonUnits(
							proposalKeyPersonFormBean.getProposalNumber(), proposalKeyPersonFormBean.getPersonId()));
					// ppc change for keyperson starts
					proposalKeyPersonFormBean.setPpcCertifyFlag(Integer.parseInt(
							row.get("CERTIFICATION_FLAG") == null ? "0" : row.get("CERTIFICATION_FLAG").toString()));
							// ppc change for keyperson ends

					/* JM 9-8-2015 add person status */
					proposalKeyPersonFormBean.setStatus((String) row.get("STATUS"));
					/* JM END */

					/* JM 1-8-2016 added isExternalPerson flag */
					proposalKeyPersonFormBean.setIsExternalPerson((String) row.get("IS_EXTERNAL_PERSON"));
					/* JM END */
	...
	public CoeusVector getProposalKeyPersonUnits(String proposalNumber, String personId)
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_PROPOSAL_KP_UNITS( <<PROPOSAL_NUMBER>> , <<PERSON_ID>>," + "<<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				keyPersonUnitBean.setProposalNumber((String) proposalLeadRow.get("PROPOSAL_NUMBER"));
	...
	// try{
	// ProposalDevelopmentTxnBean proposalDevelopmentTxnBean = new
	// ProposalDevelopmentTxnBean();
	// Vector vctCustomData =
	// proposalDevelopmentTxnBean.getOthersDetails("00000186");
	 ...
	 * <li>To fetch the data, it uses the function
	 * FN_GET_LEAD_UNIT_FOR_DEV_PROP.
	 *
	 * @return String leadUnit for the proposal number
	 * @param String
	 *            proposal number to get the lead unit
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public String getProposalLeadUnit(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT STRING UNIT_NUMBER>> = " + " call FN_GET_LEAD_UNIT_FOR_DEV_PROP(<< PROPOSAL_NUMBER >> ) }",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	/**
	 * Method used to get proposal lead unit details from OSP$EPS_PROP_UNITS for
	 * a given proposal number and personid.
	...
	public Vector getProposalLeadUnitDetails(String proposalNumber, String personId)
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalLeadRow = (HashMap) result.elementAt(rowIndex);
				proposalLeadUnitFormBean.setProposalNumber((String) proposalLeadRow.get("PROPOSAL_NUMBER"));
				proposalLeadUnitFormBean.setPersonId((String) proposalLeadRow.get("PERSON_ID"));
				proposalLeadUnitFormBean.setUnitNumber((String) proposalLeadRow.get("UNIT_NUMBER"));
				proposalLeadUnitFormBean.setUnitName((String) proposalLeadRow.get("UNIT_NAME"));
				proposalLeadUnitFormBean.setLeadUnitFlag(proposalLeadRow.get("LEAD_UNIT_FLAG") == null ? false
						: (proposalLeadRow.get("LEAD_UNIT_FLAG").toString().equalsIgnoreCase("y") ? true : false));
				proposalLeadUnitFormBean.setUpdateTimestamp((Timestamp) proposalLeadRow.get("UPDATE_TIMESTAMP"));
				proposalLeadUnitFormBean.setUpdateUser((String) proposalLeadRow.get("UPDATE_USER"));
	 ...
	 * <li>To fetch the data, it uses the procedure
	 * DW_GET_NOTICE_OF_OPPORTUNITY.
	 *
	 * @return Vector map of all Proposal Notice of Opportunity with Proposal
	 *         Notice of Opportunity code as key and proposal Notice of
	 *         Opportunity description as value.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	...
	public Vector getProposalNoticeOpp() throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call DW_GET_NOTICE_OF_OPPORTUNITY( <<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (actTypeCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < actTypeCount; rowIndex++) {
				...
				vecProposalNoticeOpp
						.addElement(new ComboBoxBean(hasProposalNoticeOpp.get("NOTICE_OF_OPPORTUNITY_CODE").toString(),
	 ...
	 *
	 * @return Vector map of all Proposal NSF code with Proposal NSFCode code as
	 *         key and proposal NSFCode description as value.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	...
	public Vector getProposalNSFCodes() throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call GET_NSF_CODES( <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (nsfCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < nsfCount; rowIndex++) {
				...
				vecProposalNSF.addElement(new ComboBoxBean(hasProposalNSF.get("NSF_CODE").toString(),
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalOthersDetails(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		ProposalCustomElementsInfoBean proposalOthersFormBean = null;
		HashMap othersRow = null;
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_EPS_PROP_CUSTOM_DATA( <<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalOthersFormBean.setProposalNumber((String) othersRow.get("PROPOSAL_NUMBER"));
				proposalOthersFormBean.setColumnName((String) othersRow.get("COLUMN_NAME"));
				proposalOthersFormBean.setColumnValue((String) othersRow.get("COLUMN_VALUE"));
				proposalOthersFormBean.setColumnLabel((String) othersRow.get("COLUMN_LABEL"));
				proposalOthersFormBean.setDataType((String) othersRow.get("DATA_TYPE"));
				proposalOthersFormBean.setDataLength(Integer.parseInt(
						othersRow.get("DATA_LENGTH") == null ? "0" : othersRow.get("DATA_LENGTH").toString()));
				proposalOthersFormBean.setDefaultValue((String) othersRow.get("DEFAULT_VALUE"));
				proposalOthersFormBean.setHasLookUp(othersRow.get("HAS_LOOKUP") == null ? false
						: (othersRow.get("HAS_LOOKUP").toString().equalsIgnoreCase("y") ? true : false));
				proposalOthersFormBean.setLookUpWindow((String) othersRow.get("LOOKUP_WINDOW"));
				proposalOthersFormBean.setLookUpArgument((String) othersRow.get("LOOKUP_ARGUMENT"));
				proposalOthersFormBean.setDescription((String) othersRow.get("DESCRIPTION"));
				proposalOthersFormBean.setUpdateTimestamp((Timestamp) othersRow.get("UPDATE_TIMESTAMP"));
				proposalOthersFormBean.setUpdateUser((String) othersRow.get("UPDATE_USER"));
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalOthersDetails(String proposalNumber, String personId) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		ProposalCustomElementsInfoBean proposalOthersFormBean = null;
		HashMap othersRow = null;
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_EPS_PROP_PER_CUST_DATA(<<PROPOSAL_NUMBER>>,<<PERSON_ID>> ," + "<<OUT RESULTSET rset>> )",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				othersRow = (HashMap) result.elementAt(rowIndex);
				proposalOthersFormBean.setProposalNumber((String) othersRow.get("PROPOSAL_NUMBER"));
				proposalOthersFormBean.setPersonId((String) othersRow.get("PERSON_ID"));
				proposalOthersFormBean.setColumnName((String) othersRow.get("COLUMN_NAME"));
				proposalOthersFormBean.setColumnValue((String) othersRow.get("COLUMN_VALUE"));
				proposalOthersFormBean.setColumnLabel((String) othersRow.get("COLUMN_LABEL"));
				proposalOthersFormBean.setDataType((String) othersRow.get("DATA_TYPE"));
	 ...
	 *
	 * @param proposalNumber
	 *            is used as an input parameter for the procedure
	 * @param personId
	 *            is used as an input parameter for the procedure
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public ProposalPersonFormBean getProposalPersonDetails(String proposalNumber, String personId)
			throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			...
			resultPerson = dbEngine.executeRequest("Coeus", "call DW_GET_PROP_PERSON_FOR_ONE ( <<PROPOSAL_NUMBER>>, "
		...
		proposalPersonFormBean.setProposalNumber((String) personRow.get("PROPOSAL_NUMBER"));
		proposalPersonFormBean.setPersonId((String) personRow.get("PERSON_ID"));
		proposalPersonFormBean.setAWPersonId((String) personRow.get("PERSON_ID"));
		proposalPersonFormBean.setSsn((String) personRow.get("SSN"));
		proposalPersonFormBean.setLastName((String) personRow.get("LAST_NAME"));
		proposalPersonFormBean.setFirstName((String) personRow.get("FIRST_NAME"));
		proposalPersonFormBean.setMiddleName((String) personRow.get("MIDDLE_NAME"));
		proposalPersonFormBean.setFullName((String) personRow.get("FULL_NAME"));
		proposalPersonFormBean.setPriorName((String) personRow.get("PRIOR_NAME"));
		proposalPersonFormBean.setUserName((String) personRow.get("USER_NAME"));
		proposalPersonFormBean.setEmailAddress((String) personRow.get("EMAIL_ADDRESS"));
		proposalPersonFormBean.setDateOfBirth(personRow.get("DATE_OF_BIRTH") == null ? null
				: new Date(((Timestamp) personRow.get("DATE_OF_BIRTH")).getTime()));
		proposalPersonFormBean
				.setAge(Integer.parseInt(personRow.get("AGE") == null ? "0" : personRow.get("AGE").toString()));
		proposalPersonFormBean.setAgeByFiscalYear(Integer.parseInt(
				personRow.get("AGE_BY_FISCAL_YEAR") == null ? "0" : personRow.get("AGE_BY_FISCAL_YEAR").toString()));
		proposalPersonFormBean.setGender((String) personRow.get("GENDER"));
		proposalPersonFormBean.setRace((String) personRow.get("RACE"));
		proposalPersonFormBean.setEduLevel((String) personRow.get("EDUCATION_LEVEL"));
		proposalPersonFormBean.setDegree((String) personRow.get("DEGREE"));
		proposalPersonFormBean.setMajor((String) personRow.get("MAJOR"));
		proposalPersonFormBean.setHandicap(personRow.get("IS_HANDICAPPED") == null ? false
				: (personRow.get("IS_HANDICAPPED").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setHandiCapType((String) personRow.get("HANDICAP_TYPE"));
		proposalPersonFormBean.setVeteran(personRow.get("IS_VETERAN") == null ? false
				: (personRow.get("IS_VETERAN").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setVeteranType((String) personRow.get("VETERAN_TYPE"));
		proposalPersonFormBean.setVisaCode((String) personRow.get("VISA_CODE"));
		proposalPersonFormBean.setVisaType((String) personRow.get("VISA_TYPE"));
		proposalPersonFormBean.setVisaRenDate(personRow.get("VISA_RENEWAL_DATE") == null ? null
				: new Date(((Timestamp) personRow.get("VISA_RENEWAL_DATE")).getTime()));
		proposalPersonFormBean.setHasVisa(personRow.get("HAS_VISA") == null ? false
				: (personRow.get("HAS_VISA").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setOfficeLocation((String) personRow.get("OFFICE_LOCATION"));
		proposalPersonFormBean.setOfficePhone((String) personRow.get("OFFICE_PHONE"));
		proposalPersonFormBean.setSecOfficeLocation((String) personRow.get("SECONDRY_OFFICE_LOCATION"));
		proposalPersonFormBean.setSecOfficePhone((String) personRow.get("SECONDRY_OFFICE_PHONE"));
		proposalPersonFormBean.setSchool((String) personRow.get("SCHOOL"));
		proposalPersonFormBean.setYearGraduated((String) personRow.get("YEAR_GRADUATED"));
		proposalPersonFormBean.setDirDept((String) personRow.get("DIRECTORY_DEPARTMENT"));
		proposalPersonFormBean.setSaltuation((String) personRow.get("SALUTATION"));
		proposalPersonFormBean.setCountryCitizenship((String) personRow.get("COUNTRY_OF_CITIZENSHIP"));
		proposalPersonFormBean.setPrimaryTitle((String) personRow.get("PRIMARY_TITLE"));
		proposalPersonFormBean.setDirTitle((String) personRow.get("DIRECTORY_TITLE"));
		proposalPersonFormBean.setHomeUnit((String) personRow.get("HOME_UNIT"));
		...
		proposalPersonFormBean.setFaculty(personRow.get("IS_FACULTY") == null ? false
				: (personRow.get("IS_FACULTY").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setGraduateStudentStaff(personRow.get("IS_GRADUATE_STUDENT_STAFF") == null ? false
				: (personRow.get("IS_GRADUATE_STUDENT_STAFF").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setResearchStaff(personRow.get("IS_RESEARCH_STAFF") == null ? false
				: (personRow.get("IS_RESEARCH_STAFF").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setServiceStaff(personRow.get("IS_SERVICE_STAFF") == null ? false
				: (personRow.get("IS_SERVICE_STAFF").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setSupportStaff(personRow.get("IS_SUPPORT_STAFF") == null ? false
				: (personRow.get("IS_SUPPORT_STAFF").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setOtherAcademicGroup(personRow.get("IS_OTHER_ACCADEMIC_GROUP") == null ? false
				: (personRow.get("IS_OTHER_ACCADEMIC_GROUP").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setMedicalStaff(personRow.get("IS_MEDICAL_STAFF") == null ? false
				: (personRow.get("IS_MEDICAL_STAFF").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setVacationAccural(personRow.get("VACATION_ACCURAL") == null ? false
				: (personRow.get("VACATION_ACCURAL").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setOnSabbatical(personRow.get("IS_ON_SABBATICAL") == null ? false
				: (personRow.get("IS_ON_SABBATICAL").toString().equalsIgnoreCase("y") ? true : false));
		proposalPersonFormBean.setProvided((String) personRow.get("ID_PROVIDED"));
		proposalPersonFormBean.setVerified((String) personRow.get("ID_VERIFIED"));
		proposalPersonFormBean.setUpdateTimestamp((Timestamp) personRow.get("UPDATE_TIMESTAMP"));
		proposalPersonFormBean.setUpdateUser((String) personRow.get("UPDATE_USER"));

		// Case #1602 Start
		proposalPersonFormBean.setAddress1((String) personRow.get("ADDRESS_LINE_1"));
		proposalPersonFormBean.setAddress2((String) personRow.get("ADDRESS_LINE_2"));
		proposalPersonFormBean.setAddress3((String) personRow.get("ADDRESS_LINE_3"));
		proposalPersonFormBean.setCity((String) personRow.get("CITY"));
		proposalPersonFormBean.setCounty((String) personRow.get("COUNTY"));
		proposalPersonFormBean.setState((String) personRow.get("STATE"));
		proposalPersonFormBean.setPostalCode((String) personRow.get("POSTAL_CODE"));
		proposalPersonFormBean.setCountryCode((String) personRow.get("COUNTRY_CODE"));
		proposalPersonFormBean.setFaxNumber((String) personRow.get("FAX_NUMBER"));
		proposalPersonFormBean.setPagerNumber((String) personRow.get("PAGER_NUMBER"));
		proposalPersonFormBean.setMobilePhNumber((String) personRow.get("MOBILE_PHONE_NUMBER"));
		proposalPersonFormBean.setEraCommonsUsrName((String) personRow.get("ERA_COMMONS_USER_NAME"));
		// Bug fix for Case #2059 by tarique Start 1
		proposalPersonFormBean
				.setSortId(personRow.get("SORT_ID") == null ? null : new Integer(personRow.get("SORT_ID").toString()));
		// Bug fix for Case #2059 by tarique End 1
		// Case #1602 End
		// COEUSQA-1674 - Allow Division Lead Unit to be modified in the person
		// details - Start
		proposalPersonFormBean.setDivision((String) personRow.get("DIVISION"));
		// COEUSQA-1674 - Allow Division Lead Unit to be modified in the person
		// details - End
	...
	/* CASE #748 End */
	/**
	 * Method to fetch the data for proposal printing. It uses
	 * get_sponsor_print_forms stored procedure to get the data. It returns a
	 * hash table of having two vector. One has got all package information and
	 * another has got page data.
	 *
	...
	public Hashtable getProposalPrintForms(String userId, String proposalNumber, String sponsCode, String primeSponCode)
		...
		param.addElement(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		param.addElement(new Parameter("SPONSOR_CODE", DBEngineConstants.TYPE_STRING, sponsCode));
		param.addElement(new Parameter("PRIME_SPONSOR_CODE", DBEngineConstants.TYPE_STRING, primeSponCode));
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call get_sponsor_print_forms(<< USER_ID >>,<< PROPOSAL_NUMBER >>,<< SPONSOR_CODE >>,<< PRIME_SPONSOR_CODE >>,<< OUT RESULTSET rset >>)",
					"Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			for (int index = 0; index < listSize; index++) {
				...
				int packageNumber = Integer
						.parseInt(invRow.get("PACKAGE_NUMBER") == null ? "1" : invRow.get("PACKAGE_NUMBER").toString());
				...
				propPrintPageFormBean.setPageNumber(Integer
						.parseInt(invRow.get("PAGE_NUMBER") == null ? "1" : invRow.get("PAGE_NUMBER").toString()));
	...
	public Vector getProposalQuestionnaire(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		ProposalYNQBean proposalYNQBean = null;
		HashMap yNQRow = null;
		param.addElement(new Parameter("av_module_item_key", DBEngineConstants.TYPE_STRING, proposalNumber));
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_QUESTIONNAIRE_ANS_GRANTS(<<av_module_item_key>>," + "<<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalYNQBean.setExplanation(
						((yNQRow.get("EXPLANATION")) == null) ? null : yNQRow.get("EXPLANATION").toString());
	...
	/**
	 * Method used to get proposalyesNoQuestion list from OSP$RIGHTS
	 * ,OSP$ROLE_RIGHTS and OSP$EPS_PROP_USER_ROLES for a given proposal number
	 * and user Id .
	 * <li>To fetch the data, it uses DW_GET_PROP_RIGHTS_FOR_USER procedure.
	 *
	 * @param proposalnumber
	 *            and userId get list of proposalRolesRight details
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalRightForUser(String proposalNumber, String userId) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalRoleRightFormBean.setRightId((String) rightRoleRow.get("RIGHT_ID"));
				proposalRoleRightFormBean.setDescription((String) rightRoleRow.get("DESCRIPTION"));
				proposalRoleRightFormBean.setRightType(rightRoleRow.get("RIGHT_TYPE").toString().charAt(0));
				proposalRoleRightFormBean.setDescendFlag(rightRoleRow.get("DESCEND_FLAG") == null ? false
						: (rightRoleRow.get("DESCEND_FLAG").toString().equalsIgnoreCase("y") ? true : false));
	 ...
	 *
	 * @param roleId
	 *            get list of proposalRoles details for this id
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalRightRole(int roleId) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		RoleRightInfoBean proposalRoleRightFormBean = null;
		HashMap rightRoleRow = null;
		param.addElement(new Parameter("ROLE_ID", DBEngineConstants.TYPE_INT, new Integer(roleId).toString()));

		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_ROLE_RIGHTS_FOR_ROLE(<<ROLE_ID>> ," + "<<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalRoleRightFormBean.setRightId((String) rightRoleRow.get("RIGHT_ID"));
				proposalRoleRightFormBean.setRoleId(Integer.parseInt(rightRoleRow.get("ROLE_ID").toString()));
				proposalRoleRightFormBean.setDescendFlag(rightRoleRow.get("DESCEND_FLAG") == null ? false
						: (rightRoleRow.get("DESCEND_FLAG").toString().equalsIgnoreCase("y") ? true : false));
				proposalRoleRightFormBean.setDescription((String) rightRoleRow.get("DESCRIPTION"));
				proposalRoleRightFormBean.setRightType(rightRoleRow.get("RIGHT_TYPE").toString().charAt(0));
				proposalRoleRightFormBean.setRoleDescendFlag(rightRoleRow.get("DESCEND_FLAG_1") == null ? false
						: (rightRoleRow.get("DESCEND_FLAG_1").toString().equalsIgnoreCase("y") ? true : false));
				rightRoleList.add(proposalRoleRightFormBean);
			}
		}
		return rightRoleList;
	...
	/**
	 * Method used to get proposal roles from osp$role for the given unit
	 * number.
	 * <li>To fetch the data, it uses GET_PROPOSAL_ROLES_FOR_UNIT.
	 *
	 * @param unitNumber
	 *            is used to get list of RoleInfoBean
	 * @return vector
	 * @exception DBException
	 *                if the instance of a dbEngine is null.
	 */
	public Vector getProposalRoles(String unitNumber) throws DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call GET_PROPOSAL_ROLES_FOR_UNIT ( <<UNIT_NUMBER>>, <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new DBException("DB instance is not available");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				roleInfoBean.setRoleId(
						Integer.parseInt(roleRow.get("ROLE_ID") == null ? "0" : roleRow.get("ROLE_ID").toString()));
				roleInfoBean.setRoleName((String) roleRow.get("ROLE_NAME"));
				roleInfoBean.setRoleDesc((String) roleRow.get("DESCRIPTION"));
				roleInfoBean.setRoleType(roleRow.get("ROLE_TYPE").toString().charAt(0));
				roleInfoBean.setUnitNumber((String) roleRow.get("OWNED_BY_UNIT"));
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalScienceCode(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_SCIENCE_CODES_FOR_PROP( <<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalScienceCodeFormBean.setProposalNumber((String) proposalScienceRow.get("PROPOSAL_NUMBER"));
				proposalScienceCodeFormBean.setScienceCode((String) proposalScienceRow.get("SCIENCE_CODE"));
				proposalScienceCodeFormBean.setDescription((String) proposalScienceRow.get("DESCRIPTION"));
				proposalScienceCodeFormBean.setUpdateTimestamp((Timestamp) proposalScienceRow.get("UPDATE_TIMESTAMP"));
				proposalScienceCodeFormBean.setUpdateUser((String) proposalScienceRow.get("UPDATE_USER"));
	 ...
	 * @return CoeusVector
	 */
	public CoeusVector getProposalSites(String proposalNumber) throws CoeusException, DBException {
	...
	/**
	 * This method gets Special Review data for the given Proposal Number. It
	 * returns a Vector(Collection) from OSP$EPS_PROP_SPECIAL_REVIEW table.
	 * <li>To fetch the data, it uses the procedure GET_PRO_SP_REV.
	 *
	 * @param proposalNumber
	 *            is given as the input parameter to the procedure.
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalSpecialReview(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_PRO_SP_REV( <<PROPOSAL_NUMBER>>, <<OUT RESULTSET rset>> )", "Coeus", param);
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalSpecialReviewFormBean.setProposalNumber((String) specialReviewRow.get("PROPOSAL_NUMBER"));
				proposalSpecialReviewFormBean.setSpecialReviewNumber(
						Integer.parseInt(specialReviewRow.get("SPECIAL_REVIEW_NUMBER").toString()));
				proposalSpecialReviewFormBean
						.setSpecialReviewCode(Integer.parseInt(specialReviewRow.get("SPECIAL_REVIEW_CODE").toString()));
				proposalSpecialReviewFormBean
						.setApprovalCode(Integer.parseInt(specialReviewRow.get("APPROVAL_TYPE_CODE").toString()));
				proposalSpecialReviewFormBean.setProtocolSPRevNumber((String) specialReviewRow.get("PROTOCOL_NUMBER"));
				proposalSpecialReviewFormBean.setApplicationDate(specialReviewRow.get("APPLICATION_DATE") == null ? null
						: new Date(((Timestamp) specialReviewRow.get("APPLICATION_DATE")).getTime()));
				proposalSpecialReviewFormBean.setApprovalDate(specialReviewRow.get("APPROVAL_DATE") == null ? null
						: new Date(((Timestamp) specialReviewRow.get("APPROVAL_DATE")).getTime()));
				proposalSpecialReviewFormBean.setComments((String) specialReviewRow.get("COMMENTS"));
				proposalSpecialReviewFormBean.setUpdateUser((String) specialReviewRow.get("UPDATE_USER"));
				proposalSpecialReviewFormBean.setUpdateTimestamp((Timestamp) specialReviewRow.get("UPDATE_TIMESTAMP"));
				proposalSpecialReviewFormBean
						.setSpecialReviewDescription((String) specialReviewRow.get("SPECIAL_REVIEW_DESCRIPTION"));
				proposalSpecialReviewFormBean
						.setApprovalDescription((String) specialReviewRow.get("APPROVAL_TYPE_DESCRIPTION"));
	 ...
	 *
	 * @return Vector map of all Proposal Status with Proposal item types code
	 *         as key and proposal item type description as value.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	...
	public Vector getProposalStatus() throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			// Start: Enhancement for ProposalStatus: Added new Procedure for
			// getting Proposal Status
			// from newly created table OSP$EPS_PROPOSAL_STATUS
			result = dbEngine.executeRequest("Coeus", "call GET_EPS_PROPOSAL_STATUS( <<OUT RESULTSET rset>> )", "Coeus",
					param);
		...
		if (typeCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < typeCount; rowIndex++) {
				...
				vecProposalStatus.addElement(new ComboBoxBean(hasProposalStatus.get("STATUS_CODE").toString(),
	...
	public ComboBoxBean getProposalStatusDetails(String proposalNumber) throws CoeusException, DBException {
		...
		if (listSize > 0) {
			proposalDevRow = (HashMap) result.elementAt(0);
			comboBoxBean = new ComboBoxBean(proposalDevRow.get("CREATION_STATUS_CODE").toString(),
	...
	/**
	 * Method used to get proposal title from OSP$EPS_PROPOSAL for a given
	 * proposal number
	 *
	 * @param proposalId
	 *            this is given as input parameter for the procedure to execute.
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public String getProposalTitle(String proposalId) throws CoeusException, DBException {
		Vector param = new Vector();
		Vector result = new Vector();
		String proposalTitle = "";
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_EPS_PROPOSAL_TITLE(<<AV_PROPOSAL_NUMBER>> , " + " << OUT STRING as_title>>)", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	 ...
	 *
	 * @return Vector map of all Proposal types with Proposal item types code as
	 *         key and proposal item type description as value.
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	...
	public Vector getProposalTypes() throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus", "call GET_PROPOSAL_TYPE_LIST( <<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (typeCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < typeCount; rowIndex++) {
				...
				vecProposalTypes.addElement(new ComboBoxBean(hasProposalTypes.get("PROPOSAL_TYPE_CODE").toString(),
	 ...
	 * @return Vector collections of PersonEditableColumnsFormBean
	 * @param proposalNumber
	 *            is used to get Users for this Proposal
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalUserRoles(String proposalNumber) throws CoeusException, DBException {
		UserDetailsBean userDetailsBean = new UserDetailsBean();
	 ...
	 *            is used to get Users for this Proposal
	 * @param unitNumber
	 *            is used to get Users for this Unit Number
	 * @param strMode
	 *            is used to differentiate the mode like New or Modify/Display
	 * @param loggedUserId
	...
	public Vector getProposalUserRoles(String proposalNumber, String unitNumber, String strMode, String loggedUserId)
		...
		if ((vecRolesBean != null) && (vecRolesBean.size() > 0)) {
			...
			for (int index1 = 0; index1 < outerlength; index1++) {
				...
				// If Modify/Display mode get all Users for this Proposal No.
				// and Role.
				if (strMode.equals("M")) {
					vecUserId = getUsersForProposalRole(proposalNumber, roleId);
				} else {
				...
				if ((vecUserId != null) && (vecUserId.size() > 0)) {
					...
					for (int index2 = 0; index2 < innerlength; index2++) {
						if (strMode.equals("M")) {
							// In modify/display mode vector contains
							// proposalUserRoleFormBean objects
							proposalUserRoleFormBean = (ProposalUserRoleFormBean) vecUserId.elementAt(index2);
							userId = proposalUserRoleFormBean.getUserId();
						} else {
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalYNQ(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		ProposalYNQBean proposalYNQBean = null;
		HashMap yNQRow = null;
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_PROPOSAL_YNQ(<<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalYNQBean.setProposalNumber((String) yNQRow.get("PROPOSAL_NUMBER"));
				proposalYNQBean.setQuestionId((String) yNQRow.get("QUESTION_ID"));
				proposalYNQBean.setAnswer((String) yNQRow.get("ANSWER"));
				proposalYNQBean.setExplanation((String) yNQRow.get("EXPLANATION"));
				proposalYNQBean
						.setReviewDate(yNQRow.get("REVIEW_DATE") == null ? null : yNQRow.get("REVIEW_DATE").toString());
				proposalYNQBean.setUpdateTimeStamp((Timestamp) yNQRow.get("UPDATE_TIMESTAMP"));
				proposalYNQBean.setUpdateUser((String) yNQRow.get("UPDATE_USER"));
				ynqList.add(proposalYNQBean);
			}
		}
		return ynqList;
	...
	/**
	 * Method used to get proposalyesNoQuestion list from OSP$YNQ_EXPLANATION
	 * for a given question Id .
	 * <li>To fetch the data, it uses DW_GET_YNQ_EXPLANATION procedure.
	 *
	 * @param questionId
	 *            get list of proposalyesNoQuestion for this id
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getProposalYNQExplantion(String questionId) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		ProposalYNQExplantionFormBean proposalYNQExplantionFormBean = null;
		HashMap yNQExplantionRow = null;
		param.addElement(new Parameter("QUESTION_ID", DBEngineConstants.TYPE_STRING, questionId));

		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_YNQ_EXPLANATION(<<QUESTION_ID>> ," + "<<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalYNQExplantionFormBean.setQuestionId((String) yNQExplantionRow.get("QUESTION_ID"));
				proposalYNQExplantionFormBean
						.setExplantionType(yNQExplantionRow.get("EXPLANATION_TYPE").toString().charAt(0));
				proposalYNQExplantionFormBean.setExplantion((String) yNQExplantionRow.get("EXPLANATION"));
				// Check This
	...
	public CoeusVector getPropPerCreditSplit(String proposalNo) throws CoeusException, DBException {
		...
		param.addElement(new Parameter("AV_PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNo));

		if (dbEngine != null) {
			result = new Vector(3, 2);
			try {
				result = dbEngine.executeRequest("Coeus",
						"{ call GET_EPS_PROP_PER_CREDIT_SPLIT(" + "<<AV_PROPOSAL_NUMBER>> , <<OUT RESULTSET rset>> ) }",
						"Coeus", param);
			} catch (DBException dbEx) {
				throw new CoeusException(dbEx.getMessage());
			}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				invCreditSplitBean.setModuleNumber(row.get("PROPOSAL_NUMBER").toString());

				invCreditSplitBean.setPersonId(row.get("PERSON_ID").toString());
				invCreditSplitBean
						.setPersonName(row.get("PERSON_NAME") == null ? "" : row.get("PERSON_NAME").toString());
	...
	public HashMap getPropPersonDetailsForMail(String proposalNumber) throws CoeusException, DBException {
		...
		HashMap hmRow = null;
		Vector param = new Vector();
		param.addElement(new Parameter("PROTOCOL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));

		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call GET_PROP_DETAILS_SEND_NOTIF( <<PROTOCOL_NUMBER>> , <<OUT RESULTSET rset>>) ", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	public Vector getPropPersonDetailsForNotif(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call GET_PROP_PERSONS_SEND_NOTIF( <<PROTOCOL_NUMBER>> , <<OUT RESULTSET rset>>) ", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		return result;
	...
	// COEUSQA-2984 : Statuses in special review - start
	/*
	 * This Method gets the protocol(Special Review) status of the proposal it
	 * uses procedure GET_PROP_SPREV_PROTO_STATUS to get the status code and
	 * status description for the particular protocol
	 *
	 * @param proposalNumber
	 *
	 * @param protocolNumber
	 ...
	 * @param specialReviewNumber returns specialReviewFormBean
	 */
	public SpecialReviewFormBean getPropProtocolStatus(String proposalNumber, String protocolNumber,
			int specialReviewNumber) throws CoeusException, DBException {
	...
	/**
	 * Get all the congressional districts for the given proposalNumber and
	 * siteNumber
	 *
	 * @param proposalNumber
	 * @param siteNumber
	...
	public CoeusVector getPropSiteCongDistricts(String proposalNumber, int siteNumber)
		...
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		param.add(new Parameter("SITE_NUMBER", DBEngineConstants.TYPE_INT, Integer.toString(siteNumber)));

		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus", "{ call GET_PROP_SITE_CONGR_DISTRICT(<< PROPOSAL_NUMBER >>, "
	...
	public CoeusVector getPropUnitCreditSplit(String proposalNo) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			try {
				result = dbEngine.executeRequest("Coeus", "{ call GET_EPS_PROP_UNIT_CREDIT_SPLIT("
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				invCreditSplitBean.setDescription(row.get("UNIT_NAME") == null ? "" : row.get("UNIT_NAME").toString());
				invCreditSplitBean.setCredit(
						row.get("CREDIT") == null ? new Double(0.0) : new Double(row.get("CREDIT").toString()));
	...
	/**
	 * Gets all the proposal role id and role name for user id . The return
	 * Vector(Collection) from OSP$ROlE table.the methods gets the user id by
	 * user id it gets the unit number by passing the unit number role id and
	 * role name is got.
	 * <li>To fetch the data, it uses the procedure
	 * DW_GET_PROPOSAL_ROLES_FOR_UNIT.
	 *
	 * @param String
	 *            user id to get role id and role name for proposal screen
	...
	public Vector getRoleAndRoleName(String userId) throws CoeusException, DBException {
		...
		if (typeCount > 0) {
			...
			for (int rowIndex = 0; rowIndex < typeCount; rowIndex++) {
				...
				vecProposalRoles.addElement(new ComboBoxBean(hasProposalRoles.get("ROLE_ID").toString(),
	...
	/**
	 * Method used to get Proposal Roles from OSP$EPS_PROP_USER_ROLES for the
	 * given Proposal Number.
	...
	public Vector getRolesForProposal(String proposalNumber) throws DBException, CoeusException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_ROLES_FOR_PROPOSAL ( <<PROPOSAL_NUMBER>>, <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
	 ...
	 *
	 * @param rolodex
	 *            id this is given as input parameter for the procedure to
	 *            execute.
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public String getRolodexAddress(String rolodexId) throws CoeusException, DBException {
		Vector result = new Vector(3, 2);
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call SELECT_ROLODEX( <<ROLODEX_ID>> , " + " <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		int listSize = result.size();
		...
		if (listSize > 0) {
			...
			strBffr.append((String) addressListRow.get("ORGANIZATION") == null ? ""
					: addressListRow.get("ORGANIZATION").toString() + "$#");
			strBffr.append((String) addressListRow.get("ADDRESS_LINE_1") == null ? ""
					: addressListRow.get("ADDRESS_LINE_1").toString() + "$#");
			strBffr.append((String) addressListRow.get("ADDRESS_LINE_2") == null ? ""
					: addressListRow.get("ADDRESS_LINE_2").toString() + "$#");
			strBffr.append((String) addressListRow.get("ADDRESS_LINE_3") == null ? ""
					: addressListRow.get("ADDRESS_LINE_3").toString() + "$#");
			strBffr.append(
					(String) addressListRow.get("CITY") == null ? "" : addressListRow.get("CITY").toString() + "$#");
			strBffr.append((String) addressListRow.get("COUNTY") == null ? ""
					: addressListRow.get("COUNTY").toString() + "$#");
			strBffr.append(
					(String) addressListRow.get("STATE") == null ? "" : addressListRow.get("STATE").toString() + " - ");
			strBffr.append((String) addressListRow.get("POSTAL_CODE") == null ? ""
					: addressListRow.get("POSTAL_CODE").toString() + "$#");
			strBffr.append((String) addressListRow.get("COUNTRY_CODE") == null ? ""
					: addressListRow.get("COUNTRY_CODE").toString());

			address = strBffr.toString();
	 ...
	 * @param String
	 *            rolodex name for the rolodex id passed
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	 *                if the instance of dbEngine is not available.
	 */
	public String getRolodexName(int rolodexId) throws DBException, CoeusException {
		Vector param = new Vector();
		Vector result = new Vector();

		String rolodexName = null;
		param.addElement(new Parameter("ROLODEX_ID", DBEngineConstants.TYPE_INT, new Integer(rolodexId).toString()));

		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call get_name_from_rolodex( << ROLODEX_ID >> , << OUT STRING ROLODEX_NAME >> ) ", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
	...
	/**
	 * Get the organization according to the given organization type from the
	 * vector cvSites
	 *
	 * @param type
	 *            - type of organization(values either Proposal Organization or
		...
		if (cvSites != null && eqOrganizationType != null) {
			...
			if (cvFilteredSites != null && cvFilteredSites.size() > 0) {
				...
				if (rolodexDetailsBean != null && rolodexDetailsBean.getRolodexId() != null) {
					organizationAddressFormBean
							.setContactAddressId(Integer.parseInt(rolodexDetailsBean.getRolodexId()));
				}
			}
		}
		return organizationAddressFormBean;
	...
	/**
	 * This method used gets Sponsor Code and Name for the given Dev. Proposal
	 * Number
	...
	public ComboBoxBean getSponsorForProposal(String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			...
			result = dbEngine.executeRequest("Coeus",
					"call GET_SPONSOR_FOR_PROPOSAL( <<PROPOSAL_NUMBER>> , <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		// int typeCount = result.size();
		...
		if (!result.isEmpty()) {
			...
			comboBoxBean
					.setCode(hasSponsor.get("SPONSOR_CODE") == null ? "" : hasSponsor.get("SPONSOR_CODE").toString());
			comboBoxBean.setDescription(
					hasSponsor.get("SPONSOR_NAME") == null ? "" : hasSponsor.get("SPONSOR_NAME").toString());
		}
		return comboBoxBean;
	}
	...
	public String getStatementForQuestion(String questionId) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT STRING resp_stmt>> = " + " call fn_get_resp_stmt_for_pi_ques(<< QUESTION_ID >> ) }",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	 ...
	 *                raised from the server side.
	 */
	public int getUserHasOSPRight(String userId, String rightId) throws CoeusException, DBException {
		int hasRight = 0;
		Vector param = new java.util.Vector();
		Vector result = new Vector();
		// calling stored function
	...
	/**
	 * This method is used to check whether the user has Proposal role. If the
	 * user has rights, it returns 1, else, returns 0. It uses the stored
	 * function FN_USER_HAS_PROPOSAL_ROLE to do the validation
	 *
	 * @param userId
	 *            is given as the input parameter to the function.
	 * @param proposalNumber
	...
	/**
	 * This method used get user has proposal right count for the
	 * userid,proposal number and right id.
	 ...
	 * @param String
	 *            user id ,proposal number and right id to get proposal right
	 *            count
	 * @exception DBException
	 *                if any error during database transaction.
	 * @exception CoeusException
	...
	public int getUserHasPropRightCount(String userId, String proposalNumber, String rightId)
		...
		Vector result = new Vector();
		param.add(new Parameter("PROPOSAL_NUMBER", DBEngineConstants.TYPE_STRING, proposalNumber));
		param.add(new Parameter("USER_ID", DBEngineConstants.TYPE_STRING, userId.trim()));
		param.add(new Parameter("RIGHT_ID", DBEngineConstants.TYPE_STRING, rightId.trim()));
		/* calling stored function */
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus", "{ <<OUT INTEGER COUNT>> = " + " call FN_USER_HAS_PROP_RIGHT( "
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getUsersForProposalRight(String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
					"call GET_USERS_FOR_PROP_RIGHT(<<PROPOSAL_NUMBER>> , " + "<<OUT RESULTSET rset>> )", "Coeus",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				proposalNarrativeModuleUsersFormBean.setUserId((String) narrativeRow.get("USER_ID"));
				...
				proposalNarrativeModuleUsersFormBean.setUserName(narrativeRow.get("USER_NAME").toString());
	...
	/**
	 * Gets all the proposal role for user Details for proposal number and role
	 * id. The return Vector(Collection) from OSP$EPS_PROP_USER_ROLES and
	 * OSP$USER table.
	 * <li>To fetch the data, it uses the procedure DW_GET_USERS_FOR_PROP_ROLE.
	 *
	 * @param String
	 *            proposal Number and integer role id to get roles for user
	 *            details.
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getUsersForProposalRole(String proposalNumber, int roleId) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		ProposalUserRoleFormBean proposalUserRoleFormBean = null;
		HashMap rightRoleRow = null;
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				rightRoleRow = (HashMap) result.elementAt(rowIndex);
				proposalUserRoleFormBean.setProposalNumber((String) rightRoleRow.get("PROPOSAL_NUMBER"));
				proposalUserRoleFormBean.setUserId((String) rightRoleRow.get("USER_ID"));
				proposalUserRoleFormBean.setRoleId(Integer
						.parseInt(rightRoleRow.get("ROLE_ID") == null ? "0" : rightRoleRow.get("ROLE_ID").toString()));
				proposalUserRoleFormBean.setUserName((String) rightRoleRow.get("USER_NAME"));
				proposalUserRoleFormBean.setUnitNumber((String) rightRoleRow.get("UNIT_NUMBER"));
				proposalUserRoleFormBean.setStatus(rightRoleRow.get("STATUS").toString().charAt(0));
				proposalUserRoleFormBean.setUnitName((String) rightRoleRow.get("UNIT_NAME"));
				proposalUserRoleFormBean.setUpdateTimestamp((Timestamp) rightRoleRow.get("UPDATE_TIMESTAMP"));
				proposalUserRoleFormBean.setUpdateUser((String) rightRoleRow.get("UPDATE_USER"));
	...
	/**
	 * Method used to get user id for combination of proposal number,seqnumber
	 * and roleid from OSP$EPS_PROP_USER_ROLES and OSP$USER
	 * <li>To fetch the data, it uses DW_GET_USERS_FOR_PROPOSAL_ROLE.
	 *
	 * @param proposalNumber
	 *            is used as input parameter for the procedure.
	 * @param roleId
	 *            is used as input parameter for the procedure.
	 ...
	 *                if the instance of a dbEngine is null.
	 */
	public Vector getUsersForPropRole(String proposalNumber, int roleId) throws DBException {
		Vector result = new Vector(3, 2);
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus", "call DW_GET_USERS_FOR_PROPOSAL_ROLE ( <<PROPOSAL_NUMBER>> , "
					+ " <<ROLE_ID>>, <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new DBException("DB instance is not available");
		}
	...
	/**
	 * Method used to get all user ids for the given Unit number from
	 * OSP$USER_ROLES, OSP$USER and OSP$ROLE tables
	 ...
	 *                if the instance of a dbEngine is null.
	 */
	public Hashtable getUsersForRole(String unitNumber) throws DBException {
		Vector result = new Vector(3, 2);
		Vector vecProposalUser = null;
		HashMap hasProposalUser = null;
		Vector param = new Vector();
		...
		if (dbEngine != null) {
			result = dbEngine.executeRequest("Coeus",
					"call DW_GET_DEFAULT_PROP_USER_ROLES ( <<UNIT_NUMBER>>, <<OUT RESULTSET rset>> )", "Coeus", param);
		} else {
			throw new DBException("DB instance is not available");
		}
	...
	public boolean getViewCertifications(String loggedinUser, String proposalNumber)
			throws CoeusException, DBException {
		boolean hasRight = false;
		...
		if (maintanenceCertificationCount > 0) {
			...
			if (maintanenceCertificationCountVector != null
					&& maintanenceCertificationCountVector.contains("VIEW_DEPT_PERSNL_CERTIFN")) {
	...
	/**
	 * This method gets all the Questions for the given QuestionType. The return
	 * Vector(Collection) from OSP$YNQ table.
	 ...
	 *                if the instance of dbEngine is not available.
	 */
	public Vector getYesNoQuestionList(String questionType, String proposalNumber) throws CoeusException, DBException {
		Vector result = null;
		Vector param = new Vector();
		QuestionListBean yNQBean = null;
		HashMap ynqRow = null;
		param.addElement(new Parameter("QUESTION_TYPE", DBEngineConstants.TYPE_STRING, questionType));
		param.addElement(new Parameter("MODULE_ITEM_KEY", DBEngineConstants.TYPE_STRING, proposalNumber));
		if (dbEngine != null) {
			result = new Vector(3, 2);
			result = dbEngine.executeRequest("Coeus",
		...
		if (listSize > 0) {
			...
			for (int rowIndex = 0; rowIndex < listSize; rowIndex++) {
				...
				yNQBean.setDescription((String) ynqRow.get("DESCRIPTION"));
				yNQBean.setNoOfAnswers(Integer
						.parseInt(ynqRow.get("NO_OF_ANSWERS") == null ? "0" : ynqRow.get("NO_OF_ANSWERS").toString()));

				yNQBean.setExplanationRequiredFor((String) ynqRow.get("EXPLANATION_REQUIRED_FOR"));
				yNQBean.setDateRequiredFor(
						ynqRow.get("DATE_REQUIRED_FOR") == null ? null : ynqRow.get("DATE_REQUIRED_FOR").toString());
				ynqList.add(yNQBean);
			}
		}
		return ynqList;
	...
	// COEUSQA-3956 ENDS
	/**
	 * This method is used to check whether the Budget information of the
	 * Proposal can be copied. This returns true if the Budget details can be
	 * copied else returns false.
	 *
	 * @param proposalNumber
	 *            is used as an input parameter to the procedure.
	 * @param userId
	 *            is used as an input parameter to the procedure.
	...
	public boolean isPropBudgetCopyAllowed(String proposalNumber, String userId, boolean hasRights)
		...
		if (getProposalHasBudget(proposalNumber) > 0) {
			// Modified for case#2903 - Modify all dev proposals should allow
			// you to copy proposal - start
			if (hasRights) {
				blnFlag = true;
			} else {
				...
				// you to copy proposal - End
		}
		return blnFlag;
	}

	/**
	 * This method is used to check whether the Narrative information of the
	 * Proposal can be copied. This returns true if the Narrative details can be
	 * copied else returns false.
	 *
	 * @param proposalNumber
	 *            is used as an input parameter to the procedure.
	...
	public boolean isUserHasRight(String user, String unitNumber, String rightId) throws CoeusException, DBException {
		int userRight = 0;
		boolean hasRight = false;
		Vector param = new Vector();
		Vector result = new Vector();
		param.add(new Parameter("AW_USER_ID", DBEngineConstants.TYPE_STRING, user));
		param.add(new Parameter("AW_UNIT_NUMBER", DBEngineConstants.TYPE_STRING, unitNumber));
		param.add(new Parameter("AW_RIGHT_ID", DBEngineConstants.TYPE_STRING, rightId));
		/* calling stored function */
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus",
					"{ <<OUT INTEGER RIGHTEXISTS>> = call fn_user_has_right(<< AW_USER_ID >>,"
							+ "<< AW_UNIT_NUMBER >> , << AW_RIGHT_ID >> ) }",
					param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (!result.isEmpty()) {
			...
			userRight = Integer.parseInt(rowRolodexId.get("RIGHTEXISTS").toString());
		}
		if (userRight == 1) {
			hasRight = true;
		} else if (userRight == 0) {
	...
	public boolean lockCheck(String proposalNumber, String loggedinUser) throws CoeusException, DBException {
		ProposalDevelopmentFormBean proposalDevelopmentFormBean = null;
		proposalDevelopmentFormBean = getProposalDevelopmentDetails(proposalNumber);
	 ...
	 * @exception Exception
	 */
	public LockingBean lockDevProposal(String proposalNumber, String loggedinUser, String unitNumber) throws Exception {
		String rowId = rowLockStr + proposalNumber;
		if (dbEngine != null) {
			conn = dbEngine.beginTxn();
		} else {
	...
	public LockingBean lockProposalRouting(String proposalNumber, String loggedinUser, String unitNumber)
			throws Exception {
	...
	public void updateLastNotificationDate(String personId, String proposalNumber) throws CoeusException, DBException {
		...
		if (dbEngine != null) {
			result = dbEngine
					.executeFunctions("Coeus",
							"{ <<OUT INTEGER NUMBER>> = "
									+ " call FN_UPD_LAST_NOTIFIC_DATE( <<PERSON_ID>>, << PROPOSAL_NUMBER >> ) }",
							param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
	...
	/* CASE #748 Begin */
	/*
	 * FOR USE IN COEUSWEB ONLY. TO BE REPLACED WHEN AUTHORIZATION API IS
	 * IMPLEMENTED FOR VIEW PROPOSAL.
	 */
	public boolean userCanViewProposal(String userId, String proposalNumber) throws DBException, CoeusException {
		...
		if (dbEngine != null) {
			result = dbEngine.executeFunctions("Coeus", "{ <<OUT INTEGER RET>> = call FN_USER_CAN_VIEW_PROPOSAL ( "
					+ "<<AW_USER_ID>> , <<AW_PROPOSAL_NUMBER>>) }", param);
		} else {
			throw new CoeusException("db_exceptionCode.1000");
		}
		if (result != null && !result.isEmpty()) {
