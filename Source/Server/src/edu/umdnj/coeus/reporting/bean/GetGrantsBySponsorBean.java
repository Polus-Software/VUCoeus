/**
 * $Header: /home/cvs/cvsroot/coeus/Source/WEB-INF/classes/edu/umdnj/coeus/reporting/bean/Attic/GetGrantsBySponsorBean.java,v 1.1.2.18 2007/09/18 18:23:05 cvs Exp $
 * $Log: GetGrantsBySponsorBean.java,v $
 * Revision 1.1.2.18  2007/09/18 18:23:05  cvs
 * Modify code for GAFA and START_DATE handling
 *
 * Revision 1.1.2.17  2007/09/06 19:29:45  cvs
 * Fiscal Year Delineation: tweak server-side code to reflect GUI changes
 *
 * Revision 1.1.2.16  2007/09/05 19:08:58  cvs
 * Server-side modifications for Fiscal Year Delineation
 *
 * Revision 1.1.2.15  2007/08/16 15:30:09  cvs
 * Add support for Headers per page
 *
 * Revision 1.1.2.14  2007/08/02 19:19:05  cvs
 * Enforce Snippet integrity in PDF output
 *
 * Revision 1.1.2.13  2007/07/31 13:36:19  cvs
 * Add Annual Reports server-side code
 * Move many methods to base class Reporting Base Bean
 *
 * Revision 1.1.2.12  2007/07/17 15:59:26  cvs
 * Display reports that total costs of 0 but with valid dates
 *
 * Revision 1.1.2.11  2007/06/11 15:39:13  cvs
 * Make sure that Sponsor Name and Project Title have nowrap = FALSE
 *
 * Revision 1.1.2.10  2007/03/27 15:19:20  cvs
 * Modify code to work with Award FnA Distributions
 *
 * Revision 1.1.2.9  2007/02/16 19:32:14  cvs
 * Correct misspelling of string Extramural
 *
 * Revision 1.1.2.8  2007/02/14 16:34:38  cvs
 * Check for empty reports
 *
 * Revision 1.1.2.7  2007/01/24 14:56:18  cvs
 ...
 *
 * Revision 1.1.2.5  2007/01/17 17:19:40  cvs
 * Add NIH support
 *
 * Revision 1.1.2.4  2007/01/17 15:26:35  cvs
 * Add CSV Support
 *
 * Revision 1.1.2.3  2007/01/04 19:09:34  cvs
 * Add support for Sponsor Type Graph by School by Sponsor Type
 *
 * Revision 1.1.2.2  2006/12/28 16:26:05  cvs
 * Correct anomaly found with displaying records in their correct subsection offset by 1
 *
 * Revision 1.1.2.1  2006/12/27 17:52:48  cvs
 *  Add support for grants by sponsor type data retrieval
 *
 *
 */
/*
 * @(#)GetGrantsBySponsorBean.java 
 *
...
public class GetGrantsBySponsorBean extends ReportingBaseBean
{
    ...
    /** Creates a new instance of GetGrantsBySponsorBean */
    public GetGrantsBySponsorBean() 
    {
	super();
        fiscalYear = 0;
        Initialization();
    ...
    public GetGrantsBySponsorBean(String UnitName, String FiscalYearType, String FiscalYear)
    {
        super();
        fiscalYear = 0;
    ...
    {
        Vector result = pobj.getGrantsBySponsor();
		...
		if (UnitName != null && UnitName.length()>0)
		{
                   ...
                   buf.append("<h2>Detailed Award List for "+UnitSchool+" by Sponsor Type</h2>");
		}
		else
		{
		   buf.append("<h2>Detailed Award List by Sponsor Type</h2>");
		}
		buf.append("<table border=1>");	
                buf.append("<tr>");
                    buf.append(printObj("<b><u>Award Number</u></b>"));
                    buf.append(printObj("<b><u>Sponsor Type</u></b>"));
            	...
            	for(int rowNum = 0; rowNum < listSize; rowNum++)
		{
                    ...
                    Object str15 = row.get("UNIT_NUMBER");

		    if (SponsorType != null && SponsorType.length() > 0)
		    {
			if (((String)str2).compareTo(SponsorType) != 0)
				continue;
  		    }
		    ...
		    if (UnitName != null && UnitName.length() > 0)
		    {
			 ...
			 if (((String)str7).compareTo(UnitName) == 0)
                            bfound = true;
                         if (((String)str15).compareTo(UnitName) == 0)
                            bfound = true;
			    ...
			    continue;
                    ...
                    if (fiscalYear > 0)
                    {
                       Object totalbegindtaecell = row.get("START_DATE");
                       Timestamp canddate = getTimeStampCellValue(totalbegindtaecell);                   
                       if (canddate == null ||
            ...
            try 
            {
                ...
                if (listSize > 0)
                {
                    ...
                    {           
                            ...
                            if (prevrow != null && (SponsorType == null || SponsorType.length()==0))
                            {
                            ...
                            if (inum == 0 || 
                                    ((SponsorType == null || SponsorType.length()==0) && strtitle.compareTo(prevstrtitle)!=0))
                            {
                               if (inum != 0)
                               {
                                  displayTotals(table);
    	...
    	throws DocumentException
    {
        ...
        String sponsorhead = "";
        if (SponsorType != null && SponsorType.length()>0)
            sponsorhead = " "+SponsorType;
    
        String strhead = "Research, Service, and Training Grants by Sponsor Type"+sponsorhead+"\nActive Extramural Grants and Contracts";
        if (UnitSchool.length() > 0)
            strhead = UnitSchool+"\nResearch, Service, and Training Grants by Sponsor Type"+sponsorhead+"\nActive Extramural Grants and Contracts";
    ...
    private void FormatRecord(HashMap row, PdfPTable table)
    	throws DocumentException
    {
        Object desccell = row.get("DESCRIPTION");
        Object unitcell = row.get("PARENT_UNIT_NUMBER");
        Object unitname = row.get("UNIT_NUMBER");
        if (SponsorType != null && SponsorType.length() > 0)
        {
            if (((String)desccell).compareTo(SponsorType) != 0)
               return;
        }
        ...
        {
           boolean bfound = false;
           if (((String)unitcell).compareTo(UnitName) == 0)
              bfound = true;
           if (((String)unitname).compareTo(UnitName) == 0)
              bfound = true;
    ...
    private void populateCurrentGrantsObject(HashMap row)
    {
        ...
        Object totalbegindtaecell = row.get("START_DATE");
        Object schoolName = row.get("PARENT_UNIT_NUMBER");
        String strSchool = getUnitSchool(getCellValue(schoolName).trim());
	...
	grantsobj.setInvestigatorName(getCellValue(investigatorcell).trim());
        grantsobj.setSchoolName(strSchool);
        if (fiscalYear == 0)
        {
            grantsobj.setAwardBeginDate(getCellValue(totalbegindtaecell).trim());    	
    		...
    		throws DocumentException
	{
            if (grantsobj.getAwardBeginDate().length()==0 && 
                    grantsobj.getAwardBeginDate().length()==0)
                return;
        ...
        j.add(new Chunk(getCurrencyFormattedNumber(grantsobj.getProjectTotalCost()),
        		FontFactory.getFont(FontFactory.HELVETICA, 8, Font.PLAIN, Color.BLACK)));
        PdfPCell cell10 = new PdfPCell(j);
        cell10.setBorderColor(Color.white);
        if (UnitName != null && UnitName.length() > 0)
            cell10.setColspan(8);
        else
            cell10.setColspan(1);
        cell10.setNoWrap(true);
        subtable.addCell(cell10);

        if (UnitName == null || UnitName.length() == 0)
        {
            String strSchool = grantsobj.getSchoolName();
            if (strSchool.length()== 0)
                strSchool = "Note - not affiliated with any unit";
            Paragraph ia = new Paragraph();
            ia.add(new Chunk(strSchool,
            		FontFactory.getFont(FontFactory.HELVETICA, 8, Font.BOLD, Color.BLACK)));
            PdfPCell cell9a = new PdfPCell(ia);
            cell9a.setBorderColor(Color.white); 
            cell9a.setColspan(8);
            cell9a.setNoWrap(true);
            subtable.addCell(cell9a);
        }
         ...
         throws ServletException, IOException
    {
                ...
                UnitName == null || UnitName.length() ==0)
        {
           ...
           PrintWriter writer = res.getWriter();
           writer.println("<html><body>");
           writer.print("Too many grant rows will be listed, please filter results by School and/or Sponsor Type.");
           writer.println("</body></html>"); 
           return;            
        }
	    ...
	    String strTitle = "Grants by Sponsor Type";
	    if (UnitName != null && UnitName.length() > 0)
	    {
            	String UnitSchool = getUnitSchool(UnitName);
		strTitle += " for "+UnitSchool;
	    }
	    ...
	    HSSFSheet sheet = wb.createSheet(strTitle);
	    ...
	    headerStyle.setFont(headerFont);
            ...
            row = sheet.createRow((short)0);
                ...
                cell = row.createCell((short)1);
                    cell.setCellValue("Sponsor Type");
	    ...
	    int currow = 0;
            for (int i=0; i < listSize; i++) 
            {
                dbrow=(HashMap)result.elementAt(i); 
                    ...
                    Object str15 = dbrow.get("UNIT_NUMBER");
		    if (SponsorType != null && SponsorType.length() > 0)
		    {
			if (((String)str2).compareTo(SponsorType) != 0)
				continue;
		    ...
		    if (UnitName != null && UnitName.length() > 0)
		    {
			 ...
			 if (((String)str7).compareTo(UnitName) == 0)
			    ...
			    continue;
                    ...
                    if (fiscalYear > 0)
                    {
                       Object totalbegindtaecell = dbrow.get("START_DATE");
                       Timestamp canddate = getTimeStampCellValue(totalbegindtaecell);                   
                       if (canddate == null ||
                           canddate.getTime() < begindate.getTime() || 
                           canddate.getTime() > enddate.getTime())
       ...
       else
       {
           ...
           writer.println("<html><body>");
           String UnitSchool = getUnitSchool(UnitName);
           writer.print("No grants");
           writer.println("</body></html>"); 
       }

    }    
    
    public void setSponsorType(String sponsor)
    {
	this.SponsorType = sponsor;
        if (sponsor == null)
           SponsorType = "";
        if (pobj != null)
            pobj.setSponsorType(sponsor);
    }
        ...
        if (result != null && result.size() > 0)
        {
            ...
            if (result != null)
             ...
             if (listSize > 0)
             {
                    ...
                    buf.append(",");
                    buf.append(printPlainObj("Sponsor Type"));
                    buf.append(",");
