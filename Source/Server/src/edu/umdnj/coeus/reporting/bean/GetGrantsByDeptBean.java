...
public class GetGrantsByDeptBean extends ReportingBaseBean
{
    ...
    /** Creates a new instance of GetGrantsByDeptBean */
    public GetGrantsByDeptBean() 
    {
	super();
        fiscalYear = 0;
        Initialization();
    ...
    public GetGrantsByDeptBean(String UnitName, String FiscalYearType, String FiscalYear)
    {
        super();
    ...
    {
        Vector result = pobj.getGrantsByDepartment(UnitName);
        ...
        if (result != null && result.size() > 0)
        {
            ...
            if (result != null)
             ...
             if (listSize > 0)
             {
                ...
                buf.append("<h2>Detailed Award List for "+UnitSchool+"</h2>");
            	...
            	for(int rowNum = 0; rowNum < listSize; rowNum++)
		{
                    ...
                    if (fiscalYear > 0)
                    {
                       Object totalenddatecell = row.get("START_DATE");
                ...
                if (listSize > 0)
                {
                    ...
                    for (int inum = 0; inum < listSize; inum++)
                    {
                        ...
                        if (UnitName != null && UnitName.length()> 0)
                            index = strtitle.indexOf("-"+UnitName);
    	                if (index > 0)
    		           strtitle = strtitle.substring(0,index);
                        if (DeptName != null && DeptName.length() > 0 && strtitle.compareTo(DeptName) != 0)
                           continue;
                    ...
                    {           
                            ...
                            if (inum > 0)
                                prevrow = (HashMap)avector.elementAt(inum-1);
                            ...
                            if (prevrow != null)
                            {
                               prevcell = prevrow.get("UNIT_NAME");
                    ...
                    if (ititlechanged == 0 && grantsobj != null)
        	    {
                ...
                if (fiscalYear > 0)
                {
                    ...
                    if (canddate != null)
                    {
                        if (canddate.getTime() < begindate.getTime() || 
                                canddate.getTime() > enddate.getTime())
                                return;
                        grantsobj.setAwardBeginDate(getCellValue(totalenddatecell).trim());    	
                    }
                }
      		else
		{
    	...
    	throws DocumentException
	{
        ...
        FormatDataCell(subtable,getCurrencyFormattedNumber(grantsobj.getProjectTotalCost()),Color.white,Color.white,Element.ALIGN_LEFT,Font.PLAIN,8,8);
